...
| 13 | 技術設計が承認された | 人間がアーキテクチャ一貫性・仕様変更妥当性・タスク粒度等を確認しゲート通過 |
| 14 | 技術設計が差し戻された | アーキテクチャ不整合、タスク粒度の問題等で再設計を要求 |
| 15 | 技術設計の生成に失敗した | トークン枯渇・エラー等でAIが完了できなかった |
| 16 | 仕様変更が検出された | 技術設計の内容からドメイン構造・ユースケース仕様への仕様変更を検出 |

### Delivery — タスク実装

| # | イベント名（過去形） | 説明 |
|---|---|---|
| 17 | テストが作成された | TDDサイクルでテストコードを先行作成 |
| 18 | タスクが実装された | 1タスク＝1コミットの粒度で実装完了 |
| 19 | 実装に失敗した | テストが通らない、実装方針が行き詰まる等で完了できなかった |

### Delivery — デリバリー実行

| # | イベント名（過去形） | 説明 |
|---|---|---|
| 20 | Planが妥当と確認された | 実装開始前にPlanの妥当性を検証し、問題なしと判断 |
| 21 | 実装中にPlanの不備が発覚した | タスク分解の漏れや前提の誤りに気づいた |
| 22 | 実装中にドメイン構造の誤りが発覚した | 集約境界や状態遷移がビジネス実態と合わないことが判明 |
| 23 | 検証ゲートを通過した | 型チェック＋lint＋テスト全パス |
| 24 | 検証ゲートが失敗した | 型エラー・lint違反・テスト失敗 |
| 25 | セルフレビューが完了した | 別コンテキストのAIによるレビューで問題なし |
| 26 | セルフレビューで指摘が見つかった | セルフレビューで修正が必要と判断 |

### Delivery — PRレビュー

| # | イベント名（過去形） | 説明 |
|---|---|---|
| 27 | PRが作成された | セルフレビュー通過後にPRを作成 |
| 28 | 自動AIレビューが完了した | CIのAIレビューボットが差分を分析 |
| 29 | 人間レビューでApproveされた | 人間レビュアーが承認 |
| 30 | PRが差し戻された | 人間レビューでRequest Changes |
| 31 | レビューでドメイン構造の問題が指摘された | レビュー中に集約・状態遷移レベルの設計問題が発覚 |
| 32 | PRがマージされた | CI全通過＋人間Approve後にマージ |

### 成果物ライフサイクル

| # | イベント名（過去形） | 説明 |
|---|---|---|
| 33 | ドメイン構造が更新された | イベント・集約・状態遷移を改訂 |
| 34 | ユースケース仕様が更新された | 受け入れ基準の追記・変更 |
| 35 | ユースケース仕様が陳腐化した | 実装との乖離が一定以上になった |
| 36 | ユースケース仕様の対象機能が廃止された | 機能そのものが不要になった |

### 横断的イベント（どのコマンド実行中でも発生しうる）

| # | イベント名（過去形） | 説明 |
|---|---|---|
| 37 | ユースケース仕様の前提が崩れた | 外部要因や発見により仕様の前提条件が無効化 |
| 38 | AIがエラーで停止した | トークン上限・API障害・タイムアウト等のオペレーション的障害 |
...
| 9 | 仕様変更を検出する | 仕様変更が検出された |
| 10 | 技術設計をレビューする | 技術設計が承認された / 差し戻された |

### Delivery — タスク実装

| # | コマンド名 | トリガーするイベント |
|---|---|---|
| 11 | テストを作成する | テストが作成された |
| 12 | プロダクションコードを実装する | タスクが実装された / 実装に失敗した |

### Delivery — デリバリー実行

| # | コマンド名 | トリガーするイベント |
|---|---|---|
| 13 | Planを検証する | Planが妥当と確認された / Planの不備が発覚した / ドメイン構造の誤りが発覚した |
| 14 | 検証ゲートを実行する | 検証ゲートを通過した / 失敗した |
| 15 | セルフレビューを実行する | セルフレビューが完了した / 指摘が見つかった |
...
|---|---|---|
| 16 | PRを作成する | PRが作成された |
| 17 | 自動AIレビューを実行する | 自動AIレビューが完了した |
| 18 | PRをレビューする | Approveされた / 差し戻された / ドメイン構造の問題が指摘された |
| 19 | PRをマージする | PRがマージされた |

### 成果物ライフサイクル

| # | コマンド名 | トリガーするイベント |
|---|---|---|
| 20 | ドメイン構造を更新する | ドメイン構造が更新された |
| 21 | ユースケース仕様を更新する | ユースケース仕様が更新された |
| 22 | ユースケース仕様の鮮度を確認する | 陳腐化した / 対象機能が廃止された |
...
| 5 | AI（plan-issue） | AI | 技術設計を生成する、仕様変更を検出する |
...
| # | ポリシー | トリガー | 実行するコマンド |
|---|---|---|---|
| 1 | 変更検出ポリシー | ユースケース仕様が作成/更新された時、ドメイン構造が更新された時 | 変更を検出する |
...
| 6 | 仕様変更検出ポリシー | 技術設計が生成された時 | 仕様変更を検出する |
...
**責務**: 技術設計・タスク分解・仕様変更検出の管理

**含むコマンド**: 技術設計を生成する、仕様変更を検出する、レビューする

**含むイベント**: 生成された、仕様変更が検出された、承認された、差し戻された、生成に失敗した
...
| 生成済み | 技術設計が出力され、仕様変更検出ポリシーの実行を待っている | 生成中 | レビュー中 |
| 失敗 | トークン枯渇・エラー等で生成が完了しなかった | 生成中 | 生成中, 破棄 |
| レビュー中 | 人間がアーキテクチャ一貫性・ドメイン構造整合・仕様変更妥当性・テストレベル選択・タスク粒度を確認している | 生成済み | 承認済み, 生成中 |
...
| # | ホットスポット | 関連する集約/イベント | 解消アクション |
|---|---|---|---|
| 10 | 「振る舞い変更」の定義が例示のみで基準がない | 技術設計 | **解消済み（#96）**: 仕様変更・技術設計変更の2層に分離して定義。event-storming.mdへの反映は#106で実施 |
| 11 | 振る舞い変更検出ポリシーの検出対象（システム間連携IF等）の網羅基準がない | 技術設計 | **解消済み（#96）**: カテゴリ列挙は行わず、原則ベースで定義。event-storming.mdへの反映は#106で実施 |
...
| 16 | 変更検出ポリシーが「いつからいつまでの変更」を検出対象とするか未定義（前回検出時点からの差分？最終マージからの差分？） | デリバリーアイテム | 変更検出の基準時点を定義 |
| 17 | ポリシー#1,#2が参照するコマンド（「変更を検出する」）とイベント（「変更が検出された」）がドメインイベント/コマンド一覧に未定義 | ポリシー | **一部解消（#106）**: ポリシー#6のコマンド/イベントを追加済み。ポリシー#1,#2分は#95対応時に追加 |
...
| 仕様変更 | ドメイン構造やユースケース仕様が変わる変更（変更パターンA/Cに該当） | 振る舞い変更セクションの必須記述対象 |
| 技術設計変更 | コードベースだけでは妥当性を判断しにくく、修正コストが高い設計判断 | 振る舞い変更セクションのピックアップ対象。仕様変更に伴う技術設計への影響も対象に含む |
