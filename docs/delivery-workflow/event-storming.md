...
| 17 | 仕様変更が検出されなかった | 技術設計の内容に仕様変更が含まれていないことを確認 |

### Delivery — タスク実装

| # | イベント名（過去形） | 説明 |
|---|---|---|
| 18 | テストが作成された | TDDサイクルでテストコードを先行作成 |
| 19 | タスクが実装された | 1タスク＝1コミットの粒度で実装完了 |
| 20 | 実装に失敗した | テストが通らない、実装方針が行き詰まる等で完了できなかった |

### Delivery — デリバリー実行

| # | イベント名（過去形） | 説明 |
|---|---|---|
| 21 | Planが妥当と確認された | 実装開始前にPlanの妥当性を検証し、問題なしと判断 |
| 22 | 実装中にPlanの不備が発覚した | タスク分解の漏れや前提の誤りに気づいた |
| 23 | 実装中にドメイン構造の誤りが発覚した | 集約境界や状態遷移がビジネス実態と合わないことが判明 |
| 24 | 検証ゲートを通過した | 型チェック＋lint＋テスト全パス |
| 25 | 検証ゲートが失敗した | 型エラー・lint違反・テスト失敗 |
| 26 | セルフレビューが完了した | 別コンテキストのAIによるレビューで問題なし |
| 27 | セルフレビューで指摘が見つかった | セルフレビューで修正が必要と判断 |

### Delivery — PRレビュー

| # | イベント名（過去形） | 説明 |
|---|---|---|
| 28 | PRが作成された | セルフレビュー通過後にPRを作成 |
| 29 | 自動AIレビューが完了した | CIのAIレビューボットが差分を分析 |
| 30 | 人間レビューでApproveされた | 人間レビュアーが承認 |
| 31 | PRが差し戻された | 人間レビューでRequest Changes |
| 32 | レビューでドメイン構造の問題が指摘された | レビュー中に集約・状態遷移レベルの設計問題が発覚 |
| 33 | PRがマージされた | CI全通過＋人間Approve後にマージ |

### 成果物ライフサイクル

| # | イベント名（過去形） | 説明 |
|---|---|---|
| 34 | ドメイン構造が更新された | イベント・集約・状態遷移を改訂 |
| 35 | ユースケース仕様が更新された | 受け入れ基準の追記・変更 |
| 36 | ユースケース仕様が陳腐化した | 実装との乖離が一定以上になった |
| 37 | ユースケース仕様の対象機能が廃止された | 機能そのものが不要になった |

### 横断的イベント（どのコマンド実行中でも発生しうる）

| # | イベント名（過去形） | 説明 |
|---|---|---|
| 38 | ユースケース仕様の前提が崩れた | 外部要因や発見により仕様の前提条件が無効化 |
| 39 | AIがエラーで停止した | トークン上限・API障害・タイムアウト等のオペレーション的障害 |
...
| 9 | 仕様変更を検出する | 仕様変更が検出された / 検出されなかった |
...
**含むイベント**: 生成された、仕様変更が検出された、仕様変更が検出されなかった、承認された、差し戻された、生成に失敗した
