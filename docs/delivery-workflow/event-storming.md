...
| レビュー中 | 人間がアーキテクチャ・振る舞い変更・タスク粒度を確認している | 生成済み | 承認済み, 生成中 |
...
| # | ホットスポット | 関連する集約/イベント | 解消アクション |
|---|---|---|---|
| 10 | 「振る舞い変更」の定義が例示のみで基準がない | 技術設計 | **解消済み（#96）**: 仕様変更・技術設計変更の2層に分離して定義 |
| 11 | 振る舞い変更検出ポリシーの検出対象（システム間連携IF等）の網羅基準がない | 技術設計 | **解消済み（#96）**: カテゴリ列挙は行わず、原則ベースで定義 |
