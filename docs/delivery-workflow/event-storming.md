...
| 13 | 技術設計が承認された | 人間がアーキテクチャ・振る舞い変更・タスク粒度を確認しゲート通過 |
...
| # | イベント名（過去形） | 説明 |
|---|---|---|
| 16 | Planが妥当と確認された | 実装開始前にPlanの妥当性を検証し、問題なしと判断 |
...
| 17 | ポリシー#1,#2,#6が参照するコマンド（「変更を検出する」「振る舞い変更を検出する」）とイベント（「変更が検出された」）がドメインイベント/コマンド一覧に未定義 | ポリシー | 関連Issue（#95,#96）の対応時にコマンド/イベント定義を追加 |
