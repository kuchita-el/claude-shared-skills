...
| # | イベント名（過去形） | 説明 |
|---|---|---|
| 13 | 技術設計が生成された | 技術設計＋タスク分解を出力 |
| 14 | 技術設計が承認された | 人間がアーキテクチャ・振る舞い変更・タスク粒度を確認しゲート通過 |
| 15 | 技術設計が差し戻された | アーキテクチャ不整合、タスク粒度の問題等で再設計を要求 |
| 16 | 技術設計の生成に失敗した | トークン枯渇・エラー等でAIが完了できなかった |
...
| # | コマンド名 | トリガーするイベント |
|---|---|---|
| 10 | Planを検証する | Planが妥当と確認された / Planの不備が発覚した / ドメイン構造の誤りが発覚した |
...
| 6 | 振る舞い変更検出ポリシー | 技術設計が生成された時 | 振る舞い変更を検出する |
...
**責務**: 技術設計・タスク分解・振る舞い変更の管理

**含むコマンド**: 技術設計を生成する、レビューする、Planを検証する

**含むイベント**: 生成された、承認された、差し戻された、生成に失敗した、Planが妥当と確認された、不備が発覚した
...
| 生成済み | 技術設計が出力され、振る舞い変更検出ポリシーの実行を待っている | 生成中 | レビュー中 |
| 失敗 | トークン枯渇・エラー等で生成が完了しなかった | 生成中 | 生成中, 破棄 |
| レビュー中 | 人間がアーキテクチャ・振る舞い変更・タスク粒度を確認している | 生成済み, 検証中 | 承認済み, 生成中 |
| 承認済み | 人間が技術設計を承認した | レビュー中 | 検証中 |
| 検証中 | 実装開始前にPlanの妥当性を検証している | 承認済み | 実装可能, レビュー中 |
| 実装可能 | Plan検証を通過し、実装に着手できる | 検証中 | 完了 |
| 完了 | 全タスクの実装が完了した | 実装可能 | 終了 |
| 破棄 | 生成失敗からのリカバリを断念した | 失敗 | 終了 |

### 実装

**責務**: TDDサイクルによるコード生成の管理
...
stateDiagram-v2
    ...
    検証待ち --> 実装中: 検証ゲートが失敗した
    セルフレビュー中 --> 完了: セルフレビューが通った
    セルフレビュー中 --> 実装中: 指摘が見つかった
    失敗 --> 実装中: リトライする
    失敗 --> 破棄: エスカレーションする
    完了 --> [*]
    破棄 --> [*]
...
| 実装中 | プロダクションコードを作成している | テスト作成中, 検証待ち, セルフレビュー中, 失敗 | 検証待ち, 失敗 |
| 検証待ち | 型チェック＋lint＋テストの自動検証を待っている | 実装中 | セルフレビュー中, 実装中 |
| セルフレビュー中 | 別コンテキストのAIがレビューしている | 検証待ち | 完了, 実装中 |
| 失敗 | 実装方針が行き詰まり、リトライかエスカレーションの判断が必要 | 実装中 | 実装中, 破棄 |
| 完了 | セルフレビューを通過し、PR作成に進める | セルフレビュー中 | 終了 |
| 破棄 | 実装を断念し、Planの見直し等にエスカレーション | 失敗 | 終了 |

### PRレビュー

**責務**: 検証・レビュー・マージのゲート管理
...
| 2 | 実装に失敗した後の判断基準がない（リトライ？タスク再分解？Plan差し戻し？） | 実装 | 失敗の種類別にエスカレーション先を定義 |
| 3 | PRが差し戻された後のリカバリフローが未定義（修正コミット？dev-loopやり直し？） | PRレビュー | 差し戻しの深刻度別にリカバリパスを定義 |
| 4 | 検証ゲート失敗時の対応が暗黙的（dev-loop内で自動修正？人間に戻す？） | PRレビュー | dev-loop内の自動修正範囲と人間エスカレーションの境界を定義 |

### フィードバックループ（#94関連）

| # | ホットスポット | 関連する集約/イベント | 解消アクション |
|---|---|---|---|
| 5 | 実装中にドメイン構造の誤りが発覚した場合、即時中断かSprint末かの判断基準がない | ドメイン構造 / 実装 | 影響範囲（集約境界変更 vs 属性追加等）で判断基準を定義 |
| 6 | レビューでドメイン構造の問題が指摘された場合の差し戻し先が不明（Discovery？Plan？） | ドメイン構造 / PRレビュー | 問題の深刻度別にエスカレーション先を定義 |
| 7 | Planの不備が発覚した場合のフローが未定義（実装中断→Plan修正→再開？） | 技術設計 / 実装 | Plan検証のタイミングと不備発覚時のフローを定義 |
...
| # | ホットスポット | 関連する集約/イベント | 解消アクション |
|---|---|---|---|
| 10 | 「振る舞い変更」の定義が例示のみで基準がない | 技術設計 | 外部観測可能な変化の定義と、パターンBとの境界を明確化 |
| 11 | 振る舞い変更検出ポリシーの検出対象（システム間連携IF等）の網羅基準がない | 技術設計 | 検出対象のカテゴリを列挙し、チェックリスト化 |
...
| 振る舞い変更 | 外部から観測可能な出力・状態遷移・エラー応答が変わる変更 | Planレビューの重点化に使用。システム間連携IFの変更を含む |
