...
**検証状況**: 以下は SKILL.md の仕様読解に基づく推論だったが、2026-06-21 の実機検証（[PR #324 降格側コメント](https://github.com/kuchita-el/claude-shared-skills/pull/324#issuecomment-4761045107)・[昇格側コメント](https://github.com/kuchita-el/claude-shared-skills/pull/324#issuecomment-4761076582)）で**降格側・昇格側の両ペアとも確認済**。実機検証は完結。今後挙動が変わった場合は「巻き戻し条件」節の手順に従って `model` 値を `inherit` に戻す。

文書ベース推論で得た以下を前提として採用する。

1. **明示指定したモデルは親モデル切替から独立して固定動作する（両ペアとも実機確認済）**:
   - 降格側（Opus 親 → Sonnet 子）: 親 Opus 4.7 のまま `code-reviewer` (`model: sonnet`) が Sonnet 4.6 で起動。
   - 昇格側（Sonnet 親 → Opus 子）: 親 Sonnet 4.6 のまま `plan` (`model: opus`) が Opus 4.8 で起動。エイリアス `opus` は現状最新版（4.8）に動的解決される。
2. ユーザーが明示指定値を上書きする公式機構は存在しない（または未文書化）。実機検証でも上書き機構の発動は観測されていない。
