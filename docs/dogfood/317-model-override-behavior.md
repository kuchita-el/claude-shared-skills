...
**検証状況**: 以下は SKILL.md の仕様読解に基づく推論で、本 Issue 初回反映時点では未検証だった。2026-06-21 の実機検証（PR #324 コメント）で**降格側ペア（Opus 親 → Sonnet 子）は確認済**。昇格側ペア（Sonnet 親 → Opus 子）は親セッション `/model` 切替操作が必要なため未実施で、後続ドッグフードで確定する。実機検証で異なる挙動が観察された場合は「巻き戻し条件」節の手順に従って `model` 値を `inherit` に戻す。

文書ベース推論で得た以下を前提として採用する。

1. `model: sonnet` を明示指定したエージェントは、メインセッションのモデル切り替えに連動せず Sonnet で固定動作する。**降格側（Opus 親 → Sonnet 子）は 2026-06-21 の実機検証で確認済**（[PR #324 コメント](https://github.com/kuchita-el/claude-shared-skills/pull/324#issuecomment-4761045107)。親 Opus 4.7 のまま子 code-reviewer が Sonnet 4.6 で起動）。**昇格側（Sonnet 親 → Opus 子）は後続検証**。
