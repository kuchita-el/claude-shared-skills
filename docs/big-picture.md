...
Big Picture の粒度原則（コンテキスト境界を跨ぐ・重要な状態遷移点・業務フロー間の合流分岐点）に従い27個を抽出した。単一コンテキスト内部の細かい工程は Design Level で掘る。
...
| 4 | 開発 | 検証 | PRがマージされた | QA環境にデプロイされた | 順方向（主フロー） |
| 5 | 開発 | リリース | PRがマージされた | ビルド成果物が公開された | 順方向（主フロー） |
| 6 | 検証 | リリース | テストサイクル完了（内部） | リリース判断（内部） | 順方向 |
...
- **系1（プロダクト変更フロー）**: 経営・ユーザー要望 → 企画 → バックログ管理 → 開発 → 検証 → リリース（ここで終端）
