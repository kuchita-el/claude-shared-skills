...
| 12 | 用語の多義性: 「デプロイ」は QA環境と本番で意味合いが異なる | 検証 / リリース | **検証側は整理済み** → `docs/qa/event-storming.md`（QA環境デプロイは外部トリガーとして定義）。本番デプロイはリリース側コンテキストで整理（#15が先決） |
...
| 15 | 検証・リリースのコンテキスト分割が必要 | 検証 / リリース | Design Levelで検証側の責務（テスト設計・実行・不具合起票）とリリース側の責務（デプロイ・稼働確認・ロールバック）が明確に異なることが判明。リリース側の所属（開発に統合 or 独立コンテキスト）を決定しBig Pictureを更新する |
