...
| 12 | ビルド成果物が公開された | ビルド成果物（バージョン付き）が生成・公開された | リリース |
...
- **機能廃止の二段階化**: 決定（企画）のみ独立イベント化し、実行は既存のユーザーストーリー実行フロー（開発→検証→リリース）を流用する
...
- **QA環境デプロイはビルド成果物公開を起点とする**: QA環境への投入（自動/手動いずれも）はビルド成果物公開の下流として扱う。デプロイというメカニズム自体の所属は揃えず、起点となるドメインイベントのみコンテキストに配置する
...
- **リリース内部**: Design Level 未実施（新設）。リリース判断プロセス、ビルド成果物のライフサイクル、本番デプロイ指示、ロールバック実行の詳細を扱う
...
| 4 | リリース | リリース判断と本番デプロイ指示・ビルド成果物管理。本番環境への通常変更を所有 | ビルド成果物が公開された、本番にデプロイされた、リリースされた、ロールバックされた |
...
| 4 | リリース | 検証 | ビルド成果物が公開された | QA環境にデプロイされた | 順方向（主フロー） |
| 5 | 開発 | リリース | PRがマージされた | ビルド成果物が公開された | 順方向（主フロー） |
| 6 | 検証 | リリース | テストサイクルが完了した | 本番にデプロイされた | 順方向（主フロー） |
| 7 | 検証 | バックログ管理 | QAで不具合が発見された | バグチケットが作成された | 順方向 |
| 8 | 観測 | バックログ管理 | バグ報告が届いた | バグチケットが作成された | 順方向（主フロー） |
| 9 | 観測 | バックログ管理 | 脆弱性/EOL/依存更新の検出 | タスクチケットが作成された | 順方向（主フロー） |
| 10 | 観測 | 運用 | アラートが発報された | （インシデント対応開始） | 順方向（主フロー） |
| 11 | 運用 | リリース | ロールバック判断がされた | ロールバックされた | 順方向 |
| 12 | 運用 | バックログ管理 | インシデント対応中の発見・事後記録 | バグチケット/タスクチケットが作成された | 順方向 |
| 13 | 検証 | 企画 | QAで不具合が発見された（仕様起因） | PRDが修正された | 逆方向（PRD修正） |
| 14 | 検証 | 開発 | QAで不具合が発見された（実装起因） | 修正が差し戻された | 逆方向（差し戻し） |
| 15 | 開発 | 企画 | 仕様の不明点がエスカレーションされた | （企画側で回答が用意される） | 逆方向（エスカレーション） |

### 構造の特徴

**主フローは2つの独立した系に分かれる:**

- **系1（プロダクト変更フロー）**: 経営・ユーザー要望 → 企画 → バックログ管理 → 開発 → リリース（ビルド成果物公開） → 検証（QAデプロイ・テスト） → リリース（本番デプロイ・リリース）
...
| 9 | 「稼働状態」（継続的）と「ビルド成果物」（バージョン単位）の視点の違いが整理されていない | リリース / 観測 | Design Level で「ビルド成果物」集約のライフサイクル（生成→デプロイ→稼働→廃止）を整理する。リリースコンテキストのDesign Levelで扱う |
...
| リリース | 未実施（新設） | リリース判断プロセス、ビルド成果物のライフサイクル、本番デプロイ指示、ロールバック実行 |
