...
| 6 | 実装プランが確定した | プランニングが完了し、開発者が実装に入れると判断した | 開発 |
| 7 | 判断依頼が発行された | プランニング中に発生した判断必要事項を上流/外部に問い合わせる | 開発 |
...
- **メインボードのチケットライフサイクルはPRマージ時点で完了する**: バックログ管理コンテキストが管理するメインボードのチケット終端はPRマージとする。QA活動の進捗・結果はメインボードのチケットステータスで表現せず、検証コンテキスト集約の状態として独立追跡する。QAで発見された不具合は新規バグチケットとしてバックログ管理に起票される（連携 #6 参照）。チケットステータス遷移は開発・検証・観測等のドメインイベントをバックログ管理コンテキストが購読して更新する（コンシューマー型）。本方針はメインボードに限定する。インシデントボード・サポートボード等の追加に伴うバックログ管理コンテキストの境界再検討は別Issueで扱う
...
| 2 | 開発 | チケットから実装プランを立て、コードとテストを書き、PRマージを経てビルド成果物公開まで持っていく。ビルド成果物の公開・保管を所有 | 実装プランが確定した、判断依頼が発行された、修正が差し戻された、PRがマージされた、ビルド成果物が公開された |
...
| 14 | 開発 | 企画 | 判断依頼が発行された | （企画側で回答が用意される） | 逆方向（判断依頼） |
| 15 | 開発 | バックログ管理 | 開発主要イベント（プランニングが開始された・判断依頼が発行された・全判断依頼項目が解消された・レビュー依頼がされた・必要Approve数に達した・PRがマージされた等） | （メインボードのチケットステータス更新。一部遷移は手動操作前提：例「待ち→進行中」は運用ルール「プランニング再開」に従い開発者が手動実行する） | 順方向（コンシューマー型。詳細はバックログ管理DLで設計） |
...
- トリアージ出力: バックログ管理 → 開発
- ステータス更新（コンシューマー型）: 開発 → バックログ管理（メインボードのチケットステータス遷移はバックログ管理が開発主要イベントを購読して更新する。詳細はバックログ管理DLで設計）
...
| 2 | リリース後にPRDが更新されないため情報が陳腐化している。鮮度維持ポリシーがない | 企画 | PRDの鮮度維持責任・更新タイミングを定義する、または「PRDは開発時点の意思決定記録」と割り切るかの判断。**staleness（陳腐化）検知**の論点: (1) 鮮度の自動失効と定期棚卸し（更新履歴・参照頻度を入力に陳腐化を検知）、(2) 陳腐化フラグの付与とアーカイブ（AI 検知→人/AI 廃棄、L2/L3）。検知の具体方式は別途設計（#227 後続「鮮度検知の仕組み」へ委譲）。背景は「BizDevOps サイクルとナレッジ循環」節を参照 |
...
| 5 | KPI計測から企画へのフィードバックループが未整備 | 観測 / 企画 | KPI計測イベントを定義し、企画への定期フィードバック経路を作る。**Feedback→Biz 循環**: テレメトリ・利用行動分析／A/B 評価／事業 KPI 評価で得た学習を「フィードバック統合→次の仮説」に集約し、ナレッジ基盤を経て Biz の価値仮説へ還流させる（入力＝KPI・利用データ、出力＝企画への定期フィードバックと新たな価値仮説）。整理は「BizDevOps サイクルとナレッジ循環」節を参照 |
...
## BizDevOps サイクルとナレッジ循環

「[コンテキスト一覧](#コンテキスト一覧)」のコンテキスト群は、BizDevOps サイクルの4フェーズ Biz（事業・企画）→ Dev（開発・検証・リリース）→ Ops（運用・観測）→ Feedback（学習・評価）に対応づけられる（バックログ管理は全フェーズを横断する調整層）。Feedback フェーズの計測起点は観測コンテキストが担う（KPI・利用率計測。「[観測コンテキストの拡張余地](#観測コンテキストの拡張余地)」参照）。

サイクルが成立するのは、Feedback で得た学習が**中心のナレッジ基盤**（エージェントの共有メモリ。概念定義は principles.md「ナレッジ＝共有メモリ前提」節）を経て Biz の価値仮説へ還流するため（Feedback→Biz 循環、ホットスポット #5）。この循環は知識の鮮度維持を前提とする（鮮度が腐れば還流する知識が劣化する、ホットスポット #2）。

### Feedback→Biz 循環（営み × 主担当(自律度) × Input × Output）

| 営み | 主担当（自律度） | インプット | アウトプット |
|---|---|---|---|
| テレメトリ・利用行動分析 | AI（L3） | イベントログ、利用データ | 利用インサイト、ファネル分析 |
| A/B・実験評価 | AI→人承認（L2） | 実験データ、仮説 | 統計的評価、勝者判定（採用/打ち切りは人） |
| 事業KPI評価 | 人＋AI（L1） | KPI、財務、市場反応 | 評価レポート |
| フィードバック統合→次の仮説 | 人＋AI（L1） | 上記すべて | 学習、新たな価値仮説（Biz へ循環） |

KPI・利用データを入力に得た学習が「フィードバック統合→次の仮説」へ集約され、ナレッジ基盤を経て Biz の価値仮説として還流する。これが企画への定期フィードバック経路の実体（ホットスポット #5 の解消方向）。

### フェーズ × ナレッジの循環（預け入れ／引き出し）

各フェーズは中心のナレッジ基盤に対し預け入れ／引き出しを行う。

| フェーズ | 預け入れる知識 | 引き出す知識 |
|---|---|---|
| Biz | 価値仮説、投資判断の根拠、市場洞察 | 過去の仮説と結果、顧客知識 |
| Dev | 設計判断（ADR）、コード、テスト、規約 | ドメインルール、既存アーキ、過去の失敗 |
