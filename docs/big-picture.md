...
| 7 | 「修正が差し戻された」の起点（レビュー/QA/動作確認）ごとの条件・粒度が曖昧 | 開発 / 検証・リリース | **開発側は解消済み** → `docs/development/event-storming.md`（レビュー判定軸で「変更が要求された」と「レビューが承認された」に分離。プラン見直し要否は開発者判断）。検証・リリース側は未着手 |
