...
| 7 | 「修正が差し戻された」の起点（レビュー/QA/動作確認）ごとの条件・粒度が曖昧 | 開発 / 検証・リリース | **開発側は解消済み** → `docs/development/event-storming.md`（レビュー判定軸で「変更が要求された」と「レビューが承認された」に分離。変更要求時は常に新しい実装プランを作成）。検証・リリース側は未着手 |
...
| 13 | 用語の多義性: 「テスト」は自動テスト（TDD）と探索的テスト（手動）で性質が違う | 開発 / 検証・リリース | **開発側は解消済み** → `docs/development/event-storming.md`（TDDのテスト作成＝設計行為、検証ゲート＝品質ゲートとして分離定義）。検証・リリース側は未着手 |
