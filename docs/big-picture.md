...
7 コンテキストは Biz（事業・企画）→ Dev（開発）→ Ops（運用）→ Feedback（学習・評価）の BizDevOps サイクルとして循環する。サイクルが成立するのは、Feedback で得た学習が中心のナレッジ基盤を経て Biz の価値仮説へ還流するため（Feedback→Biz 循環、ホットスポット #5）。この循環は知識の鮮度維持を前提とする（鮮度が腐れば還流する知識が劣化する、ホットスポット #2）。
