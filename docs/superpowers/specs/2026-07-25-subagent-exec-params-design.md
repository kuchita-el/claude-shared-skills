...
> **スキーマに関する注記（2026-08-07 追記）**: 本書は 2026-07-25 時点の設計記録であり、当時の ADR スキーマを前提に「意図的に決めなかった facet は `## 保留した決定` 節へ置く」という規則（当時の出典は ADR-202607201441-01）を適用している。同節はその後 ADR-202608070711-01 により廃止された。現行の規約では、射程の限定は専用の節を設けず、ADR 全体に掛かるものは `## Context` へ、特定の決定に掛かるものは当該決定の本文の注記として散文で書く。決定7（`context: fork` を範囲外とする）の記録も、実際には ADR-202607251922-02 の `## Context` へ置かれている。**本書を入力に後続工程へ進む際、`## 保留した決定` 節を新たに書かないこと。**

> **参照先に関する注記（2026-08-29 追記）**: 本書は 2026-07-25 時点の参照先を記録として据え置いている。本書が名指す `plugins/dev-workflow/references/subagent-execution-parameters.md` は `docs/references/subagent-execution-parameters.md` へ搬出され、`output-format-batch-subagent.md` は #804 で `output-format-batch.md`「入口: サブエージェント返却形式」節へ統合されたため、いずれも現存しない。本書の記述を現況の参照先として使わないこと。
...
SKILL.md も `model` / `effort` / `context: fork` / `agent` を受け付ける。ただし指定が実効値を動かすかは起動経路で分かれる。2026-08-06 時点の実測では、対話経路で `model` は実効モデルを変えず、ヘッドレス（`-p`）経路でのみ宣言値へ切り替わって以降のターンも持続した。`effort` は 2026-08-05 時点の実測（ヘッドレス経路）で、効果がそのターンに限られ次のプロンプトでセッション値へ戻った。`context: fork` はスキル本文をサブエージェントの指示として実行するもので、会話履歴を参照できず、既定では背景実行となり、利用できるツールも縮小される。
...
これは採らなかった選択肢であり、後で決着させる必要のある未決事項ではない。当時の「保留した決定」節には記録せず、却下した選択肢として扱った。

### 決定6: 実行パラメータの統制点をサブエージェント側に集約する

SKILL.md の frontmatter には `model` と `effort` を置かない。理由は 3 つある。

1. **効き方が経路と対象で食い違う。** `create-issue` / `domain-modeling` / `event-storming` / `plan-issue` / `manage-adr` / `intake` / `implementation` は対話や承認の待ち合わせで複数のターンにまたがる。対話経路では `model` が実効モデルを変えず、統制として成立しない。ヘッドレス経路では `effort` がこれらで最初のターンにしか効かず一貫した統制にならない一方、`model` は効いた上で持続し、スキルを1度起動するとセッションのモデルが書き換わったまま残る。効かないことと、意図しない範囲まで効き続けることのいずれも、統制点として成立しない。
2. **削減の対象は軽量な処理に限られる一方、影響は以降のターン全体へ及ぶ。** サブエージェント側を明示的に固定したことにより、スキルの `model` 指定はサブエージェントの実行には届かない（スキルの指定は最後尾の「親セッション」の位置に入るにとどまり、定義が値を明示している限り参照されない）。直接の対象はバッチ分割・結果集約・整形といった処理であり、ここを下げても削減幅は小さい。にもかかわらず、`model` が効くヘッドレス経路では持続するため、効果は当該スキルの処理にとどまらず以降のターン全体へ波及する（対話経路ではそもそも下げる手段にならない）。得られる削減に対して波及範囲が釣り合わない。
...
これは S3 または独立した課題として後で決着させる未決事項であるため、当時の規約に従い ADR の「保留した決定」節へ記録した（同節は廃止済み。現在の置き場所は冒頭の注記を参照）。
...
また意図的に決めなかった facet は独立した ADR にせず、当時の規約（ADR-202607201441-01）に従い `## 保留した決定` 節に置いた。ただしここに記録するのは後で決着させる必要がある未決事項に限り、単に採らなかった選択肢は却下として扱った。同節はその後 ADR-202608070711-01 により廃止されている（冒頭の注記を参照）。
...
| `context: fork` を範囲外とする（決定7） | 決定ではなく意図的な非決定 | — | 当時の `## 保留した決定` 節へ（同節は廃止済み） |
...
- Context: 効き方が経路と対象で食い違い、対話経路では `model` が実効モデルを変えず、ヘッドレス経路では `effort` が対話を伴うスキルで最初のターンにしか効かない一方 `model` はセッションのモデルを書き換えたまま持続する。削減の直接対象はメイン側の軽量な処理に限られる一方、`model` の影響は以降のターン全体へ波及する。統制点の分散は追跡コストを生む
- 保留した決定（当時の節。廃止後は `## Context` の射程の限定へ移設済み）: `context: fork` の導入（想定継承先: S3 または独立した課題）
