...
本リポジトリは既に「ADR は判断の記録、`references/*.md` は規約の単一出典」という分離を持つ。`context-budget.md` が ADR-20260627 の正本として機能しているのがその例である。今回の決定も同じ構造に合わせる。
...
判定には 2 つの軸を用いる。**ADR 化の要否**は粒度判定基準の 4 項目で判定する。**1 本にまとめるか分けるか**は ADR-20260711-3 決定3 の「独立に反転しうる core を束ねない」で判定する。両者は別の基準であり、混同しない。

また意図的に決めなかった facet は独立した ADR にせず、`## 保留した決定` 節に置く（ADR-20260720-4）。ただしここに記録するのは後で決着させる必要がある未決事項に限る。単に採らなかった選択肢は却下として扱う。
