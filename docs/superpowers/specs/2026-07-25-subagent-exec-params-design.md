...
SKILL.md も `model` / `effort` / `context: fork` / `agent` を受け付ける。ただし持続範囲は `model` と `effort` で異なる。2026-08-05 時点の実測（ヘッドレス経路、スキル起動ターンとその次のターンまで観測）では、`effort` の効果はそのターンに限られ次のプロンプトでセッション値へ戻る一方、`model` は次のプロンプト以降も持続し、スキルを1度起動するとセッションのモデルが書き換わったまま残った。`context: fork` はスキル本文をサブエージェントの指示として実行するもので、会話履歴を参照できず、既定では背景実行となり、利用できるツールも縮小される。
...
1. **持続範囲が `model` と `effort` で食い違う。** `create-issue` / `domain-modeling` / `event-storming` / `plan-issue` / `manage-adr` / `intake` / `implementation` は対話や承認の待ち合わせで複数のターンにまたがる。`effort` はこれらで最初のターンにしか効かず、一貫した統制にならない。`model` は逆に次のプロンプト以降も持続し、スキルを1度起動するとセッションのモデルが書き換わったまま残る。効かないことと、意図しない範囲まで効き続けることのいずれも、統制点として成立しない。
2. **削減の対象は軽量な処理に限られる一方、影響は以降のターン全体へ及ぶ。** サブエージェント側を明示的に固定したことにより、スキルの `model` 指定はサブエージェントの実行には届かない（スキルの指定は最後尾の「親セッション」の位置に入るにとどまり、定義が値を明示している限り参照されない）。直接の対象はバッチ分割・結果集約・整形といった処理であり、ここを下げても削減幅は小さい。にもかかわらず `model` は持続するため、効果は当該スキルの処理にとどまらず以降のターン全体へ波及する。得られる削減に対して波及範囲が釣り合わない。
...
- Context: 持続範囲が両者で食い違い、`effort` は対話を伴うスキルでは最初のターンにしか効かず、`model` は逆にセッションのモデルを書き換えたまま持続する。削減の直接対象はメイン側の軽量な処理に限られる一方、`model` の影響は以降のターン全体へ波及する。統制点の分散は追跡コストを生む
