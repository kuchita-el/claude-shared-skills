...
SKILL.md も `model` / `effort` / `context: fork` / `agent` を受け付ける。ただし持続範囲は `model` と `effort` で異なる。`effort` の効果はそのターンに限られ、次のプロンプトでセッション値へ戻る。`model` は次のプロンプト以降も持続し、スキルを1度起動するとセッションのモデルが書き換わったまま残る。`context: fork` はスキル本文をサブエージェントの指示として実行するもので、会話履歴を参照できず、既定では背景実行となり、利用できるツールも縮小される。
...
- Context: 持続範囲が両者で食い違い、`effort` は対話を伴うスキルでは最初のターンにしか効かず、`model` は逆にセッションのモデルを書き換えたまま持続する。効果がメイン側の軽量な処理に限られる。統制点の分散は追跡コストを生む
