...
`plugins/` を正本兼配布物とする構成は本設計で新たに導入する判断ではなく、既存の配布構造を継続するものである。このため、新規ADRの対象にしない。採用理由と生成成果物を設けない理由は、本設計の設計案比較と採用アーキテクチャに保持する。

新規ADRの対象とするのは、`dev-workflow` をスキル単位に分けず、共有規約とagentsの凝集に沿って限定的に分割する判断だけである。この判断の粒度判定は次のとおりである。
...
依存凝集に沿った限定分割の判断は3点でADR化推奨に当たる。ただし `adr-scoping.md` の起票タイミングに従い、最初の出現である本PRでは本設計とPR説明に保持する。Wave 0の実装Issue着手時に `manage-adr` へ再入力し、この判断を一つの反転単位として新規ADRに起票する。
