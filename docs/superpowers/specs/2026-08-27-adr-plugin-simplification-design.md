...
- `docs/distribution-boundary.md`: `adr-scoping-cases.sh` を配置判断の**例示**として引用している箇所。決定そのものは無効化されないため、例示の陳腐化を注記で処理する。`docs/adr/ADR-202607230648-01` も同じ例示を持つが、ADR は時点の決定の記録であるため注記も置かず無編集とする（2026-08-27 のユーザー裁定2。陳腐化の注記は生きた規約文書の側のみへ置く）
...
- **既存ADRとの決着**: 判定仕様・固定題材集合・退行検出網に関わる既存ADRを洗い出し、各件の判定結果（変更不要を含む）を記録する。2026-08-27 の実測（`docs/adr/` 全件走査）では遷移（上書き/廃止）を要するADRは無い: `ADR-202607230648-01` は例示引用のみで無編集（「削除に伴う波及修正」参照）、`ADR-202608011651-01` 決定2 は閾値の内製化を反転と扱わないことを自ら明記している。本簡素化自体が設計判断であり、ADR化要否の判定を経る。
