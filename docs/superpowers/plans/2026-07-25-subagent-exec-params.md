...
> **スキーマに関する注記（2026-08-07 追記）**: 本書は 2026-07-25 時点の実装プランであり、当時の ADR スキーマを前提に `**保留した決定:**` を ADR の記入項目として指示している。同節は ADR-202608070711-01 により廃止された。現行の規約では、射程の限定は専用の節を設けず、ADR 全体に掛かるものは `## Context` へ、特定の決定に掛かるものは当該決定の本文の注記として散文で書く。**本書を入力に後続工程へ進む際、`## 保留した決定` 節を新たに書かないこと。**
...
スキル側に置かない理由は起動経路によって異なる。対話経路では `model` が実効値を動かさず、統制として成立しない。ヘッドレス（`-p`）経路では `effort` の効果がそのターンに限られ、対話や承認の待ち合わせで複数のターンにまたがるスキルでは最初のターンにしか効かない一方、`model` は効いた上で持続し、スキルを1度起動するとセッションのモデルが書き換わったまま残る。効かないことと、意図しない範囲まで効き続けることは、いずれも統制点をスキル側へ置かない理由になる。
...
- スキル側の指定が実効値を動かすかは起動経路で分かれる。2026-08-06 時点の実測では、対話経路で `model` は実効モデルを変えず、ヘッドレス（`-p`）経路でのみ宣言値へ切り替わって以降のターンも持続した。`effort` は 2026-08-05 時点の実測（ヘッドレス経路）で、効果がそのターンに限られ次のプロンプトでセッション値へ戻った
...
- 本決定の下では、スキルのメイン側処理（バッチ分割・結果集約・整形）は親セッションの設定で動く。スキル front-matter の `model` でこの部分を下げることは、対話経路では手段として成立せず、ヘッドレス経路では可能だが効果が以降のセッション全体へ波及する
