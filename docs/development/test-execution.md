...
### fixture の有効性の検査（変異検査）

`lint-ja.sh` の fixture については、負例が赤く正例が緑であることとは別に、**その fixture が意図した条件によってその色になっているか**を検査する。負例は別の検出項目がたまたま反応しているだけでも赤くなり、正例は免除や除外が効いていなくても、そもそも検出条件に触れていなければ緑になる。どちらも色だけでは有効性の証拠にならない。

実体は `scripts/tests/lint-ja-mutation.bats` であり、runner の `bats` スイートに含まれる。手順は次のとおり。

1. `lint-ja.sh` を一時ディレクトリへ複製し、条件を1つだけ無効化した変異体を作る。被テスト検査器そのものへ無効化の経路は作らない
2. 置換が原本を実際に書き換えたことを確かめる。空振りしていれば以降の判定は無意味になるため、その場合は失敗として報告する
3. 全ての fixture を変異体へ食わせ、**対応する fixture だけが転じ、他は元の色のまま残る**ことを確かめる

変異は3つの向きへ置く。検出殺しだけを置くと、免除と除外の分岐が無検査のまま残り、到達しない死に分岐や条文が認める形を通していない免除が、正例を緑にしたまま潜り込む。

**検出条件の無効化**（対応する負例が赤から緑へ転じる）

| 変異 | 転じる fixture |
|---|---|
| 一文の長さの判定を無効化する | 負例 01・02・03・04・05a・05b・05c・05d・05e・05f・06・07・08・09・10・11・12・13・14・15・16・17・18 |
| 見出しの判定を記号だけに戻す | 負例 04・04（候補） |
| 括弧の対応の数え上げを無効化する | 負例 03・05a・05b・05c・05d・05e・05f |
| 段落の行の連結を無効化する | 負例 02・07 / 正例 21 |
| 括弧の対から「」を落とす | 負例 05a |
| 括弧の対から『』を落とす | 負例 05b |
| 括弧の対から【】を落とす | 負例 05c |
| 括弧の対から［］を落とす | 負例 05d |
| 括弧の対から()を落とす | 負例 05e |
| 括弧の対から[]を落とす | 負例 05f |
| 半角の疑問符と感嘆符を区切りに加える | 負例 06 |
| インラインコードの内側からも記法の記号を落とす | 負例 08 |
| 閉じない囲みでも範囲を作る | 負例 09 |
| 箇条書きの項目も下線形式の見出しの対象にする | 負例 12 |
| 閉じない区切り線でも範囲を作る | 負例 10 |
| 注釈の終了を伏せた写しで探す | 負例 13 / 正例 24 |
| コードスパンの囲みの長さの一致を1字に固定する | 負例 18 |

**免除と除外の無効化**（対応する正例が緑から赤へ転じる）

| 変異 | 転じる fixture |
|---|---|
| リンク記法の除去を無効化する | 正例 05 |
| 参照形式リンクの除去を無効化する | 正例 15 |
| 文の区切りから疑問符と感嘆符を落とす | 正例 08 |
| 文の区切りから疑問符だけを落とす | 正例 08 |
| 文の区切りから感嘆符だけを落とす | 正例 08 |
| インラインコードの伏せ字を無効化する | 負例 18 / 正例 09 |
| 半角4字の字下げの除外を無効化する | 負例 17 / 正例 07 |
| タブ1字の字下げの除外を無効化する | 正例 17 |
| チルダのフェンスの認識を落とす | 正例 06 |
| バッククォートのフェンスの認識を落とす | 正例 02 |
| 囲みを別の種類の記号で閉じられるようにする | 正例 06・18 |
| 注釈の除外を無効化する | 正例 19・24 |
| 縦棒で始まる表の行の除外を無効化する | 負例 14 / 正例 11 |
| 縦棒で始まらない表の除外を無効化する | 正例 22 |
| 引用の行の除外を無効化する | 負例 15 / 正例 11 |
| 見出しの行の除外を無効化する | 負例 16 / 正例 23 |
| 下線形式の見出しの除外を無効化する | 正例 21・25 |
| 冒頭のメタデータの除外を無効化する | 正例 12 |
| 箇条書きのマーカー除去を無効化する | 負例 12 / 正例 13・20 |
| 番号付き項目の丸括弧の形を落とす | 正例 20 |
| 強調の記号の除去を無効化する | 正例 14・16 |
| 二つ続いたチルダの除去だけを落とす | 正例 16 |
| 箇条書きの直後の区切り線を項目へ折り込む | 正例 25 |

**候補の検出条件の無効化**（対応する候補 fixture が候補を出さなくなる）

| 変異 | 転じる fixture |
|---|---|
| 不透明な識別子の判定を無効化する | 負例 04（候補） / 候補 01・02・03・04・05 |
| 識別子の形から課題番号を落とす | 負例 04（候補） / 候補 03 |
| 識別子の判定でインラインコードの囲みを外さない | 候補 02 |
| 骨格位置の助詞から「が」を落とす | 負例 04（候補） / 候補 04 |
| 骨格位置の助詞から「を」を落とす | 候補 05 |

各変異は3つの群すべてへ、2つの軸（終了コードと候補の有無）すべてで当てる。1つの群・1つの軸にしか当てないと、変異が別の群や別の軸まで巻き込んでいることを検出できない。上表で群を跨いで転じる行があるのは、その巻き込みが実装の性質として説明できるものを明示的に宣言しているためである。既定でない軸で転じる場合は「04（候補）」のように軸を添える。

変異前の値は宣言せず、原本を走らせて測る。宣言で持つと、宣言と実測がずれても気づけない。

上表そのものの縮退も検査する。変異表から行を削っても判定対象が減るだけで全緑のまま通り、上表だけが古くなっても誰も落ちない。`lint-ja-mutation.bats` が上表と変異表を説明と期待の両方で突き合わせる。

候補（第2条）は終了コードへ寄与しないため、この群だけは終了コードではなく候補の出力の有無で判定する。終了コードで判定すると、候補の検出を丸ごと落としても全件緑のまま通る。

### 独立レビューの fixture の台帳の検査

`doc-reviewer` の判定は決定的でない。そのため fixture を実際に読ませる確認は runner の対象に含めていない（理由は `scripts/fixtures/doc-reviewer/README.md`）。その結果、fixture を足しても対応表の更新を忘れると、その欠陥の型は永久に確認されないまま残る。確認そのものは機械化できないが、確認の台帳が対象を覆っているかは機械で検査できる。

実体は `scripts/tests/doc-reviewer-fixtures.bats` であり、対応表と実ファイルの一致を双方向に見る。台帳に載っていない fixture と、台帳が指す実在しないファイルの両方を失敗として報告する。
...
- **先頭の `1..N` が報告ケース総数**である（`N` はスイートの登録数で決まる）。この値は失敗の有無・件数によらず一定である。テストファイルの前提（fixture corpus や被テスト検査器）が満たされない場合も、そのファイルのケース群が1件へ潰れることはない。専用の「前提: …」ケースが前提不成立を名指しして落ち、その前提に依存する面のケースも1件ずつ個別に落ちる。前提不成立の失敗が1件で済むという意味ではない。潰れないことと総数が動かないことが不変条件である。
