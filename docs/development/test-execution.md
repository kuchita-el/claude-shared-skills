...
| 変異 | 転じる fixture |
|---|---|
| 不透明な識別子の判定を無効化する | 候補 01・02・03・04・05 |
| 識別子の形から課題番号を落とす | 候補 03 |
| 識別子の判定でインラインコードの囲みを外さない | 候補 02 |
| 骨格位置の助詞から「が」を落とす | 候補 04 |
| 骨格位置の助詞から「を」を落とす | 候補 05 |

各変異は3つの群すべてへ当てる。1つの群にしか当てないと、変異が別の群まで巻き込んでいることを検出できない。上表で群を跨いで転じる行があるのは、その巻き込みが実装の性質として説明できるものを明示的に宣言しているためである。
