...
| スイート | 実体 | 内容 |
|---|---|---|
| `bats` | `scripts/tests/*.bats` | adr プラグイン同梱の検査器4本（`lint-adr.sh` / `gen-adr-index.sh` / `next-adr-id.sh` / `adr-scoping-cases.sh`）、writing プラグイン同梱の `lint-ja.sh`、および配布物外の `scripts/lint-domain-doc.sh` のテスト |
...
`lint-ja.sh` の負例 fixture については、負例が赤いこととは別に、**その負例が意図した検出条件によって赤くなっているか**を検査する。負例は別の検出項目がたまたま反応しているだけでも赤くなるため、赤いことは有効性の証拠にならない。検査語彙を狭めたときに、その語彙へ依存していた負例が黙って無力化する事故を捕まえるために置いている。

実体は `scripts/tests/lint-ja-mutation.bats` であり、runner の `bats` スイートに含まれる。手順は次のとおり。

1. `lint-ja.sh` を一時ディレクトリへ複製し、検出条件を1つだけ無効化した変異体を作る。被テスト検査器そのものへ無効化の経路は作らない
2. 置換が原本を実際に書き換えたことを確かめる。空振りしていれば以降の判定は無意味になるため、その場合は失敗として報告する
3. 全ての負例を変異体へ食わせ、**対応する負例だけが緑へ転じ、他の負例は赤のまま残る**ことを確かめる

変異と、緑へ転じることを期待する負例の対応は次のとおり。

| 変異 | 緑へ転じる負例 |
|---|---|
| 一文の長さの判定を無効化する | 01・06・07 |
| 未接地語の語の切り出しを無効化する | 02・03 |
| 不透明な識別子の判定を無効化する | 04・05 |
| 未接地語の対象からカタカナを落とす | 03 |
| 未接地語の対象から英語を落とす | 02 |
| 識別子の判定でインラインコードの囲みを外さない | 05 |

最後の1件は、インラインコードで囲まれた識別子を第2条の検査が拾うという要件（囲みを外してから説明句を探す）が、実際に負例の赤さを支えていることを確かめる。

**結果（2026-08-08）**: 6つの変異について、変異が適用されたことの確認6件と、変異体×負例の42件を判定し、48件すべてが期待どおりだった。
...
76 の内訳は **面 67 ＋ 前提不成立 9** である。旧ラベルを持たない新規ケースは次の **12件** に限る。
