...
- 版: 0.8（例1件の呼称を EX へ改め、統合例へ多重操作の `順次` を追加。変更点は末尾「改訂履歴」）
...
- **軸を同値類で宣言したとき、類の分け方の誤りは検知しない**（§6.10）。同じ類に入れた別の値が違う振る舞いをしても、代表値の EX が緑である限り機械は何も言わない。軸の宣言漏れが検知できないのと同じ理由であり、残余は規約（§10）と PdM の承認へ回る。
- **手書きの `前提` が到達不能な値の組を持つことは検知しない**（§6.9）。判別共用体が縛るのはフィールドの共存までで、値の組み合わせの到達可能性は縛らない。統合例は `順次` で中間状態を実装に書かせられるが、ドメインの例テーブルは `前提` を手書きするため、本番では生じない組（支払額と無関係な返金額を持つキャンセル済み注文）に対する期待を承認してしまう経路が残る。押さえるのは INV であって型ではない。
...
- 0.8（2026-09-13）: 設計レビュー指摘の #4（例テーブルと受入条件の関係）を反映。例1件の呼称を `AC-xxx`（受入条件）から `EX-xxx`（受入例）へ改めた。1件は要求に対する外延の点であって条件ではなく、点と点の間を埋める内包は INV が担う（0.7 の決着）。「条件」の含意が「AC はユースケース記述か」という誤読の源だったため、実体に合う語へ寄せ、実装が始まって ID がテスト名・manifest・PR 本文へ入る前に済ませた（§3, §5, §6, §7, §8, §13, §14）。統合例へ多重操作の `順次` を追加した（§6.9）。ドメイン層は `前提` をドメイン型で書くため判別共用体がフィールドの共存を強制するが、統合例の `前提` は DB 表現でフラットであり共存すら縛らない。本番では生じない行を前提に置いた例が緑のまま実装の真の振る舞いを見ない経路になるため、`順次` に中間の行を実装へ書かせ、逐次の再入を同時の競合と対で置けるようにした。型は操作の形ごとに枝を分けて照合子を決める形とし、`期待` の第1引数を照合済みの結果の形へ一般化、`順不同` の定義（記号キーによる名義化）を補った。§6.3・§6.9 の型を最小再現して `tsc --strict` で確認し、枝の取り違え（順次の枝に `順不同`、単一の枝に配列）とリテラルのゆれが赤になること、段数と結果の件数の不一致は型では止まらないことを変異で確かめた。件数は `統合例を実行` の実行時検証へ回す。`順次` の各段が別リクエストとして境界を跨ぐこと、`並行` と `順次` の入れ子は持たないことも規定した。`観測` の項名を辞書のドメイン語へ縛り、例示を `返金実行回数` へ改めた（§6.3, §6.9）。決定表は列挙で宣言する軸に限り同値類で立ててよく EX は各類の代表値で書くこと、決定表がドメインの例テーブルと不変条件から立つため統合例はセルを埋めないこと（manifest の `cell` も統合例では持たない）を規定し、対応する限界3件を §12 へ追加した（§6.10, §12, §14）。§13 へ用語「例（EX）」を新設し、例テーブルと EX の包含関係とユースケース記述でないことを明記した。
