...
- 版: 0.5（0.4 に §15 パイロット計画を追加。変更点は末尾「改訂履歴」）
...
2026-09-12 に確定。ADR-202609110017-01 決定1（新方式を新プラグインに起こす）を実行に移す順序を定める。§11 の導入ステップを、配布元リポジトリ（claude-shared-skills）の外にある題材で回す計画である。

### 15.1 題材

`budget-baker`（経費申請ドメイン）を最初の題材にする。理由は3つ。

- §14 が「核（`期待` の型・並行性・格子）が固まるまで一般化しない」と定めており、核を固めるには状態遷移・不変条件・DB を持つドメインが要る。同リポジトリの `docs/expense-application/event-storming.md` に7状態・不変条件7件・金額ルールの日本語仕様があり、これを備える
- 実装が白紙のため、§4 のレイアウト・§5 の日本語識別子・§10 の `CLAUDE.md` を設計どおり敷ける。既存規約との摩擦が無い
- イベントストーミングの記述が `intent.md` と型定義の素材にそのまま使える

却下した代替:

- `work-abstraction`（成熟した TS monorepo。zod schema が型の正本で識別子は英語）。ここで始めると「既存コードへどう後付けするか」という別の問いが核の設計に混ざる。後付けの問いは核が固まった後の移行検証（§11 最終行）に回す
- `quagmire-of-probability`（確率計算の純関数のみ）。状態・DB・並行が無く、格子（§6.10）と統合例（§6.9）を検証できない

候補3件がいずれも TypeScript であるため、最初の言語キットは TypeScript で確定する。

受容するコスト: テスト・CI・製品コードをゼロから作る。§11 週0〜1 はもともと既存製品を要さず、週2 の「既存機能1つ」は「最初の機能（経費申請の提出）」と読み替える。設計書が前提とする PdM・デザイナーは個人開発では1人が兼ねるため、週2〜3 の承認ループの運用感は検証が薄くなる。機械側（型・例・格子・ミューテーション・トレーサビリティ）は全て検証できる。

### 15.2 spec-kit の置き場

`budget-baker` 内に一体構成で置く（§4 のレイアウト、§14 の順序どおり）。規律を1つ置く: `spec-kit/` から `src/` を import しない（依存は `src/` → `spec-kit/` の一方向）。これを守れば後の抽出は「ディレクトリを切り出す」作業で済み、先回りの一般化をしない。

却下した代替:

- 配布元リポジトリに TypeScript パッケージを起こし `file:` 依存で引く案。2リポジトリ跨ぎで反復が重く、配布元へ TypeScript ツールチェーンを持ち込む。§14 が退けた「先に一般化」に当たる
- 専用の npm リポジトリを最初から起こす案。配布先が1つしか無い段階で分離する利点がない

### 15.3 着手順

§11 週0〜1 をキット骨格から始める。§6 の注文キャンセルを fixture として `例を定義` / `例を実行` / `例を表にする` / `格子を定義` / `例を格子にする` / manifest / `check-intent` / `traceability` を最小構成で動かし、次に `intent.md`（経費申請）を起こして fixture を実ドメインへ差し替える。差し替えの時点が、`部分<T>`・`期待` の照合子・格子の軸宣言が実ドメインに耐えるかの最初の検証になる。

却下した代替: `intent.md` から入る案。例テーブルは `例を定義` の型に依存するため、いずれにせよキットが先に要る。

### 15.4 配布元リポジトリ側の作業

パイロットと並行して配布元（claude-shared-skills）で行うのは次の3つに限る。

- 新プラグイン（名称未定）の器を `plugins/` に起こし、固まった方式層（skills 3本・`CLAUDE.md` 断片・CI 雛形・manifest と intent 目録の schema）を移す
- 統治機構の縮小（ADR-202609110017-01 決定3・4・7・8）。方式の成否に依存せず独立に成立するため、パイロットの合間の別セッションで随時進める
- プロジェクト `CLAUDE.md` の旧工程前提の記述の是正。これだけは先行させる。配布元で作業するどのセッションにも効くため
