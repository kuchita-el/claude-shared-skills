...
| 2 | 既存機能1つを題材にパイロット。Stryker 閾値を実測で決める。PdM に例の一覧と TS 例テーブルの両方を見せ、どちらでレビューするか決める。畳み込み後のセル数・AC 件数・1ブロックの行数・REQ あたりの決定表の件数と軸数・表記ゆれ判定（距離）の偽陽性率を測る | 運用感の確認 |
