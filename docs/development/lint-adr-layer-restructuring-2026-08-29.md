...
| AC | 内容 | 充足 | 根拠 |
|---|---|---|---|
| AC1 | 5レイヤがレイヤ単位で個別に起動できる単位へ分離され、各レイヤが消費する事実の収集が検査本体と別の単位に分かれている | 充足 | `lint-adr.sh` に `collect_scan_targets` / `collect_facts` の2つの収集単位と、`check_layer1_frontmatter_schema` / `check_layer2_index_sync` / `check_layer3_forward` / `check_layer3_reverse` / `check_layer4_related_references` / `check_layer5_filename_and_identifier` の6つの検査単位が在る。収集単位は違反を1件も出力しない。トップレベルの起動部は「走査対象の収集 → 事実の収集 → 各レイヤの呼び出し → 終了コードの決定」の並びのみ。任意の1レイヤを他レイヤの検査を実行せずに起動できることは `lint-adr-layers.bats` 面⑱と `lint-adr-xref.bats` 面⑧が検査する |
| AC2 | 検査内容が変更前後で同一。基準版と改訂後へ全 corpus を通し全文と終了コードが一致し、負例はいずれも終了コード1と1行以上の出力を返し、全22分岐が照合の中で発火する | 充足 | 基準版 `c126d72` との出力全文照合が `base=c126d72 corpora=41 diffs=0`。負例実測が `invalid_dirs=33 rc1=33 output_lines=37`（33件すべてが終了コード1、出力行の総数37）。22分岐の発火測定が `zero_fire_branches=0`。走査対象0件・対象ディレクトリ不在の境界も基準版と一致することを別途実測 |
...
| AC5 | 追加・改訂したテストに行番号固定・版据え置き型の常設ゲートが含まれない。回帰ケースに由来がテスト内コメントとして書かれている。新設・改訂した検査分岐について、当該分岐を落とす変異で対応する負例のケースだけが赤になることが実測・記録されている | 充足 | 追加した面⑰・面⑱・面⑲（layers）と面⑧（xref）はいずれも corpus と関数名だけを消費し、行番号にも版番号にも依存しない。各面のコメントに由来（Issue #800、および参照先の #722 / #794）を書いた。検出力は変異5件で実測し、本記録「検出力の変異実測」に当てた変異・期待・実測・赤になったケース名を表として残した |
| AC6 | `bash scripts/run-tests.sh` が終了コード0で終わり、runner の既存ガードがいずれも不発火 | 充足 | `all suites passed (6 suites)` / bats は `154 tests, 0 failures`。`EXPECTED_BATS` と glob 結果の双方向照合は「期待するテストファイルが無い」「期待リストに未登録のテストファイル」のいずれも報告していない。TAP のプラン行と結果件数の照合も不発火 |
...
| AC8 | 本 PR の設計判断について、着手時点の main に固定した `adr-scoping.md` の版（commit を明記）に照らした ADR 化要否の判定結果が記録されている | 充足 | 本記録「ADR 化要否の判定」。条文の版は commit `c126d72` と明記。設計判断4件について必要条件の成否・4項目それぞれの点数と根拠・合計点・境界（3点）との関係・結論を記録した。D3 のみ4点で境界を越え、起票の時期はユーザー裁定により PR 説明へ留める |
...
- **節4 の「lint-adr 系 bats 4ファイル（計914行）は書き直し」を実行していない。** spec 策定時点（2026-08-27）の現況に基づく指示であり、着手時点の現況は5ファイル1579行、うち lint の検査内容を検査するのは3本769行であった。そのまま実行すると直前の Issue #793 が投入した負例 fixture 4本と変異実測8件の被覆を作り直すことになる。節4 の眼目は「網羅の根拠を挙動の列挙に置き、既存テストのケース移植を出発点にしない」ことであり、被覆の根拠を実装ヘッダと条文から独立に導出したうえで既存アサーションを事後照合する形はこれを満たす（判断 D2）。
