...
- 含む集約: 実装プラン / 判断依頼 / 実装 / 検証実行 / PRレビュー / ビルド成果物 / 技術方針（仮称）
- 入力: チケット（種別問わず同一フロー）
...
骨子段階のため状態型は定義しない（Issue #130 で定義予定）。状態遷移も Issue #130 で定義予定。
...
合計 22 イベント（DL の全イベント網羅）。内部 21 件 + 外部 1 件。
...
合計 20 コマンド（DL の全コマンド網羅）。内部 19 件 + 外部 1 件。
