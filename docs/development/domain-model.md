...
| 実装 | タスク消化中 / 検証待ち / 完了 | 実装を完了する、タスクを再開する | 全タスクが完了した（派生）、実装が完了した、タスクが再開された |
...
実装プランを確定する:
    ...
    イベント: 実装プランが確定した / 確定プラン
    失敗時:
        - 未解消の判断依頼あり (判断依頼IDの一覧を持つ)

判断依頼の発行を反映する:
    契機: ポリシー（判断依頼発行反映ポリシー）
    入力: プランニング中プラン
    イベント: プランが判断依頼待ちに入った / 判断依頼中プラン

判断依頼の解消を反映する:
    契機: ポリシー（判断依頼解消反映ポリシー）
    入力: 判断依頼中プラン
    イベント: プランが判断依頼待ちから戻った / プランニング中プラン
```
...
`全タスクが完了した` は派生イベント。タスク消化中状態で全タスクが完了した時点で発火される（独立コマンドからは発火しない）。P5（検証実行起動ポリシー）のトリガーとなる。`実装が完了した` `タスクが再開された` はそれぞれシステム内部コマンド `実装を完了する` `タスクを再開する` の処理結果として発火される。
...
タスクを再開する:
    契機: ポリシー（検証失敗時タスク再開ポリシー）
    入力: 検証待ち実装
    イベント: タスクが再開された / タスク消化中実装
```
...
`検証中 → 完了` 遷移はコマンド `検証ゲートを実行する` の `イベント:` で発火するイベント（`検証ゲートを通過した` または `検証ゲートが失敗した`）と同期する集約内部の導出遷移として表現する。両イベントとも遷移後状態は `完了検証実行` に統一されるため、記法ガイド §「OR と / の混在禁止」に従い `イベント:` フィールドには遷移後状態を併記しない。
...
| 3 | 開始コマンド `検証ゲートを実行する` のアクター直接発行はない | `契機: ポリシー（検証実行起動ポリシー）` で唯一の発行経路を制限（Issue #179） |
...
変更を要求する:
    ...
    イベント: 変更が要求された / 変更要求対応中PR

レビューを承認する:
    契機: 外部指示
    入力: レビュー中PR AND レビュアー
    イベント: レビューが承認された / 必要Approve数に達した（派生）

PRをマージする:
    契機: 外部指示
    入力: マージ可能PR AND 開発者
    イベント: PRがマージされた / マージ済みPR
    失敗時:
        - 必要Approve数未達
...
PRレビュー集約は集約境界またぎ反映に独立コマンド・ポリシーを設けない。受け手集約観点でのイベント言語化として、`レビュー依頼がされた` `変更が要求された` 自体が集約内の状態遷移（`PR作成済みPR`/`変更要求対応中PR` → `レビュー中PR`、`レビュー中PR` → `変更要求対応中PR`）を表現するため、独立した反映コマンドは不要となる。状態出力は各コマンドの `イベント:` フィールドに `/` 区切りで記載する。
...
```
ビルド成果物を公開する:
    契機: ポリシー（ビルド成果物公開ポリシー）
    入力: チケットID AND ビルド成果物バージョン AND 公開先registry
    イベント: ビルド成果物が公開された / 公開済みビルド成果物
    失敗時:
        - バージョン重複
```
