...
| 実装 | タスク消化中 / 検証待ち / 完了 | 実装を開始する、実装を完了する、タスクを再開する | 実装が開始された、全タスクが完了した（派生）、実装が完了した、タスクが再開された |
...
`実装が開始された` はコマンド `実装を開始する` の処理結果として発火される（Issue #184）。`全タスクが完了した` は派生イベント。タスク消化中状態で全タスクが完了した時点で発火される（独立コマンドからは発火しない）。P5（検証実行起動ポリシー）のトリガーとなる。`実装が完了した` `タスクが再開された` はそれぞれシステム内部コマンド `実装を完了する` `タスクを再開する` の処理結果として発火される。
...
`タスク消化中 → 検証待ち` 遷移は派生イベント `全タスクが完了した` の発火と連動する集約内部の導出遷移であり、独立コマンドは持たない（コマンド発行の契機を持たない自集約内部の事実反映のため）。発火時、検証実行集約のライフサイクル開始（P5）を契機する。
...
合計 19 コマンド（DL の全コマンド網羅）。内部 18 件 + 外部 1 件。
