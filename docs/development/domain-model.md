...
| 実装 | タスク消化中 / 検証待ち / 完了 | 実装を開始する、タスクを完了する、実装を完了する、タスクを再開する | 実装が開始された、全タスクが完了した（派生）、実装が完了した、タスクが再開された |
...
タスクを完了する:
    入力: タスク AND 開発者
    イベント: （個別タスクの完了。最後のタスク完了時のみ派生イベント `全タスクが完了した` を発火）
...
`タスク消化中 → 検証待ち` 遷移はサブエンティティコマンド `タスクを完了する` の処理過程で発生する集約状態遷移（最後のタスク完了時、ガード条件「全タスク完了」を満たした時点で遷移）。コマンド処理結果として派生イベント `全タスクが完了した` が発火し、検証実行集約のライフサイクル開始（P5）を契機する（Issue #184 で「独立コマンドを持たない自集約内部の事実反映」記述を上書き）。
