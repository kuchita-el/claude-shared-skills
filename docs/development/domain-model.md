...
`レビュアー` は PRレビュー集約のみで参照されるが、`開発者` `テックリード` と並ぶアクター型として共通値節に集約する（命名規則の統一を優先）。`テックリード` は技術方針（仮称）集約のみで参照される（Task 8 でアクター型統一の理由により共通値に置く）。
...
`発火元` は判断依頼を発行した集約種別。現行値は `実装プラン集約` のみ（単一値の OR）。Issue #146 で `実装集約` を追加予定であり、その時点で OR 化を検討する（論点節参照）。
...
回答を受理する:
    ...
    出力（成功時）: 項目が回答済みになった OR 全判断依頼項目が解消された
    出力（失敗時）: 受理失敗理由
...
`検証を開始する` は全タスク完了を事前条件として要求する。検証失敗時は `タスク消化中` に戻り、修正後に再度 `検証を開始する` → `検証ゲートを実行する` を実行する。
...
破棄する:
    入力: PR作成済みPR OR レビュー中PR OR 変更要求対応中PR
        AND 開発者
    出力: 破棄済みPR
```
...
公開失敗理由 =
    バージョン重複
    OR registry接続失敗（実装側関心。ドメイン語彙としては保持）
```
...
**現状の対応**: 4 ワークフロー（実装プランを確定する / PRをマージしてビルド成果物を公開する / 変更要求からの再プランニング / プランニング再開）はいずれも明確な集約横断であり、判断に迷う粒度は含めなかった。
