...
- ポリシー P5（検証実行起動）は実装集約 `全タスクが完了した` を契機に検証実行集約 `検証ゲートを実行する` を発火する（Issue #179 で集約境界をまたぐ Saga ポリシーとして新設）
...
P6（実装完了反映ポリシー）と P7（検証失敗時タスク再開ポリシー）は、検証実行集約のドメインイベントを契機に実装集約のシステム内部コマンドを発行する Saga ポリシー。記法ガイド §ポリシーの配置に従い、コマンドを発行する側（受け手集約 = 実装集約）のセクションに配置する（Issue #179）。
...
P5（検証実行起動ポリシー）は実装集約の派生イベント `全タスクが完了した` を契機に検証実行集約の `検証ゲートを実行する` を発行する Saga ポリシー。記法ガイド §ポリシーの配置に従い、コマンドを発行する側（受け手集約 = 検証実行集約）のセクションに配置する（Issue #179）。
