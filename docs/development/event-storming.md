...
| H4 | ADR（Architecture Decision Records）のライフサイクル管理が未定義。横断的技術方針の蓄積・参照・廃止の仕組みがない | 技術的意思決定 / 組織横断 | **解消済み（Issue #130、ADR-20260711-3 で状態モデル再設計）**: 「技術的意思決定」集約として正式モデル化。Issue #130 は英文4状態で確定したが、ADR-20260711-3 が承認軸（提案中/承認済み/却下）×有効性軸（有効/上書き済み/廃止済み）の2軸へ再設計（同 ADR は #570 で決定クラスタ単位の後継6本へ分割され `上書き済み`。2軸モデルの現行出典は ADR-20260726-01）。ライフサイクルコマンド/イベント・アクター解像度（提案者/承認者）を確定。運用基盤（テンプレート・採番運用・参照導線・粒度判定基準）は #169 / #170 / #171 に委譲 |
...
| 技術的意思決定 | 横断的な技術方針（アーキテクチャ・設計方針）の意思決定を表すドメイン集約。実体は ADR 形式の Markdown ファイル | Issue #130 で「技術方針（仮称）」から正式名称に確定。ライフサイクルは ADR-20260711-3 で承認軸（提案中/承認済み/却下）×有効性軸（有効/上書き済み/廃止済み）の2軸に再設計（同 ADR は #570 で後継6本へ分割され `上書き済み` となり、2軸モデルの現行出典は ADR-20260726-01） |
