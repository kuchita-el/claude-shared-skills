...
| 仕様の不明点がエスカレーションされた | プランニング中の質問を上流（企画）に返送。複数項目を束ねた1イベント | 実装プラン |
...
| 全エスカレーション項目が解消された | 実装プラン集約の全未回答項目が解消されたことを示す派生イベント。プランニング再開（P1ポリシー）のトリガー | 実装プラン |
...
| 実装プランを確定する | 実装プランが確定した | 全エスカレーション項目が解消されていることが不変条件 |
...
| # | アクター名 | 種別 | 発行するコマンド |
|---|---|---|---|
| 1 | 開発者 | 人間 | プランニングを開始する、実装プランを確定する、仕様の不明点をエスカレーションする、検証ゲートを実行する、PRを作成する、レビュー依頼をする、PRをマージする、PRを破棄する |
...
**含むコマンド**: プランニングを開始する、実装プランを確定する、仕様の不明点をエスカレーションする

**含むイベント**: プランニングが開始された、実装プランが確定した、仕様の不明点がエスカレーションされた、仕様の回答が届いた、全エスカレーション項目が解消された
...
stateDiagram-v2
    ...
    プランニング中 --> エスカレーション中: 仕様の不明点がエスカレーションされた
    エスカレーション中 --> プランニング中: 全エスカレーション項目が解消された
...
| 状態 | 定義 | 遷移元 | 遷移先 |
|---|---|---|---|
| プランニング中 | 技術設計・タスク分解を進めている | 初期状態, エスカレーション中 | エスカレーション中, 確定 |
| エスカレーション中 | 未回答のエスカレーション項目が存在する状態（エスカレーション項目の状態から導出される） | プランニング中 | プランニング中 |
| 確定 | 開発者が実装に入れると判断した状態。全エスカレーション項目が解消されていることが不変条件 | プランニング中 | 終了 |
...
|---|---|---|---|
| P1 | プランニング再開 | 全エスカレーション項目が解消された | プランニングを開始する（WIPインスタンスがあれば同一インスタンスで作業継続、なければ新規作成。いずれも正常系） |
...
- **エスカレーション項目のサブエンティティ化（Issue #127 F2）**: エスカレーション項目は実装プラン集約に閉じる（他集約から参照されず、実装プラン集約と運命を共にし、独立トランザクション境界を要しない）ため、独立集約化は過剰。実装プラン集約のサブエンティティとしてフラット構造で扱う。業務用語「エスカレーション」は1回の問い合わせ行為として用語・イベント・コマンドに残し、モデル内部は項目フラット。※ **永続性の観点から Issue #139 で再検討予定**: 本プロジェクトの成果物はローカルmarkdownファイルで、WIP実装プラン削除時にエスカレーション項目情報が失われ解消判定不能となる問題が識別された。エスカレーション1回単位での独立集約化とイベント駆動連携に再設計する方針
- **実装プラン確定の不変条件（Issue #127 F2）**: 実装プラン確定の不変条件として「全エスカレーション項目が解消された」を要求し、再開トリガーは派生イベント `全エスカレーション項目が解消された` とする
...
| エスカレーション項目 | エスカレーションとして問い合わせた個別の不明点。実装プラン集約のサブエンティティとして状態（未回答/回答済み）を持つ。全項目が回答済みになると実装プラン確定の前提条件が満たされ、派生イベント `全エスカレーション項目が解消された` が発行される | |
