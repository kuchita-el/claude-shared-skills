...
| P6 | 実装完了反映 | 検証ゲートを通過した | （実装集約の `タスク消化中→完了` 遷移） | 検証実行集約から実装集約への戻し遷移（成功時）。Sagaポリシー化（Issue #179、設計判断 L347 を上書き） |
| P7 | 検証失敗時タスク再開 | 検証ゲートが失敗した | （実装集約のタスク消化中状態を維持。修正継続） | 戻し遷移（失敗時）。状態は変化せず、再修正後の `全タスクが完了した` 再発火で検証実行集約の新インスタンスが起動する（Issue #179、設計判断 L347 を上書き） |
...
- **検証実行集約の独立化と二視点併存解消（Issue #179）**: ローカル実行（開発者視点・オーケストレーター視点）とCI実行（外部委任視点・待ち状態）が同一コマンド `検証ゲートを実行する` で併存していた構造問題（Issue #173 パターン3）を解消するため、検証実行を実装集約から分離独立集約として扱う方針（E方針）を採用した。
    ...
    - 視点統合: モデル内ではローカル/CI を区別しない（設計判断「セルフ動作確認の統合」L349 を踏襲）。実行コンテキストの違いは運用情報として H5 補足に残し、アクターからの直接発行（旧「開発者: 検証ゲートを実行する」「CI: 検証ゲートを実行する」）は廃止
    ...
    - L347（検証失敗時のポリシー廃止）の上書き: 集約分離により失敗時の戻し遷移が集約をまたぐSaga連携となるため、L347 を上書きし P7 を再導入した
    ...
    - **後続Issue #182（パターン2 戦術設計）への前提条件**: 実装集約の含むコマンドから `検証ゲートを実行する` が消失。コマンド名は維持されたため、#182 の確定方針D（`検証ゲートを実行する` の `成功時:`/`失敗時:` への統合）はそのまま新集約（検証実行集約）配下に適用可能。`docs/development/domain-model.md` の関連箇所（L21 集約サマリ表、L344 実装集約定義、L379-422 検証ゲート関連型・関数定義、L971-994 関数表）の更新は #182 で実施する
    - **後続Issue #181（パターン1 集約間依存Saga化）への前提条件**: 集約間Sagaポリシーの先行例として本Issueの記法（P5/P6/P7、実行コマンド欄に状態遷移記述を許容）を参照可能。直接の影響はなし（独立軸）
    - **`docs/big-picture.md:72`**: 「セルフ動作確認は検証ゲートに統合」の記述は据え置き。「検証ゲート」は仕様名として維持されるため、Big Picture レベルでは表記変更不要
