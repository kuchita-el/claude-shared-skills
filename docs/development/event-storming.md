...
| 12 | 変更が要求された | レビュアーによるRequest Changes。ブロッキング要素を含む | PRレビュー |
| 13 | レビューが承認された | 個別レビュアーによる承認 | PRレビュー |
| 14 | 必要Approve数に達した | レビュイーが定めた承認基準を満たした | PRレビュー |
| 15 | PRがマージされた | レビュー通過後にマージ | PRレビュー |
...
| 5 | テストを作成する | テストが作成された | |
| 6 | プロダクションコードを実装する | 実装された | |
| 7 | ブランチを作成する | ブランチが作成された | |
| 8 | 検証ゲートを実行する | 検証ゲートを通過した / 検証ゲートが失敗した | ローカル: 手動/git hook、CI: PR作成/更新時自動 |
| 9 | PRを作成する | PRが作成された | |
| 10 | 変更を要求する | 変更が要求された | |
| 11 | レビューを承認する | レビューが承認された | |
| 12 | PRをマージする | PRがマージされた | |
...
**含むコマンド**: PRを作成する、変更を要求する、レビューを承認する、PRをマージする

**含むイベント**: PRが作成された、変更が要求された、レビューが承認された、必要Approve数に達した、PRがマージされた
...
| 変更要求対応中 | 変更要求を受けて修正コミットを作成中 | レビュー中 | レビュー中 |
...
| P3 | 変更要求対応 | 変更が要求された | プロダクションコードを実装する |
| P4 | マージ可能判定 | 必要Approve数に達した | マージ可能状態になる（マージは開発者が手動実行） |
...
| H7 | 実装プランのNo-op判定基準が未定義（差し戻し時、どこまでが「軽微」か） | 実装プラン | **解消済み**: 確定判断の主体は常に開発者。プラン見直し要否の判断も開発者に委ねる |
| H8 | 差し戻し時の同一インスタンス巻き戻し vs 新規インスタンス作成の判断が未定義 | 全集約 | **解消済み**: 集約のライフサイクルは線形。開発者が必要と判断した場合に新規インスタンスを作成する |
...
- **プラン見直しの判断**: 変更要求を受けた際にプラン見直しが必要かは開発者が判断する。特定のドメインイベントや集約状態としてはモデル化しない。必要と判断した場合、開発者が新規インスタンスを作成する
...
| 変更要求 | レビュアーがRequest Changesでブロッキング要素を含む修正を要求すること | |
