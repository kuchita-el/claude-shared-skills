...
| 4 | 横断的な技術方針を決定する | 横断的な技術方針が決定された | 技術方針集約（詳細は Issue #130） |
...
| H8 | 差し戻し時の同一インスタンス巻き戻し vs 新規インスタンス作成の判断が未定義 | 全集約 | **解消済み（導出結果）**: 独立決定ではなく、WIP/確定の状態分類から導出される。WIPインスタンスがあれば状態遷移で再開、なければ新規作成。変更要求時はWIP非存在のため必然的に新規作成となる（P1/P2参照） |
...
- **外部/内部イベント分離（Issue #127 F2）**: 企画コンテキスト境界を明示するため、企画発の外部イベント（`外部_仕様の回答が返された`）と開発側の内部イベント（`仕様の回答が届いた`）を分離する。上流との連携方式の明確化に寄与する
