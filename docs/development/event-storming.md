...
| 全タスクが完了した | タスク消化中の全タスクが完了。検証実行起動ポリシー（P5）のトリガー | 実装 |
...
| # | アクター名 | 種別 | 発行するコマンド |
|---|---|---|---|
| 1 | 開発者 | 人間 | プランニングを開始する、実装プランを確定する、判断依頼を発行する、回答を受理する、PRを作成する、レビュー依頼をする、PRをマージする、PRを破棄する |
...
**責務**: TDDサイクルによるコード変更の管理。検証ゲートの実行は検証実行集約に分離（Issue #179）。

**含むコマンド**: （タスク消化中の内部工程のみ。集約境界外への発火は `全タスクが完了した` イベント）
...
stateDiagram-v2
    [*] --> タスク消化中: 実装プランが確定した
    タスク消化中 --> 完了: 検証ゲートを通過した
    完了 --> [*]
```

| 状態 | 定義 | 遷移元 | 遷移先 |
|---|---|---|---|
| タスク消化中 | TDDサイクル（Red→Green→Refactor）をタスク単位で繰り返している。全タスク完了時に `全タスクが完了した` イベントを発火し、検証実行集約のライフサイクル開始（P5）を契機する | 初期状態 | 完了 |
| 完了 | 検証通過。レビュー依頼に進める | タスク消化中 | 終了 |

- `タスク消化中 → 完了` 遷移は実装完了反映ポリシー（P6）が検証実行集約の `検証ゲートを通過した` イベントを受信して駆動する（Saga 戻し遷移）。状態遷移ラベルはクロス集約イベントを直接表記している
- `検証ゲートが失敗した` 受信時は実装集約のタスク消化中状態を維持し、修正を継続する（検証失敗時タスク再開ポリシー P7。状態は変化せず）。再修正後の `全タスクが完了した` 再発火で検証実行集約の新インスタンスが起動する
- 検証実行集約とは直接参照を持たず、ドメインイベント経由で連携する（F7 集約疎結合方針と整合）
- 旧設計の `検証中` 状態は本集約から削除し、検証実行集約に移管した（Issue #179）
...
| 状態 | 定義 | 遷移元 | 遷移先 |
|---|---|---|---|
| 検証中 | 検証ゲート（型チェック＋Lint＋テスト）を実行中 | 初期状態 | 完了 |
| 完了 | 検証ゲートの通過/失敗事実が確定し、ドメインイベントが発火された状態 | 検証中 | 終了 |
...
| P5 | 検証実行起動 | 全タスクが完了した | 検証ゲートを実行する | 実装集約から検証実行集約への駆動。Sagaポリシー化（Issue #179） |
| P6 | 実装完了反映 | 検証ゲートを通過した | （実装集約の `タスク消化中→完了` 遷移） | 検証実行集約から実装集約への戻し遷移（成功時）。Sagaポリシー化（Issue #179、設計判断「検証失敗時のポリシー廃止」を上書き） |
| P7 | 検証失敗時タスク再開 | 検証ゲートが失敗した | （実装集約のタスク消化中状態を維持。修正継続） | 戻し遷移（失敗時）。状態は変化せず、再修正後の `全タスクが完了した` 再発火で検証実行集約の新インスタンスが起動する（Issue #179、設計判断「検証失敗時のポリシー廃止」を上書き） |
...
- P5/P6/P7 は Issue #179 で新規採番。集約境界をまたぐSagaポリシーであり、実行するコマンド欄が状態遷移記述となる項目を含む（実装集約に対する戻し遷移は集約境界の外から駆動されるため）
...
- **検証失敗時のポリシー廃止（Issue #179 で上書き）**: ~~検証ゲート失敗→タスク消化中への遷移は実装集約の状態遷移図で直接表現されており、独立したポリシーとして定義する必要がない。状態遷移図が権威ある定義~~ → Issue #179 で検証実行集約を実装集約から分離した結果、検証ゲート失敗→タスク消化中への戻し遷移は集約をまたぐSaga連携となり、実装集約の状態遷移図のみでは表現できなくなった。P7（検証失敗時タスク再開ポリシー）として再導入し、状態維持のみのSagaポリシー化を採用する
...
- **検証実行集約の独立化と二視点併存解消（Issue #179）**: ローカル実行（開発者視点・オーケストレーター視点）とCI実行（外部委任視点・待ち状態）が同一コマンド `検証ゲートを実行する` で併存していた構造問題（Issue #173 パターン3）を解消するため、検証実行を実装集約から分離独立集約として扱う方針（E方針）を採用した。
    ...
    - Sagaポリシー化: 集約間連携を P5（検証実行起動）/P6（実装完了反映）/P7（検証失敗時タスク再開）で表現。実装集約は「全タスクが完了した」イベントを発火し、P5 が検証実行集約の `検証ゲートを実行する` コマンドを発行する
    - 状態粒度（検証実行集約）: `検証中 / 完了` の2状態。失敗時も終端を `完了` に統一し、再実行は新インスタンスとして扱う（終端状態を分けると失敗時のライフサイクル方針規定が論点拡散を招くため）
    - 実装集約の改変: `検証中` 状態を削除し `タスク消化中 → 完了` に簡素化。「自集約イベントが自集約状態を遷移させる」構造（Issue #173 パターン2）が実装集約から消失
    - F7 集約疎結合の踏襲: 検証実行集約も実装集約・PRレビュー集約への直接参照を持たず、ドメインイベントとチケット ID 経由で連携する
    - 設計判断「検証失敗時のポリシー廃止」の上書き: 集約分離により失敗時の戻し遷移が集約をまたぐSaga連携となるため、当該設計判断を上書きし P7 を再導入した
    - 新規イベント `全タスクが完了した` の追加: 旧 mermaid 図のガード条件テキスト「全タスク完了」をドメインイベント化。実装集約の終端で発火し P5 のトリガーとなる
    - 戦略設計レベルの記法: ポリシー表のみで集約間連携を表現（戦術設計記法の `契機: ポリシー(...)` フィールドは domain-model.md 側で扱う、#182 のスコープ）。実行コマンド欄が状態遷移記述となる項目を許容
    - **P6/P7 分割の根拠**: 同一集約内に対する戻し遷移でも、トリガーイベント（`検証ゲートを通過した` / `検証ゲートが失敗した`）と反応の方向（完了遷移 / 状態維持）が異なるため、別ポリシーIDに分割した。単一ポリシーで成功/失敗を分岐させる記法も選択肢としてあったが、Sagaポリシー表で「1イベント=1ポリシー行」の対応を維持する方が読解しやすいと判断した
    - **mermaid 記法上の例外**: 実装集約の状態遷移mermaidラベルに、検証実行集約のドメインイベント `検証ゲートを通過した` をクロス集約イベントとして直接表記する。これは Saga 戻し遷移のうち状態遷移を伴うもの（成功時=P6）のみであり、失敗時（P7）は状態を変化させないためラベル化せず注記で表現する。非対称な扱いの理由は「状態遷移を起こすのは成功時のみ」という事実の反映であり、記法上の不整合ではない
    - **後続Issue #182（パターン2 戦術設計）への前提条件**: 実装集約の含むコマンドから `検証ゲートを実行する` が消失。コマンド名は維持されたため、#182 の確定方針D（`検証ゲートを実行する` の `成功時:`/`失敗時:` への統合）はそのまま新集約（検証実行集約）配下に適用可能。`docs/development/domain-model.md` の関連箇所（集約サマリ表、実装集約定義、検証ゲート関連の型・関数定義、関数一覧表）の更新は #182 で実施する
    - **後続Issue #181（パターン1 集約間依存Saga化）への前提条件**: 集約間Sagaポリシーの先行例として本Issueの記法（P5/P6/P7、実行コマンド欄に状態遷移記述を許容）を参照可能。直接の影響はなし（独立軸）
