...
| 4 | 仕様の回答が届いた | エスカレーション項目に対する回答を開発側で受信（部分回答を含む） | 実装プラン |
| 5 | 全エスカレーション項目が解消された | プラン集約の全未回答項目が解消されたことを示す派生イベント。プラン再開（P1ポリシー）のトリガー | 実装プラン |
| 6 | 横断的な技術方針が決定された | 実装プラン確定の前提となるアーキテクチャ・設計方針の決定 | 技術方針（仮称、Issue #130 で確定） |
...
| 15 | PRが破棄された | PRがCloseされ、対応が終了された | PRレビュー |
...
| 状態 | 定義 | 遷移元 | 遷移先 |
|---|---|---|---|
| 作成中 | 技術設計・タスク分解を検討中。エスカレーション項目の存在（未回答/回答済み）は派生属性として保持 | 初期状態 | 確定 |
| 確定 | 開発者が実装に入れると判断した状態。全エスカレーション項目が解消されていることが不変条件 | 作成中 | 終了 |

- 「エスカレーション中」は独立した状態ではなく、未回答エスカレーション項目の存在という派生属性で表現する
- 確定の判断主体は常に開発者。作成主体（人間/エージェント）によらない
- P1（プラン再開）トリガー時、WIPインスタンス（＝作成中プラン集約）があれば同一インスタンスで作業再開。WIPがなければ新規作成
...
| 状態 | 定義 | 遷移元 | 遷移先 |
|---|---|---|---|
| PR作成済み | PRが作成され、検証ゲート通過後のレビュー依頼を待っている | 初期状態 | レビュー中, Close済み |
| レビュー中 | レビュアーが確認中。個別の承認（レビューが承認された）はこの状態内で蓄積 | PR作成済み, 変更要求対応中 | 変更要求対応中, マージ可能, Close済み |
| 変更要求対応中 | 変更要求を受けており、再依頼またはCloseの判断・対応を検討中の状態 | レビュー中 | レビュー中, Close済み |
| マージ可能 | 必要Approve数に到達。開発者がマージを実行できる | レビュー中 | マージ済み |
| マージ済み | マージ完了。ビルド成果物集約へ引き渡し（P3ポリシー） | マージ可能 | 終了 |
| Close済み | PRが破棄され、対応が終了した状態 | PR作成済み, レビュー中, 変更要求対応中 | 終了 |
...
|---|---|---|---|
| P1 | プラン再開 | 全エスカレーション項目が解消された | 実装プランを作成する（WIPインスタンスがあれば状態遷移で再開、なければ新規作成。いずれも正常系） |
...
| H6 | 必要Approve数の決定基準が暗黙的（レビュイーの個人判断） | PRレビュー | **現状運用の記録**: チーム最低制約として「1 Approve以上」が明文化済み。それ以上はレビュイー個人裁量で、決定タイミングも動的（事前確定不可なケースあり）。モデル化・DoR組み込みは却下（マージ判断はユーザーから不可視、モデル化は過剰）。暗黙運用の現状記録にとどめる |
...
- **ポリシー分岐の本質とWIP/確定分類（Issue #127 F1）**: P1/P2のコマンドが同一（「実装プランを作成する」）であるのは、集約のライフサイクル状態（WIPインスタンスの有無）による分岐の結果。WIPあれば状態遷移で再開、なければ新規集約ライフサイクル開始。P2は確定済み状態からの発火のためWIP非存在、必然的に新規作成となる。WIP消失は異常系ではなく正常系として扱う
...
- **エスカレーション項目のサブエンティティ化（Issue #127 F2）**: エスカレーション項目はプラン集約に閉じる（他集約から参照されず、プランと運命を共にし、独立トランザクション境界を要しない）ため、独立集約化は過剰。プラン集約のサブエンティティとしてフラット構造で扱う。業務用語「エスカレーション」は1回の問い合わせ行為として用語・イベント・コマンドに残し、モデル内部は項目フラット
- **プラン集約状態の派生属性化（Issue #127 F2）**: 「エスカレーション中」を独立状態として持たず、未回答エスカレーション項目の存在（派生属性）として表現する。状態遷移は「作成中」「確定」の2状態に単純化。プラン確定の不変条件として「全項目回答済み」を要求し、再開トリガーは派生イベント `全エスカレーション項目が解消された` とする
...
- **「変更要求対応中」状態の純化（Issue #127 F7）**: 変更要求対応中はPRレビュー側の状態として、再依頼またはCloseの判断・対応を検討中であることを示す。プラン側の動き（新プラン作成、旧プラン参照関係）は集約外の話として切り離す。P2ポリシーは `変更が要求された` イベント発行時に独立起動する
