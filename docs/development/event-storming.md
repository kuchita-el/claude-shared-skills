...
| # | アクター名 | 種別 | 発行するコマンド |
|---|---|---|---|
| 1 | 開発者 | 人間 | プランニングを開始する、実装プランを確定する、実装を開始する、判断依頼を発行する、回答を受理する、PRを作成する、レビュー依頼をする、PRをマージする、PRを破棄する |
...
stateDiagram-v2
    [*] --> プランニング中: プランニングを開始する
    プランニング中 --> 判断依頼中: [判断依頼集約に未解消インスタンス存在]
    判断依頼中 --> プランニング中: [判断依頼集約に未解消インスタンスなし]
...
**含むコマンド**: 実装を開始する、実装を完了する、タスクを再開する（`実装を完了する` `タスクを再開する` はシステム内部コマンドで P6/P7 が自動発行。`実装を開始する` は P10 自動発行と開発者直接発行のハイブリッド）
...
| 状態 | 定義 | 遷移元 | 遷移先 | entry時発行イベント |
|---|---|---|---|---|
| 検証中 | 検証ゲート（型チェック＋Lint＋テスト）を実行中 | 初期状態 | 完了 | なし（コマンド受理時点ではドメインイベントは発行されない） |
| 完了 | 検証ゲートの通過/失敗事実が確定し、ドメインイベントが発火された状態 | 検証中 | 終了 | 検証通過時: `検証ゲートを通過した` / 検証失敗時: `検証ゲートが失敗した`（並列遷移ケース） |
...
stateDiagram-v2
    ...
    レビュー中 --> マージ可能: レビューを承認する [必要Approve数到達]
    レビュー中 --> 破棄済み: PRを破棄する
