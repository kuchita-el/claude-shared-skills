...
- 採番方式・配置 `docs/adr/`・テンプレート・粒度判定基準などの運用基盤詳細は Issue #169（Phase 0）で確定する。本ドキュメントでは集約モデル上の扱いのみを規定する
...
| 採番方式 | 技術的意思決定集約のインスタンスを一意に識別するADR識別子の採番規則 | 規則の実体は `manage-adr` スキルの `references/adr-model.md`「採番方式」節。詳細運用は Issue #169 に委譲 |
