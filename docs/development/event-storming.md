...
| 6 | 検証ゲートを通過した | 型チェック＋Lint＋テスト全パスの複合検証 | 実装 |
| 7 | 検証ゲートが失敗した | 型エラー・Lint違反・テスト失敗のいずれか | 実装 |
| 8 | PRが作成された | PRをGitHub上に作成 | PRレビュー |
| 9 | レビュー依頼がされた | 検証ゲート通過後、レビュアーにレビューを依頼 | PRレビュー |
| 10 | 変更が要求された | レビュアーによるRequest Changes。ブロッキング要素を含む | PRレビュー |
| 11 | レビューが承認された | 個別レビュアーによる承認 | PRレビュー |
| 12 | 必要Approve数に達した | レビュイーが定めた承認基準を満たした | PRレビュー |
| 13 | PRがマージされた | レビュー通過後にマージ | PRレビュー |
...
| 5 | ブランチを作成する | ブランチが作成された | |
| 6 | 検証ゲートを実行する | 検証ゲートを通過した / 検証ゲートが失敗した | ローカル: 手動/git hook、CI: PR作成/更新時自動 |
| 7 | PRを作成する | PRが作成された | |
| 8 | レビュー依頼をする | レビュー依頼がされた | 前提条件: 検証ゲート通過 |
| 9 | 変更を要求する | 変更が要求された | |
| 10 | レビューを承認する | レビューが承認された | |
| 11 | PRをマージする | PRがマージされた | |

## アクター

| # | アクター名 | 種別 | 発行するコマンド |
|---|---|---|---|
| 1 | 開発者 | 人間 | 実装プランを作成する、仕様の不明点をエスカレーションする、ブランチを作成する、PRを作成する、レビュー依頼をする、PRをマージする |
...
**含むコマンド**: 検証ゲートを実行する、ブランチを作成する

**含むイベント**: 検証ゲートを通過した、検証ゲートが失敗した、ブランチが作成された
