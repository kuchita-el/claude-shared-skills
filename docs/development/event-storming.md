...
| 技術的意思決定が却下された | 提案が承認に至らず終了した。承認軸 `却下` への遷移時に発行（ADR-20260726-01 の2軸モデルで承認軸終端として追加） | 技術的意思決定 |
...
| 技術的意思決定を却下する | 技術的意思決定が却下された | 技術的意思決定集約。発行アクター: 承認者。状態遷移 `提案中 → 却下` の遷移トリガー（ADR-20260726-01 の2軸モデルで承認軸終端として追加） |
...
- 後継ADR参照（属性、有効性軸 `上書き済み` のとき保持。分割時は複数後継をリスト値で保持）: 後続ADRの識別子を文字列として保持する値オブジェクト。集約インスタンスへの直接参照は持たず、自己参照・循環参照の構造的不整合を防ぐ。front-matter の `superseded-by` に対応し、1本の意思決定が複数後継へ分割される場合はリスト値を取る（ADR-20260726-01 決定2 ／ ADR-20260726-02 決定1）

#### 状態遷移

状態は**承認軸**（`提案中`/`承認済み`/`却下`・承認の歴史事実で不変）と**有効性軸**（`有効`/`上書き済み`/`廃止済み`・現在の効力で可変）の2軸で表現する。承認軸が `承認済み` に達した後、有効性軸が内部で遷移する合成状態として1つの状態機械に展開する（`却下` は承認軸の終端）。スキーマ・遷移ルールの生きた実体は `manage-adr` スキルの `references/adr-model.md`（値域・遷移ごとの必須ルールをここに集約する）。この2軸モデルの採択は ADR-20260726-01 決定1。
...
- 承認軸 `承認済み` は不変の履歴事実であり、退役（`上書き済み`/`廃止済み`）は有効性軸で表現する。承認済みに達すると有効性軸の初期サブ状態 `有効` に入る。この2軸分離により「承認された歴史事実」を「今の効力」で上書きしない（ADR-20260726-01 決定1）
...
| H4 | ADR（Architecture Decision Records）のライフサイクル管理が未定義。横断的技術方針の蓄積・参照・廃止の仕組みがない | 技術的意思決定 / 組織横断 | **解消済み（Issue #130、ADR-202607112231-01 で状態モデル再設計）**: 「技術的意思決定」集約として正式モデル化。Issue #130 は英文4状態で確定したが、ADR-202607112231-01 が承認軸（提案中/承認済み/却下）×有効性軸（有効/上書き済み/廃止済み）の2軸へ再設計（同 ADR は #570 で決定クラスタ単位の後継6本へ分割され `上書き済み`。2軸モデルの現行出典は ADR-20260726-01）。ライフサイクルコマンド/イベント・アクター解像度（提案者/承認者）を確定。運用基盤（テンプレート・採番運用・参照導線・粒度判定基準）は #169 / #170 / #171 に委譲 |
...
| 技術的意思決定 | 横断的な技術方針（アーキテクチャ・設計方針）の意思決定を表すドメイン集約。実体は ADR 形式の Markdown ファイル | Issue #130 で「技術方針（仮称）」から正式名称に確定。ライフサイクルは ADR-202607112231-01 で承認軸（提案中/承認済み/却下）×有効性軸（有効/上書き済み/廃止済み）の2軸に再設計（同 ADR は #570 で後継6本へ分割され `上書き済み` となり、2軸モデルの現行出典は ADR-20260726-01） |
...
| 上書き済み | 技術的意思決定の有効性軸の状態のひとつ。運用中の意思決定が後継ADRにより置き換えられ、後継ADR識別子（`superseded-by`）を保持する状態 | ADR-20260726-01 の2軸モデルで確定（旧英文状態名を置換）。承認軸 `承認済み` は不変のまま有効性軸がこの状態へ遷移する。コマンド名は和文「技術的意思決定を上書きする」 |
