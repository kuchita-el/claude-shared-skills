...
| イベント名（過去形） | 説明 | 所属集約 |
|---|---|---|
| 実装プランが確定した | エスカレーション・技術相談を経てプラン承認 | 実装プラン |
| 仕様の不明点がエスカレーションされた | プラン作成中の質問を上流（企画）に返送。複数項目を束ねた1イベント | 実装プラン |
| 外部_仕様の回答が返された | 企画担当が開発側へ回答を返送（部分回答を含む） | 企画コンテキスト（外部） |
| 仕様の回答が届いた | エスカレーション項目に対する回答を開発側で受信（部分回答を含む）。`外部_仕様の回答が返された` を受信してACL層が発行 | 実装プラン |
| 全エスカレーション項目が解消された | プラン集約の全未回答項目が解消されたことを示す派生イベント。プラン再開（P1ポリシー）のトリガー | 実装プラン |
...
| コマンド名 | トリガーするイベント | 備考 |
|---|---|---|
| 実装プランを作成する | 実装プランが確定した | WIP有無により挙動分岐（WIPあれば状態遷移で再開、なければ新規作成）。詳細はP1/P2参照 |
| 仕様の不明点をエスカレーションする | 仕様の不明点がエスカレーションされた | 複数項目を束ねた1コマンド |
| 仕様の回答を返す | 外部_仕様の回答が返された | 外部コマンド（企画コンテキスト） |
...
- **ID付与の方針（Issue #127 F9, Issue #138で既存ID削除を適用）**: ポリシー（P1-P3）・ホットスポット（H1-H8）のID体系は維持。ドメインイベント・コマンドの#連番IDは Issue #138 で削除し、名前参照に統一した。以降の新規追加（イベント・コマンド・アクター）は名前参照を基本とし、ID付与は行わない。連番IDは項目追加・削除で番号がズレ、他文書からの参照が壊れやすいため
