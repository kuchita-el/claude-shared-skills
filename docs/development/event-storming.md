...
| 13 | ビルド成果物が公開された | ビルド成果物（バージョン付き）が生成・公開された | ビルド成果物 |
...
| 11 | ビルド成果物を公開する | ビルド成果物が公開された | PRマージを起点にCIが自動実行 |
...
stateDiagram-v2
    [*] --> 生成中: PRがマージされた
    生成中 --> 公開済み: ビルド成果物が公開された
    公開済み --> [*]
```

| 状態 | 定義 | 遷移元 | 遷移先 |
|---|---|---|---|
| 生成中 | CIがビルドを実行中 | 初期状態 | 公開済み |
| 公開済み | registry に登録完了。検証コンテキスト（QAデプロイ）・リリースコンテキスト（デプロイ候補選択）から参照可能 | 生成中 | 終了 |

- 廃止イベントは扱わない（新バージョンが追加されていくのみ、ホットスポット Big Picture #9 解消方針）
...
- **ビルド成果物公開を開発コンテキスト所属とする**: CI（ビルド・テスト・統合）とCD（Continuous Delivery＝ビルド成果物公開まで）は開発コンテキストが所有。ビルド設定・テスト・依存解決の知識領域と整合する。成果物の利用判断（デプロイ可否判定・リリース候補選択）はリリースコンテキスト所有で、「ビルド成果物管理」と「ビルド成果物の利用判断」を切り分ける（Issue #125）
...
| ビルド成果物 | CIがマージ後に生成するバージョン付きのデプロイ可能な成果物 | registry が外部マネージドかself-hostかは実装選択 |
