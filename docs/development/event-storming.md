...
| 技術的意思決定を上書きする | 技術的意思決定が上書きされた | 技術的意思決定集約。発行アクター: 承認者。状態遷移 `承認済み（有効） → 上書き済み` の遷移トリガー。後継ADR識別子（`ADR-YYYYMMDD[-N]` 形式）を入力に含む（Issue #130） |
