...
| 5 | ブランチが作成された | 初回実装時にブランチを作成（変更要求対応時は既存ブランチを使用） | 実装 |
| 6 | テストが作成された | TDDサイクルでテストコードを先行作成（設計行為） | 実装 |
| 7 | 実装された | プロダクションコードの変更 | 実装 |
| 8 | 検証ゲートを通過した | 型チェック＋Lint＋テスト全パスの複合検証 | 実装 |
| 9 | 検証ゲートが失敗した | 型エラー・Lint違反・テスト失敗のいずれか | 実装 |
| 10 | PRが作成された | PRをGitHub上に作成 | PRレビュー |
| 11 | レビュー依頼がされた | 検証ゲート通過後、レビュアーにレビューを依頼 | PRレビュー |
...
| P2 | 検証失敗時修正 | 検証ゲートが失敗した | タスク消化中に戻る（修正対象は開発者判断） |
