...
| 技術的意思決定が提案された | 技術的意思決定の起案。承認軸 `提案中` への遷移時に発行（Issue #130） | 技術的意思決定 |
| 技術的意思決定が承認された | 実装プラン確定の前提となるアーキテクチャ・設計方針の決定。承認軸 `承認済み`（有効性軸 `有効`）への遷移時に発行（Issue #130 で旧名 `横断的な技術方針が決定された` から改名し、命名規則を集約内で統一） | 技術的意思決定 |
| 技術的意思決定が却下された | 提案が承認に至らず終了した。承認軸 `却下` への遷移時に発行（ADR-202607261459-01 の2軸モデルで承認軸終端として追加） | 技術的意思決定 |
| 技術的意思決定が廃止された | 運用中の意思決定が後継なしで廃止された。有効性軸 `廃止済み` への遷移時に発行（Issue #130） | 技術的意思決定 |
| 技術的意思決定が上書きされた | 運用中の意思決定が後継ADRにより上書きされた。有効性軸 `上書き済み` への遷移時に発行（Issue #130） | 技術的意思決定 |
...
| 技術的意思決定を提案する | 技術的意思決定が提案された | 技術的意思決定集約。発行アクター: 提案者。状態遷移 `[*] → 提案中` の起点コマンド（Issue #130） |
| 技術的意思決定を承認する | 技術的意思決定が承認された | 技術的意思決定集約。発行アクター: 承認者。状態遷移 `提案中 → 承認済み（有効）` の遷移トリガー。Issue #130 で旧コマンド「横断的な技術方針を決定する」を置換し、発火イベントも旧名 `横断的な技術方針が決定された` から改名（破壊的変更） |
| 技術的意思決定を却下する | 技術的意思決定が却下された | 技術的意思決定集約。発行アクター: 承認者。状態遷移 `提案中 → 却下` の遷移トリガー（ADR-202607261459-01 の2軸モデルで承認軸終端として追加） |
| 技術的意思決定を廃止する | 技術的意思決定が廃止された | 技術的意思決定集約。発行アクター: 承認者。状態遷移 `承認済み（有効） → 廃止済み` の遷移トリガー（Issue #130） |
| 技術的意思決定を上書きする | 技術的意思決定が上書きされた | 技術的意思決定集約。発行アクター: 承認者。状態遷移 `承認済み（有効） → 上書き済み` の遷移トリガー。後継ADR識別子を入力に含む（Issue #130） |
...
| 4-b | 承認者 | 人間 | 技術的意思決定を承認する、技術的意思決定を却下する、技術的意思決定を廃止する、技術的意思決定を上書きする |
...
- アクター #4-a「提案者」・#4-b「承認者」は技術的意思決定の役割を抽象化したアクター。組織内の具体的役職（テックリード・アーキテクト等）への対応付けは、導入先の組織の運用ルールで定義する。`#4-a` `#4-b` は既存アクター #4「テックリード / チーム」（Issue #130 で削除）の分割継承を表す表記であり、設計判断「ID付与の方針」が抑止する**新規採番**ではない

## 集約

> 本規約は集約のライフサイクル状態遷移図（`stateDiagram-v2`）に適用する。タスクの内部遷移のような作業プロセス図（`flowchart` 等）は対象外。
>
...
作業プロセス図のため、集約のライフサイクル状態遷移図とは記法を分け `flowchart` で表現する。ノードは工程（行為）を表し、TDDフェーズ名（Red/Green/Refactor）は補足として併記する。

```mermaid
flowchart LR
    Start([タスク開始]) --> Test[テストを書く（Red）]
    Test --> Impl[実装する（Green）]
    Impl --> Refactor[リファクタリングする（Refactor）]
    Refactor --> Done([タスク完了])
```
...
**含むコマンド**: 技術的意思決定を提案する、技術的意思決定を承認する、技術的意思決定を却下する、技術的意思決定を廃止する、技術的意思決定を上書きする

**含むイベント**: 技術的意思決定が提案された、技術的意思決定が承認された、技術的意思決定が却下された、技術的意思決定が廃止された、技術的意思決定が上書きされた

**内部構造**:
- ADR識別子（属性、全状態で保持）: 集約インスタンスを一意に識別する文字列識別子。採番方式の実体は `manage-adr` スキルの `references/adr-model.md`「採番方式」節にあり、ここで再定義しない
- 配置パス（属性、全状態で保持）: `docs/adr/` 配下の Markdown ファイル
- 後継ADR参照（属性、有効性軸 `上書き済み` のとき保持。分割時は複数後継をリスト値で保持）: 後続ADRの識別子を文字列として保持する値オブジェクト。集約インスタンスへの直接参照は持たず、自己参照・循環参照の構造的不整合を防ぐ。front-matter の `superseded-by` に対応し、1本の意思決定が複数後継へ分割される場合はリスト値を取る（ADR-202607261459-01 決定2 ／ ADR-202607261501-01 決定1）

#### 状態遷移

状態は**承認軸**（`提案中`/`承認済み`/`却下`・承認の歴史事実で不変）と**有効性軸**（`有効`/`上書き済み`/`廃止済み`・現在の効力で可変）の2軸で表現する。承認軸が `承認済み` に達した後、有効性軸が内部で遷移する合成状態として1つの状態機械に展開する（`却下` は承認軸の終端）。スキーマ・遷移ルールの生きた実体は `manage-adr` スキルの `references/adr-model.md`（値域・合法な front-matter を表す状態の型をここに集約する）。この2軸モデルの採択は ADR-202607261459-01 決定1。

```mermaid
stateDiagram-v2
    [*] --> 提案中: 技術的意思決定を提案する
    提案中 --> 却下: 技術的意思決定を却下する
    提案中 --> 承認済み: 技術的意思決定を承認する
    却下 --> [*]
    承認済み --> [*]

    state 承認済み {
        [*] --> 有効
        有効 --> 上書き済み: 技術的意思決定を上書きする
        有効 --> 廃止済み: 技術的意思決定を廃止する
        上書き済み --> [*]
        廃止済み --> [*]
    }
```

| 軸 | 状態 | 定義 | 遷移元 | 遷移先 | entry時発行イベント |
|---|---|---|---|---|---|
| 承認軸 | 提案中 | 提案者により起案され、承認者の判断を待っている初期状態 | 初期状態 | 承認済み, 却下 | `技術的意思決定が提案された` |
| 承認軸 | 承認済み | 承認された履歴事実（以後不変）。有効性軸に入り初期サブ状態 `有効` となる | 提案中 | （有効性軸へ） | `技術的意思決定が承認された` |
| 承認軸 | 却下 | 提案が承認に至らず終了した終端。一度も運用されない | 提案中 | 終了 | `技術的意思決定が却下された` |
| 有効性軸 | 有効 | 承認済みかつ現在運用中（承認時に entry） | 承認済み（承認時） | 上書き済み, 廃止済み | （承認時の `技術的意思決定が承認された`） |
| 有効性軸 | 上書き済み | 後継ADRに置き換えられた終端。後継ADR識別子を保持（分割時はリスト値） | 有効 | 終了 | `技術的意思決定が上書きされた` |
| 有効性軸 | 廃止済み | 後継 ADR なしで ADR としての効力を終えた終端。後継参照は持たない | 有効 | 終了 | `技術的意思決定が廃止された` |

- 承認軸の起点は `提案中` 必須。`提案中` をスキップして直接 `承認済み` に到達する経路は持たない。即承認のケースは「提案 → 承認」を運用ルール上の連続実行として表現する。`提案中` からは `承認済み`（承認）と `却下`（却下）の2分岐を持つ
- 承認軸 `承認済み` は不変の履歴事実であり、退役（`上書き済み`/`廃止済み`）は有効性軸で表現する。承認済みに達すると有効性軸の初期サブ状態 `有効` に入る。この2軸分離により「承認された歴史事実」を「今の効力」で上書きしない（ADR-202607261459-01 決定1）
- 実装プラン集約からは直接参照を持たず、ドメインイベント経由で連携する（F7 集約疎結合方針と整合）。実装プラン集約は本集約のイベントを購読しない（参照層の整備は Issue #169 / #171 で扱う）
- `承認済み`（有効）への遷移時に発火するイベントは `技術的意思決定が承認された`。Issue #127 F8 由来の旧名 `横断的な技術方針が決定された` から Issue #130 で改名し、集約内のイベント命名（`技術的意思決定が*`）を統一した
- 承認イベントを契機にしたポリシーは現時点で定義しない（参照層・捕捉プロセスは Issue #169 / #170 / #171 に委譲）
- 採番方式・配置 `docs/adr/`・テンプレート・粒度判定基準などの運用基盤詳細は `manage-adr` スキルの参照ファイル群が実体を持つ（採番方式は `references/adr-model.md`「採番方式」節。運用基盤の確定は Issue #169〔Phase 0〕）。本ドキュメントでは集約モデル上の扱いのみを規定する
...
| H4 | ADR（Architecture Decision Records）のライフサイクル管理が未定義。横断的技術方針の蓄積・参照・廃止の仕組みがない | 技術的意思決定 / 組織横断 | **解消済み（Issue #130、ADR-202607112231-01 で状態モデル再設計）**: 「技術的意思決定」集約として正式モデル化。Issue #130 は英文4状態で確定したが、ADR-202607112231-01 が承認軸（提案中/承認済み/却下）×有効性軸（有効/上書き済み/廃止済み）の2軸へ再設計（同 ADR は #570 で決定クラスタ単位の後継6本へ分割され `上書き済み`。2軸モデルの現行出典は `manage-adr` スキルの `references/adr-model.md`。後継 ADR-202607261459-01 も 2026-07-30 に退役し、当時の採用決定の記録として読む）。ライフサイクルコマンド/イベント・アクター解像度（提案者/承認者）を確定。運用基盤（テンプレート・採番運用・参照導線・粒度判定基準）は #169 / #170 / #171 に委譲 |
...
- **`実装を開始する` 入力の F7 整合（Issue #203）**: コマンド入力を `チケットID AND 開発者` に限定し、`確定プラン`（実装プラン集約の実体スナップショット）を除去する（A案採択）。プラン内容は実装集約内でチケットID経由の都度クエリで取得する。実装開始はプラン確定を前提としない（任意）ため、状態遷移 `[*] → タスク消化中` にガードを置かない。同論点は e9960a1（2026-04-26）で前身コマンド `タスクを開始する` に対し既に A案採択済みであり、Issue #184 / PR #200 の再編時に入力 `確定プラン` が先祖返りしたものの是正にあたる。F7 文言（コマンド入出力で `プランID` 等を持たない）は維持し改訂しない
...
- **横断的技術方針の独立集約化（Issue #127 F8、Issue #130 で詳細設計確定）**: 複数の実装プラン集約を横断・実装プラン集約と運命を共にしない・独立した意思決定プロセス・別アクター（テックリード/チーム）という性質から、実装プラン集約の関心事外として切り出す（Issue #127 F8）。**Issue #130 で詳細設計確定**: 集約名を「技術的意思決定」に正式化。ADR形式の Markdown ファイル（`docs/adr/` 配下、採番方式は当時、同日2件目以降にのみ連番を付与する形式。後に改定され、現行の採番規則は `manage-adr` スキルの `references/adr-model.md`「採番方式」節が定める）を実体として位置付け。ライフサイクル4状態（英文表記。後に ADR-202607112231-01 で承認軸×有効性軸の2軸・和文へ再設計。同 ADR は #570 で決定クラスタ単位の後継6本へ分割され `上書き済み` となり、2軸モデルの現行出典は `manage-adr` スキルの `references/adr-model.md`。後継 ADR-202607261459-01 も 2026-07-30 に退役し、当時の採用決定の記録として読む）と対応するコマンド/イベント（提案/承認/廃止/上書き）を追加。「横断的な技術方針を決定する」コマンドは「技術的意思決定を承認する」に置換（破壊的変更）。承認時のイベントは旧名 `横断的な技術方針が決定された` から `技術的意思決定が承認された` に改名し、集約内イベント命名（`技術的意思決定が*`）の統一を優先（破壊的変更）。運用基盤（テンプレート・採番運用・参照導線・粒度判定基準）は Issue #169 / #170 / #171 に委譲
...
- **技術的意思決定集約の状態名（Issue #130 で英文表記 → ADR-202607112231-01 で和文2軸へ改訂）**: Issue #130 は集約の状態名を ADR 業界慣例（Nygard ADR / MADR）に倣い英文4状態で確定していた（実体の ADR ファイル Status 欄表記との一致・国際的に通じる用語という理由）。**ADR-202607112231-01（#130 を上書き）がこれを承認軸×有効性軸の2軸モデルへ再設計し、状態名を和文ユビキタス言語（提案中/承認済み/却下・有効/上書き済み/廃止済み）に統一した。** 同 ADR は #570 で決定クラスタ単位の後継6本へ分割され `上書き済み` となり、2軸モデルは後継 ADR-202607261459-01 が担ったが、同後継も 2026-07-30 に `廃止済み` へ退役した。以後、2軸モデルの出典は `manage-adr` スキルの `references/adr-model.md` であり、ADR 側は当時の採用決定の記録として読む（本設計判断の見出しは改訂当時の経緯の記録であるため識別子を据え置く）。改訂理由: 単一 Status 欄に承認軸（承認の歴史事実・不変）と有効性軸（現在の効力・可変）が混在し、「承認された歴史事実」を「今の効力」で上書きする構造的欠陥があった（詳細は ADR-202607261459-01 決定1）。ADR ファイルとの表記一致は、front-matter の値を和文ユビキタス言語とすることで維持する（キーは英語の構造的フィールド名、値は和文）
...
| 技術的意思決定 | 横断的な技術方針（アーキテクチャ・設計方針）の意思決定を表すドメイン集約。実体は ADR 形式の Markdown ファイル | Issue #130 で「技術方針（仮称）」から正式名称に確定。ライフサイクルは ADR-202607112231-01 で承認軸（提案中/承認済み/却下）×有効性軸（有効/上書き済み/廃止済み）の2軸に再設計（同 ADR は #570 で後継6本へ分割され `上書き済み` となり、2軸モデルの現行出典は `manage-adr` スキルの `references/adr-model.md`。後継 ADR-202607261459-01 も 2026-07-30 に退役し、当時の採用決定の記録として読む） |
| ADR | Architecture Decision Records。技術的意思決定集約の実体として `docs/adr/` 配下に保管される Markdown ファイル形式 | 採番方式・テンプレート・粒度判定などの運用基盤詳細は `manage-adr` スキルの参照ファイル群が実体を持つ（確定は Issue #169） |
| 提案者 | 技術的意思決定を起案するアクター（ロール抽象化） | 組織内の具体的役職（テックリード・アーキテクト等）への対応付けは、導入先の組織の運用ルールで定義 |
| 承認者 | 技術的意思決定を承認・廃止・上書きするアクター（ロール抽象化） | 組織内の具体的役職への対応付けは、導入先の組織の運用ルールで定義 |
| 上書き済み | 技術的意思決定の有効性軸の状態のひとつ。運用中の意思決定が後継ADRにより置き換えられ、後継ADR識別子（`superseded-by`）を保持する状態 | ADR-202607261459-01 の2軸モデルで確定（旧英文状態名を置換）。承認軸 `承認済み` は不変のまま有効性軸がこの状態へ遷移する。コマンド名は和文「技術的意思決定を上書きする」 |
| 採番方式 | 技術的意思決定集約のインスタンスを一意に識別するADR識別子の採番規則 | 規則の実体は `manage-adr` スキルの `references/adr-model.md`「採番方式」節（確定は Issue #169） |
