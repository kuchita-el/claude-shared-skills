...
#### エスカレーション項目の内部遷移（集約レベルのドメインイベントではない）

エスカレーション項目は実装プラン集約のサブエンティティであり、個別の問い合わせと回答の状態を持つ。これらは作成中状態の派生属性として管理され、集約の状態遷移を引き起こさない（全項目が回答済みになった時点で派生イベント `全エスカレーション項目が解消された` が発行される）。
