...
| 横断的な技術方針が決定された | 実装プラン確定の前提となるアーキテクチャ・設計方針の決定。Accepted 状態への遷移時に発行 | 技術的意思決定 |
...
| 技術的意思決定を承認する | 横断的な技術方針が決定された | 技術的意思決定集約。発行アクター: 承認者。状態遷移 `Proposed → Accepted` の遷移トリガー。Issue #130 で旧コマンド「横断的な技術方針を決定する」を置換（破壊的変更） |
...
**含むイベント**: 技術的意思決定が提案された、横断的な技術方針が決定された、技術的意思決定が廃止された、技術的意思決定が上書きされた
...
| Accepted | 承認者により承認され、運用中の意思決定 | Proposed | Deprecated, Superseded | `横断的な技術方針が決定された` |
...
- 「横断的な技術方針が決定された」イベント名は Issue #127 F8 由来の既存名を維持。Accepted 状態への遷移時に発火する仕様で位置付ける
