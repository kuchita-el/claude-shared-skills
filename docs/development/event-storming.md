...
**含むコマンド**: 実装を完了する、タスクを再開する（いずれもシステム内部コマンドで、P6/P7 が自動発行）

**含むイベント**: 全タスクが完了した、実装が完了した、タスクが再開された
...
stateDiagram-v2
    [*] --> タスク消化中: 実装プランが確定した
    タスク消化中 --> 検証待ち: 全タスクが完了した
    検証待ち --> 完了: 実装が完了した
    検証待ち --> タスク消化中: タスクが再開された
    完了 --> [*]
```

| 状態 | 定義 | 遷移元 | 遷移先 |
|---|---|---|---|
| タスク消化中 | TDDサイクル（Red→Green→Refactor）をタスク単位で繰り返している。全タスク完了時に `全タスクが完了した` イベントを発火し、検証実行集約のライフサイクル開始（P5）を契機する | 初期状態, 検証待ち | 検証待ち |
| 検証待ち | 全タスク消化済み、検証実行集約による検証ゲート結果を待っている状態。検証通過なら P6 経由で `完了` へ、失敗なら P7 経由で `タスク消化中` に戻る | タスク消化中 | 完了, タスク消化中 |
| 完了 | 検証通過。レビュー依頼に進める | 検証待ち | 終了 |
...
| P6 | 実装完了反映 | 検証ゲートを通過した | 実装を完了する | 検証実行集約から実装集約への戻し遷移（成功時、`検証待ち → 完了`）。Sagaポリシー化（Issue #179、設計判断「検証失敗時のポリシー廃止」を上書き） |
...
- P5/P6/P7 は Issue #179 で新規採番。集約境界をまたぐSagaポリシーで、いずれも実行するコマンド欄に当該集約のコマンド名が入る標準形式（QAコンテキストのポリシー表と同じ）。実装集約に発行される P6/P7 のコマンド（`実装を完了する` / `タスクを再開する`）はシステム内部コマンドであり、アクターからの直接発行はない
...
- **検証失敗時のポリシー廃止（Issue #179 で上書き）**: ~~検証ゲート失敗→タスク消化中への遷移は実装集約の状態遷移図で直接表現されており、独立したポリシーとして定義する必要がない。状態遷移図が権威ある定義~~ → Issue #179 で検証実行集約を実装集約から分離した結果、検証ゲート失敗からの戻し遷移は集約をまたぐSaga連携となり、実装集約の状態遷移図のみでは表現できなくなった。P7（検証失敗時タスク再開ポリシー）として再導入する。実装集約に新設した `検証待ち` 状態から `タスク消化中` への戻し遷移を伴うSagaポリシーとし、`タスクを再開する` コマンドを発行する
...
- **検証実行集約の独立化と二視点併存解消（Issue #179）**: ローカル実行（開発者視点・オーケストレーター視点）とCI実行（外部委任視点・待ち状態）が同一コマンド `検証ゲートを実行する` で併存していた構造問題（Issue #173 パターン3）を解消するため、検証実行を実装集約から分離独立集約として扱う方針（E方針）を採用した。
    ...
    - Sagaポリシー化: 集約間連携を P5（検証実行起動）/P6（実装完了反映）/P7（検証失敗時タスク再開）で表現。実装集約は `全タスクが完了した` イベントを発火し、P5 が検証実行集約の `検証ゲートを実行する` コマンドを発行する。検証ゲート結果は P6 / P7 が実装集約のシステム内部コマンド `実装を完了する` / `タスクを再開する` に変換し、実装集約のライフサイクルに反映する
    ...
    - パターン2（自集約イベント→自集約状態遷移）の取り扱い: `タスク消化中 → 検証待ち` 遷移は自集約イベント `全タスクが完了した` で駆動するため、Issue #173 パターン2を実装集約に再導入する形となる。これはライフサイクル区間を状態として正直に表現するための意図的な選択であり、二視点併存（パターン3）を解消する代償として許容する
    ...
    - **P6/P7 分割の根拠**: トリガーイベント（`検証ゲートを通過した` / `検証ゲートが失敗した`）と発行するコマンド（`実装を完了する` / `タスクを再開する`）、駆動する状態遷移方向（`検証待ち → 完了` / `検証待ち → タスク消化中`）がそれぞれ異なるため、別ポリシーIDに分割した。単一ポリシーで成功/失敗を分岐させる記法も選択肢としてあったが、Sagaポリシー表で「1イベント=1ポリシー行」の対応を維持する方が読解しやすいと判断した
    - **mermaid 起点ラベル規約の論点**: 検証実行集約の起点遷移（`[*]` から `検証中` へ）の遷移ラベルにコマンド名 `検証ゲートを実行する` を使用している点について、他集約の起点遷移ラベル（自集約過去形イベント または クロス集約イベント）の慣例と揃っていない。本ドキュメント冒頭の規約「ドメインイベント（イベント一覧に定義されたもの）とガード条件」とも厳密には合致しないが、検証実行集約は P5 がコマンド受信時に生成され、生成時に対応するイベントが存在しないため、本Issueでは現状維持とする。遷移ラベル規約全体（自集約発行イベント / 起因イベント / コマンドのいずれを採るか）の明確化は Issue #184 で扱う
    ...
    - **後続Issue #181（パターン1 集約間依存Saga化）への前提条件**: 集約間Sagaポリシーの先行例として本Issueの記法（P5/P6/P7、実行コマンド欄に標準形式の集約コマンド名を採用）を参照可能。直接の影響はなし（独立軸）
    - **`docs/big-picture.md` 「省略したイベント」節**: 「セルフ動作確認は検証ゲートに統合」の記述は据え置き。「検証ゲート」は仕様名として維持されるため、Big Picture レベルでは表記変更不要
- **失敗時イベント発火記法の取り扱い（Issue #180）**: 関数型ドメインモデル記法ガイド（`skills/domain-modeling/references/domain-model-notation.md`、公開契約）の `失敗時:` フィールドが Either.Left 系失敗とドメインイベント系失敗を区別できない問題を受けて、方針C（複数イベント返却）を採用した。
    ...
    - 新記法サンプル（本Issueの具体例 `検証ゲートが失敗した` の表現）:
      ...
      `失敗時:` は同期失敗なしのため省略。Issue #180 冒頭で課題とされた「`検証ゲートが失敗した` を同コマンドエントリ内で表現する手段がない」状態が解消された
