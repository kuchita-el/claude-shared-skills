...
| 6 | ブランチが作成された | 初回実装時にブランチを作成（変更要求対応時は既存ブランチを使用） | 実装 |
...
| 11 | PRが作成された | PRをGitHub上に作成 | PRレビュー |
| 12 | レビュー依頼がされた | 検証ゲート通過後、レビュアーにレビューを依頼 | PRレビュー |
| 13 | 変更が要求された | レビュアーによるRequest Changes。ブロッキング要素を含む | PRレビュー |
| 14 | レビューが承認された | 個別レビュアーによる承認 | PRレビュー |
| 15 | 必要Approve数に達した | レビュイーが定めた承認基準を満たした | PRレビュー |
...
| 9 | PRを作成する | PRが作成された | |
| 10 | レビュー依頼をする | レビュー依頼がされた | 前提条件: 検証ゲート通過 |
| 11 | 変更を要求する | 変更が要求された | |
...
| # | アクター名 | 種別 | 発行するコマンド |
|---|---|---|---|
| 1 | 開発者 | 人間 | 実装プランを作成する、テストを作成する、プロダクションコードを実装する、ブランチを作成する、PRを作成する、レビュー依頼をする、PRをマージする |
...
- 変更要求を受けた場合は常に新しいインスタンスを作成する（P3ポリシー）

### 実装

**責務**: TDDサイクルによるコード変更の管理

**含むコマンド**: テストを作成する、プロダクションコードを実装する、検証ゲートを実行する、ブランチを作成する

**含むイベント**: テストが作成された、実装された、検証ゲートを通過した、検証ゲートが失敗した、ブランチが作成された
...
| 変更要求対応中 | 変更要求を受けて新しい実装プラン→実装サイクルを実行中（P3ポリシー） | レビュー中 | レビュー中 |
...
| P3 | 変更要求対応 | 変更が要求された | 実装プランを作成する（新規インスタンス） |
...
| H7 | 実装プランのNo-op判定基準が未定義（差し戻し時、どこまでが「軽微」か） | 実装プラン | **解消済み**: 変更要求を受けた場合は常に新しい実装プランを作成する（P3ポリシー）。プランの粒度は開発者判断 |
