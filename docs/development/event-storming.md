...
状態は**承認軸**（`提案中`/`承認済み`/`却下`・承認の歴史事実で不変）と**有効性軸**（`有効`/`上書き済み`/`廃止済み`・現在の効力で可変）の2軸で表現する。承認軸が `承認済み` に達した後、有効性軸が内部で遷移する合成状態として1つの状態機械に展開する（`却下` は承認軸の終端）。スキーマ・遷移ルールの生きた実体は `manage-adr` スキルの `references/adr-model.md`（値域・遷移ごとの必須ルールをここに集約する）。この2軸モデルの採択は ADR-202607261459-01 決定1。
