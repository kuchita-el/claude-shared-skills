...
| 全タスクが完了した | タスク消化中の全タスクが完了。実装集約を `タスク消化中 → 検証待ち` に遷移させ、P5（検証実行起動ポリシー）のトリガーとなる | 実装 |
...
- **検証実行集約の独立化と二視点併存解消（Issue #179）**: ローカル実行（開発者視点・オーケストレーター視点）とCI実行（外部委任視点・待ち状態）が同一コマンド `検証ゲートを実行する` で併存していた構造問題（Issue #173 パターン3）を解消するため、検証実行を実装集約から分離独立集約として扱う方針（E方針）を採用した。
    ...
    - **mermaid 起点ラベル規約の論点**: 検証実行集約の起点遷移（`[*]` から `検証中` へ）の遷移ラベルにコマンド名 `検証ゲートを実行する` を使用している点について、他集約の起点遷移ラベル（自集約過去形イベント または クロス集約イベント）の慣例と揃っていない。本ドキュメント冒頭の規約「ドメインイベント（イベント一覧に定義されたもの）とガード条件」とも厳密には合致しないが、検証実行集約は P5 がコマンド受信時に生成され、生成時に対応するイベントが存在しないため、本Issueでは現状維持とする。遷移ラベル規約全体（自集約発行イベント / 起因イベント / コマンドのいずれを採るか）の明確化は別Issueで扱う
