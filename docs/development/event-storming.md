...
| タスクを完了する | （個別タスクの完了。最後のタスク完了時のみ派生イベント `全タスクが完了した` を発火）| サブエンティティ「タスク」のコマンド。開発者が個別タスクの完了を宣言する。コマンド処理結果として当該タスクを完了済みに遷移させ、全タスク完了時には集約状態 `タスク消化中 → 検証待ち` の遷移と派生イベント `全タスクが完了した` の発火を伴う |
...
>
> 中間遷移には、当該遷移を駆動するコマンド（自集約コマンドまたはサブエンティティコマンド）とガード条件を記載する。同一コマンドの処理過程で結果に応じて分岐する場合（例: 検証実行集約の `検証中 → 完了` 結果分岐）、起点遷移にコマンドが記載済みであるため中間遷移はガード条件のみで結果分岐を表現してよい。
>
