...
- **自動テスト**: `scripts/tests/lint-domain-doc.bats` で fixture（`scripts/fixtures/lint-domain-doc/{valid,invalid}/*.md`）に対する exit code とメッセージ部分一致を検査する。実行は `bash scripts/run-tests.sh`（commit ゲート経由でも起動する。手動実行の範囲は `docs/development/test-execution.md`）。GitHub Actions 等の CI 連携は未導入。
...
- **ドキュメント間相互参照の欠落を機械的に不整合と判定しない**: 参照先に対応する記述が無いことをもって不整合とする前に、2段のチェックを踏む。(1) **リンクの意味を識別する** — 構造的参照（「設計更新の結果として X が解消された」という結果参照。参照先の個別記述の存在は要求しない）か、個別記述参照（「対応する記述が存在する」という内容参照）か。(2) **記述欠落の理由を判定する** — DDD コンテキストの責務範囲外による正常な省略（記述がないことが境界の正しさの証拠）か、実際の記述漏れか。由来は PR #124（2026-04-15 マージ）のレビューで、Big Picture ホットスポット #7 の解消アクションが参照する QA ドキュメントに「差し戻し判断」の記述がないことを Critical と誤判定した事例である。QA は「差し戻し判断」を持たない設計であり、記述の不在は整合の証拠だった。この2段判定を自動 lint で完全に行うのは困難であり、本リントの検査範囲を超える。運用側では、レビュー観点プロンプトに「参照＝個別記述対応」の前提を埋め込まないことで担う。レビュー観点が単純な機械的判定より複雑になり、レビュアーに DDD コンテキスト境界の理解を要求する点は、この判断が受容した犠牲である。
