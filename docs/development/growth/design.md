...
- この文書の位置づけ: growth の設計判断の根拠を持つ**ストック**——現在真であり続ける義務を負う文書。確定した判断はここに集約するが、他文書の上位に立つ権威としてではなく、ADR や配布物側の spec が改訂すれば追随して正しくなる側として持つ。実装詳細に近い記述は、詳細度の高い文書（配布物側の各 spec・各 SKILL.md）へ抽象度に応じて委譲する。集約した判断のうち横断的に再発したものだけを `docs/adr/` へ昇格させる（ADR 運用ルール「迷ったら書かない／まず判断を書き、再発で昇格」に従う）。
- 進捗の出典（単一出典）: 実装進捗・現在地は **GitHub Issue / PR**（Phase 1 = #343–#348、Phase 2–6 エピック = #349–#353）を参照する。本ドキュメントが持つのは設計判断の根拠であり、進捗トラッカーではない——§5 ロードマップは Phase 構成（設計属性）のみを保持し、進捗状態は持たない。チャット文脈・「次の一手」は memory `project_growth_plugin` に置く。役割分担の詳細は §6 決定事項9。
...
5. **忘却・圧縮を検証と同格に置く**: 足場の肥大は注意を希薄化させ、ある点を越えると性能を下げる。学びを足場に焼くだけでなく、具体ルールを上位原理・強キャリアへ畳む能動的な**畳み込み**を行う。良い内省システムは足場を太らせず、痩せさせる。（痩せさせる操作の総称＝**整理**〔効かない・害ある学びをエントリごと物理除去する**忘却**＋テキスト規範を上位原理・強キャリアへ昇華して除去する**畳み込み**の2形〕の詳細は [`learning-store-spec.md`](../../../plugins/growth/references/learning-store-spec.md)「整理」。）
...
上図がドメインモデルの正典（ユビキタス言語）。知識の状態・ドメイン動作・ゲートを domain 語で一箇所に固定する。各段を実装する担い手（スキルと成果物）は §4 と配布物側の各 spec・各 SKILL.md が持ち、本節では再述しない。学習シグナルの価値は単一でなく2軸——**摩擦知**（環境摩擦・手続違反）と**判断知**（選好・却下理由・設計判断）——で、型は観測記録・仮説を貫く **`type` 属性**として運ばれる（fork でなく同一流路・共有ライフサイクル。ADR-202607010734-01）。1観測が両型にまたがる混在ゾーンの partition 硬度は非排他 tag（多値 set）で確定し、`candidates.md` の `type`（単値）→ `tags`（多値 set）改称を設計決定として記録した（決定事項10・#440）。
...
- **measure / retire**: 観測値は配布ファイルに持たせず、各コンシューマのローカル使用台帳→集約ダイジェスト（fan-in）に置く。撤回は origin 側 learnings.md からの物理除去。発火観測・使用台帳の事後解析再構成は決定事項7／[`learning-store-spec.md`](../../../plugins/growth/references/learning-store-spec.md)。
- **用語（本節が保守義務を負う範囲）**: 正典フレーム（`観測 → 仮説形成 → 仮説検証`）を #448 で growth 全ドキュメントへ全面反映した。旧名対応: `観測記録`（旧 観察）・`制度化`（旧 取り込み）・起点 `学習の契機`。`摩擦知`/`判断知`/`type`/`behavior-diff`/`decision-record` 等 #432 の語は維持。distill 出力単位名は ADR-202607051220-01 §1 に従い『仮説』へ移行済み（旧名対応は同 ADR §1 を SoT とする。本節の語彙追随は当初 #438 が担う予定だったが、未適用のまま close したため #453 が引き取り本掃引で反映した）。学習語彙での用語見直し（`知` の位置・procedural/declarative 等）・`キャリア`/`スコープ` の日本語化は follow-up の「ユビキタス言語移行」epic（#409）へ。実装識別子（`captures.md`・`candidates.md`・`candidate-status`・各スキル名）は据え置き。**用語集（旧 glossary.md）は #409 で廃止した**——各語の定義は上記の各設計節へ集約した（日本語正典名と英語別名の分離規約は ADR-202607051220-01 §0 が保持）。本書は他文書の上位に立つ権威としてではなく、**ストックとして現在真であり続ける保守義務**を負う文書として用語を持つ——ADR が原理や語彙規約を改訂すれば、本書が追随して正しくなる側である。実装詳細に近い語は、詳細度の高い文書（配布物側の各 spec・各 SKILL.md）へ抽象度に応じて委譲する。glossary が担っていた drift-guard（避ける語台帳）は手動再建せず、各語の定義の置き場＋意味レビュー＋再発 denylist の follow-up 機構へ委ねる（中間期は drift-guard 空白を許容。将来 DSL 化で用語 drift を parse error 化する対象は、設計書ではなく配布物側の動作語彙とする——drift が実際に起きたのは動作語彙であり、設計語彙〔原理1〜7〕では起きていないため）。
...
この「種別」軸（改善還元 / 汎用ルール）は、学び置き場内の**共有境界軸**（パブリック / 閉じた ＝ universal / project-local。後述「母集団 × 適用範囲マトリクス」）とは直交する。本表下段「汎用ルール → growth 学び置き場」を、[`learning-store-spec.md`](../../../plugins/growth/references/learning-store-spec.md) が 2空間（共有境界）へ細分して具体化する（#344）。

上の2系統を配布媒体の粒度へ具体化すると、学びを載せる先は4種の**キャリア（配布先媒体）**に分かれる。キャリアは共有境界軸（空間軸 ＝ universal / project-local）とは直交する独立軸である（**キャリア軸 ⊥ 空間軸**）。

| キャリア | 配布先媒体 | 性質 |
|---|---|---|
| 強キャリア | hook / lint / test / 型 / CI・skill / CLAUDE.md | テキスト規範より強い構造的キャリア。うち hook / lint / test / 型 / CI は**機構強制**（モデルの裁量外で機械的に強制・検出する「二度と起こせない構造」。原理4）、skill / CLAUDE.md は**モデル媒介**（構造化された指示だが遵守は確率的）。learnings.md には載らず、テキスト規範から除去される（畳み込みの移送先）。値域と機構強制／モデル媒介の区別は [`career-spec.md`](../../../plugins/growth/references/career-spec.md)「強キャリアの内訳」が単一出典であり、本セルはその再掲である（#657） |
...
- **個人 store のエントリ状態（実装層の正典モデル）**: 状態軸は候補側 `candidate-status` の1本に集約する。`captures.md` は無状態の append-only 観測コーパスであり、エントリ単位の状態フィールドを持たない（ADR-202607111014-01）。`captures.md` と `candidates.md` は `provenance`（由来＝`captures.md` の `## <timestamp>` 見出し）で結ばれるが、これは同一性追跡のためであり状態同期のためではない。distill が処理すべき観測の選定（処理源選択）は、provenance 導出（`promoted`/`pending` 候補を持つ観測の除外）と distill 専用の処理済みカーソル（`distill-state.md` の `distill-cursor`）の合成で有界化する（ADR-202607111225-01〔ADR-202607121331-01 が restate〕）。promote（仮説検証）が Issue 起票に成功すると `candidate-status` のみを前進させ、`captures.md` は書き換えない。値のスキーマ詳細（`candidate-status` の値域・`provenance` の値・カーソルの格納と前進規約）と状態遷移は [`personal-store-spec.md`](../../../plugins/growth/references/personal-store-spec.md) が SoT（#345、ADR-202607111014-01 / ADR-202607111225-01〔ADR-202607121331-01 が restate〕）。
...
この二段ゲートは新規概念ではなく、既存の自律度モデル L0–L3（ADR-202606012328-01 / ADR-202606020032-01）の承認ゲート軸で表現する。ナレッジ保存＝L3（AI 自律・承認段が縮退）、仕組み化＝L2（提案→承認の二段、Proposed→Accepted と同型）。Phase 4 のマルチエージェントレビュー化は、ADR-202606020032-01 が述べる「検証能力の成熟に伴う L2→L3 移動」の具体例にあたる。
...
配布物（学び置き場）の**2空間**——パブリック/グローバル（全世界 × 全PJ ＝ public plugin セル）と閉じた空間（チーム/プロジェクト ＝ チーム行のセル）——は、本マトリクスのうち**コミットされた配布物が成立するセル**を Route 粒度で畳んだもの。個人行は共有のデッドエンドであり学び置き場では扱わない（既存 memory 機構が担う）。詳細は [`learning-store-spec.md`](../../../plugins/growth/references/learning-store-spec.md)（#344）。
...
1. **エンジン**: 学習ループの各段を担うスキル群。1段＝1スキルで関心を分離し、段ごとに別タイミングで起動する。
   ...
   - `intake` スキル（制度化＝二段ゲート L2・Phase 2）: 複数の仮説 Issue を1つの取り込み Issue へ束ね、人間が昇格先キャリアを裁定する集約点を運用する。承認後は各仮説を取り込み時クローズして inbox から外し、取り込み Issue 自体は通常の単一 Issue として既存ワークフロー（refine-issue / DoR / PR レビュー）に乗る。取り込み Issue の構造・裁定結果の記録形式・集約トポロジの不変条件は [`intake-issue-spec.md`](../../../plugins/growth/references/intake-issue-spec.md)（ADR-202606282107-01）。
   - 残る Distribute（検証済みの学びの `learnings.md` への物理昇格）は後続 Phase（Phase 2）。検証を経た仮説は Issue 起票（小さければ PR）として既存ワークフローに渡す。昇格 Issue のテンプレートは [`promotion-issue-spec.md`](../../../plugins/growth/references/promotion-issue-spec.md)（#382）で定義する。昇格先キャリアは distill が仮説単位で生成し、確定（裁定）は上記 `intake` の集約点が担う（career 決定モデルは ADR-202606282107-01。#382 は本決定に合わせ再定義予定）。出来事ベースの昇格 Issue を `learnings.md` の1欄エントリへ翻訳する変換規約は [`learning-promotion-spec.md`](../../../plugins/growth/references/learning-promotion-spec.md)（#383）で定義する。
...
| Phase | 内容 | 何が増えるか |
|---|---|---|
| 0 | 本設計ドキュメント。原理・スキーマ・境界モデルの固定 | 共通言語 |
| 1 | エンジン最小: `capture` 明示起動（拾う → 検証 → Issue 起票 / 小さければ PR）。痕跡は現セッションの会話履歴。生の観測は個人ローカル、検証後に committed。学び置き場は単一可読ファイル | 学びの観測と共有開始 |
| 2 | 手動ゲートでの昇格: Route + 承認で配布物へ | ナレッジ → 行動変化 |
| 3 | 自発トリガー（機構は実装時決定）＋セッションログ横断解析＋git revert 取得 | 自発性と客観痕跡 |
| 4 | マルチエージェントレビュー化: 承認ボトルネックを置換 / 補強 | 人手なしで越境スケール |
| 5 | チーム / 組織配布: team private marketplace を Route 対象に追加 | 共有範囲の拡大 |
| 6 | メタ学習: 内省の仕組み自体（検知基準・昇格閾値）を内省対象に。仕組み変更は人間ゲート必須 | 自己改修 |

> **Phase と版**: Phase は進行の単位であり、配布上の版とは対応しない。プラグインの版はコミット SHA が担い、`plugin.json` は `version` フィールドを持たない（ADR-202609061416-01）。
...
1. **学び置き場の形式**: 単一の人間可読ファイル。全体を一覧でき、肥大が見えるため原理5（足場を痩せさせる）に資する。内容モデル（2面・fan-out/fan-in 分離・配布の2空間・1欄スキーマ・忘却＝物理除去）・スキーマ・配置（パブリック空間の確定パス `plugins/growth/learnings.md`）・ライフサイクルは #344 で確定（[`learning-store-spec.md`](../../../plugins/growth/references/learning-store-spec.md)）。
...
3. **個人 store の置き場**: 生の観測は個人ローカル（共有されない）に置き、検証を経たものだけ committed へ昇格させる。昇格経路を必ず持つことで memory の「個人で終わる」欠点を克服する。具体形式・置き場（`~/.claude/projects/<project-id>/growth/captures-YYYY-MM-DD.md`〔日付バケットのセグメント〕、per-project）・状態管理・有界保持（retention）は #345 / #485 で確定（[`personal-store-spec.md`](../../../plugins/growth/references/personal-store-spec.md)）。
...
7. **活性化モデルと発火観測**: #380（Phase 3 の子チケット）で確定。
   ...
   - **エントリスキーマへの波及**は [`learning-store-spec.md`](../../../plugins/growth/references/learning-store-spec.md) に記録。専用トリガ欄は不要（既存の領域・状況の選択軸で本文選択を代替し簡潔さを維持）、安定 ID は使用台帳の参照キーとして Phase 3 で必須化する。
...
9. **バージョニング規約と進捗の単一出典**:
   - **バージョニング規約は廃止した**（**supersede 注記**: 本項は元は「**Phase = minor**。`plugin.json` の version は Phase 完了ごとに minor を上げる〔Phase N 完了 → `0.N.0`〕。Phase 内のバグ修正・スキル調整・配布物フォーマットの破壊的変更は patch。ループが Capture→Retire まで一巡し実運用で安定したら 1.0.0。version の正は `plugin.json`」としていたが、ADR-202609061416-01 が全プラグインの manifest から `version` フィールドを削除し、繰り上げ幅の意味論と 1.0 到達条件を定義しないと決めたため、規約の対象が消滅した。growth の版もコミット SHA が担う。Phase は進行の単位としてのみ残り、版とは対応しない）。
   - **進捗の単一出典＝GitHub Issue / PR**。実装進捗の正は Issue / PR（Phase 1 = #343–#348、Phase 2–6 エピック = #349–#353）に置く。**本書は進捗を一切保持しない**——§5 ロードマップは Phase 構成（設計属性）のみを持ち、進捗状態の列を持たない。§ステータスも現在地を書かず Issue / PR を参照する。チャット文脈・「次の一手」は memory `project_growth_plugin` が担う。三者の役割を分離し、従来 §ステータス・§5「状態」列・memory の 3 箇所に分散していた進捗記述を SoT 一本化する。本書側に写しを持たないため、Phase 遷移時の更新漏れによる乖離（origin/main で実際に「実装は未着手」のまま放置されていた事象）が構造的に起きない。
   - **ADR 化は不要**。進捗の単一出典は運用ルールであり後戻りコストは限定的。本決定事項＋§5 の記述で足り、ADR 化候補として追跡しない（ADR 運用ルールの粒度判定 3 点未満）。この判定はバージョニング規約にも掛けていたが、同規約は前項のとおり ADR-202609061416-01 が全プラグイン横断で覆したため、本項の射程は進捗の単一出典だけになった。

2026-07-05 に混在ゾーン（1観測が両知識型にまたがる領域）の partition 硬度を #440 で確定した。

10. **混在ゾーンの partition 硬度＝非排他 tag**（#440。ADR-202607051220-01 decision 2 が委譲した未決事項を確定）:

   本決定事項は自セクションを ADR-202607051220-01 の現行正典語彙（観測＝capture / 仮説形成＝distill / 仮説検証＝promote / 仮説 / スコープ・キャリア）で記述する。他セクション（§3 図表・他決定事項）の語彙追随は #438 が担い、本決定では行わない。英語実装識別子（`candidates.md`・`behavior-diff`・`decision-record`・`type`/`tags`）は据え置く（ADR-202607051220-01 原則0）。

   - **代表パターン（混在ゾーンの実在）**: 混在ゾーン＝1観測が `behavior-diff`（摩擦知）と `decision-record`（判断知）の両知識型にまたがる領域。知識型は #439 の直交性（検出方法 HOW ⊥ 価値族。検出器が拾った信号の値の帰属は 1:1 でない＝ADR-202607051220-01 decision 2 注記）の帰結として構造的に不可避に生じる。signal 値域（判断群: 選好 / 却下理由 / 目標表明 / 設計判断、摩擦群: 訂正 / ツール拒否 / 反復試行 / 期待違反 / 客観痕跡）に接地した代表パターンを2型に分けて示す。
     - **時間解決型（初回は判断知、再発で摩擦知が顕在化）**: user が設計判断を却下理由つきの訂正として述べる観測（例:「プランは追跡対象にしない——追跡可否は利用者に委ねられている」）。初回は教示信号検出器（価値語では旧「判断知検出子」。ADR-202607051220-01 検出器名同定注記の HOW 軸命名を正典とする）が却下理由（判断群 signal）として拾い、復元不能な設計境界＝`decision-record` として読める。同一トリガー（プランを追跡対象にしようとする振る舞い）が後続セッションで再発すると、予測誤差検出器（旧「摩擦検出器」）が訂正の反復（摩擦群 signal の N 再発）として拾い、「配布した規範が効いていない」摩擦知＝`behavior-diff` の価値軸（再発）も帯びる。同一の根観測が時間を経て両検出器・両価値軸で読める。
   ...
   - **partition 硬度＝非排他 tag（J1 確定）**: partition 硬度を**非排他 tag（多値 set）**で確定する。選択肢 A（単一 type へ倒す＝排他）/ B（非排他 tag）/ C（provenance 共有の2仮説へ分割）のうち **B** を採る。
     - **根拠**: 知識型は摩擦知／判断知の2直交属性（#439）であり、2直交属性の忠実な表現は集合（tag）であって enum（type）ではない。enum は2次元を1次元へ落とす非可逆エンコードで、混在ゾーン（直交の帰結＝構造的に不可避）に平板化または N 再発トラッキング喪失の**二重損失**（ADR-202607010734-01 が是正した偽陰性・平板化の再来）を再導入する。A はこの二重損失を招く。C は `candidates.md` の provenance キー一意性（upsert 冪等の前提）を崩す。
   ...
   - **仮説形成（distill）の evidence-gated 分岐（確定）**: 観測段の型仮説（#437「観測段の型は仮説」）を仮説形成（distill）が **evidence-gated** で確定する。既定は単一タグ（最尤の1知識型）、第2タグは**陽性証拠がある時のみ**付与し、**既定 both（無条件の両タグ付与）を禁ずる**。
     ...
     - ADR-202607010734-01 §4「distill 出力2系統分離」は**不変**: 系統数は2のまま、両タグ仮説が両系統に入る＝系統メンバーシップが関数（1仮説→1系統）から集合関係（1仮説→1〜2系統）になるのみ。
   ...
   - **整合確認（AC4）**: 本決定（partition 硬度＝非排他 tag）が上流前提・既存 ADR と整合することを各該当節参照つきで確認した。
     ...
     - **(2) #439 直交性・#441 裁定基準・ADR-202607051220-01 decision 2**: `type` は tag の要素数1特殊ケースで tag が type を包含し、確信ケースを要素数1で表せる。decision 2 が委譲した partition 硬度（「不十分なら tag」の tag 寄り示唆）を tag 確定で充足する。
     - **(3) ADR-202607010734-01 §1（価値2軸）/§4（出力2系統分離）**: tag 採用が二重損失（平板化 or N 再発喪失）を回避し、§4 の系統数2が不変（両タグ仮説が両系統に入り、系統メンバーシップが関数から集合関係になるのみ）。
...
本節が持っていた候補表は**配布元へ移設した**（#652）。切り分けの決め手は、配布物と配布元の境界規約が置く判断軸のうち「配布先の環境で解決できるか」である。本プラグインから配布元へ張られていた相対リンクは3件で、そのすべてが本節の表の中にあった。移設先は本リポジトリの開発ドキュメント配下にあり、配布物からはパスで指さない（参照方向を配布元 → 配布物の一方向に保つ）。

もう一方の軸（配布元固有のデータを含むか）は本節と他節を区別しない。本書は本節の外でも本リポジトリの Issue 番号を名指しており、同軸だけでは線が引けない。**設計文書そのものを配布物へ置くかどうかは #652 の移設では未決のまま残った**。この論点は #657 が実行不能性テストという判断軸を足して決着させ、本書自身を配布元へ移した（後掲「設計分割の決定（#657）」の問い1(a)・問い5）。

節番号は据え置く。本節を名指す既存の記録（ADR-202606261847-01 / ADR-202607010734-01 の Context）は当時の所在を述べたものであり、遡及して改めない。

本節が併せて定めていた昇格の契機のうち「実装 PR 時の昇格」も移設先へ移った。本書冒頭「この文書の位置づけ」が持つのは**再発による昇格**（横断的に再発したものだけを昇格させる）であり、実装 PR 時の契機を含まない。これは growth の設計判断を本リポジトリの ADR へ昇格させる手続きであって、本プラグインを導入した環境での動作には関与しない。
...
| 配布元 `docs/development/growth/` に設計文書 | `plugins/growth/` 同梱（配布物に設計文書を含める） | 配布物のみを導入した利用者から解決できない参照を配布物へ残さない。設計文書は配布物に含めず、配布物が持つのは学習ループを回すスキルとその動作契約、および配布される学びの実体に限る（#657）。同梱は当初「複数プラグイン文書の混在を避け、設計とコードをコロケーションできる」「設計文書は自動ロードされないため配布の害がない」を理由に採用していたが、#657 で覆した。覆せた決め手は**実行不能性テスト（配布境界の第3軸）を足したこと**である。#652 の時点で配布物から配布元への相対リンクは既に 0 件であり、参照の解決性を問う第1軸だけでは同梱を却下できなかった。第3軸で動作契約と根拠の線を引き、動作の判定に要る定義を配布物側へ引き込んだ結果、残った本書は根拠のみからなり、配布物に留まる理由を失った（本書は §7 の外でも本リポジトリの Issue 番号を名指すため、配布元固有のデータを含むかを問う第2軸にも掛かる）。当初の理由が誤っていたのは、配布の害を**自動ロードの有無で測った**点にある。なお配布元固有の管理表〔旧 §7 の候補表〕は第1軸により先行して #652 で配布元へ移している（当時は §7 の表が配布元への相対リンク 3 件をすべて抱えていた） |
...
本節は**時点固定の記録**である。他節が現在真であり続ける義務を負うストックであるのに対し、本節は上記の決定日に採った方向と、その時点の実測に基づく理由を固定して残す。以後の設計変更で本節の記述が現状と食い違っても、記述の誤りではない。

本書（旧 `plugins/growth/DESIGN.md`）を動作契約と根拠に分割し、配布物から設計文書を無くす作業について、#657 が立てた調査の問い1〜6 に採った方向と理由を記録する。

**問い1(a) 動作契約と根拠を分ける軸**

- 方向: **実行不能性テスト**を採る。その定義を持たずに読むと手順のどの行かが評価不能になるものだけを動作契約として配布物側の spec へ置き、それ以外（原理1〜7・二段ゲート・学習ループ・決定事項の各項）は根拠として本書に残す。軸は growth 限定ではなく repo 横断の判断軸として `docs/distribution-boundary.md` §2 へ置く。条文の形が固有語を含まず一般であり、growth 固有なのは適用結果の側だけだからである。
- 理由: 原理を**動作の判定根拠として名指す全箇所で**同一行に内容を再掲していた。したがって名指しを落としても動作定義は残る。表の「対応原理」列のような**由来注記は動作の判定に効かないため、本テストの射程外**である（射程を動作の判定へ限って初めて「定義が要るかどうか」で線が引ける）。対して強キャリアだけが定義を配布物の外へ委譲しており、移設を要する概念が1件に絞れた。
- 本決定（決定A）は ADR 級と判定した。却下した代替（被参照テスト）とその却下理由・帰結を含む詳述は ADR が Decision として持つため、本節は方向の記録に留める。ADR は本 Issue 内で別途起票する。
...
- 方向: 配布物 `plugins/growth/references/` にキャリア軸の spec を1本新設し、強キャリアの値域と機構強制／モデル媒介の区別をそこへ単一出典化する。残る2概念（母集団＝共有境界、キャリア軸 ⊥ 空間軸）は spec 側に実体定義があり、引用の内在化と誤引用の除去で足りる。
- 理由: 強キャリアの値域は実測で3通りに分裂していた（本書 §3・`career-promotion-spec.md`・`learning-store-spec.md` の 3 件で列挙が食い違う）。強キャリアは複数の spec と distill / intake の2スキルが使うため、#409 が採った「定義は用語集ではなく使う場所へ置く」では「使う場所」が一意に定まらない。単一出典を立てなければ名指しを除去しても分裂は続く。
- 新設が #409 で廃止した用語集の再建に当たらない理由: 廃止したものが全用語の台帳＋避ける語台帳であるのに対し、新設するものはキャリア軸という1つの動作契約の spec であり、既存の `*-spec.md` 群と同格である。

**問い3 用語正典の宣言**

- 方向: 「本設計書が用語の唯一の正典」という宣言を、**ストックとしての保守義務**（本書は現在真であり続ける義務を負い、ADR が原理や語彙規約を改訂すれば追随して正しくなる側である）と**抽象度による委譲**（実装詳細に近い語は詳細度の高い文書へ任せる）へ置き換える。権威の上下関係ではなく保守義務の形で書く。
- 理由: 「唯一の正典」は既に事実と乖離していた。ADR-202607010734-01 §6 が原理3 を精緻化しており、ADR が原理の定義を改訂した先例が既にある。また日本語正典名と英語別名の分離規約は ADR-202607051220-01 §0 が保持しており、語彙規約は委譲済みである。正典は設計文書・ADR 群・spec 群の3箇所へ既に分散しており、問い1・問い2 の決定はその分散を追認して線を引き直すものである。
- 帰結: 「実装詳細に近い語は詳細度の高い文書へ」を本書自身の重複記述にも適用し、spec 側に単一出典がありながら本書が再述していた箇所を本 Issue 内で削った。内容を変えず重複を単一出典へ委譲する操作であり、再述が無ければ乖離しようがないという点で「ストックとして正しい」の実現手段にあたる。
- **ADR 非該当と判定した（決定B）**: 宣言を反転しても矛盾するのは宣言文1〜2箇所にとどまり、規範の適用先も配布元の設計文書1つで、粒度判定が境界に届かない。ADR を持たない以上、却下した代替（「唯一の正典」の宣言を維持する案）の却下理由は本書が自ら持つ必要がある。上記「理由」の2点——ADR-202607010734-01 §6 が原理3 を精緻化した先例があること、および語彙規約が ADR-202607051220-01 §0 へ委譲済みであること——がそれにあたる。

**問い4 `plugins/growth/README.md` の構成**

- 方向: `plugins/adr/README.md` の体裁（H1 と目的の要約1段落 → `## 提供物` → 境界の説明 → `## 導入`）に準拠する。`## 提供物` にはスキル群と `learnings.md` を挙げ、`references/` の spec 群は列挙しない。設定節は置かない。配布元へのリンクも置かない。
- 理由: spec への導線を置かないのは、到達性を実行経路が担い README は担わないためである。設定節を置かないのは、growth に利用者が設定する項目（環境変数・override）が存在せず、`plugins/adr/README.md` の「ADR ディレクトリの指定」に相当するものが無いためである。配布元へのリンクを置かないのは、配布物内から配布元を指すと参照方向を文書側で破るためである。
- 境界の説明を置く理由は2つある。揃え先の `plugins/adr/README.md` が同型の記述（テストランナーと fixture を含めない旨）を持つこと、および本 Issue の成果を配布物側で表明しておくことが、将来「設計文書を配布物へ戻す」判断に対する抑止になることである。

**問い5 分割後の設計文書の配布元での配置**

- 方向: `docs/development/growth/` 直下に階層を作らず集約し、`README.md` を入口として置く。導線は `docs/distribution-boundary.md` の既存の名指しを `README.md` へ張り替えることで繋ぐ。
- 理由: `docs/` には index が無く、導線は関連文書からの名指しで成立している。`docs/development/` のサブディレクトリの慣習は、複数ファイルが集まるものが `README.md` を持ち、1ファイルのみのものは持たない、というものであり、分割後の growth は入口を置く側に当たる。
- ストック（本書等）と時点固定（spike の一次調査記録）の性質差は、ディレクトリ階層ではなく `README.md` の記述で表す。この規模で階層を切ると導線が2段になり、配布元の導線から辿れることの検証が煩雑になるためである。

**問い6 将来の DSL 化との整合**

- 方向: 整合する。ただし DSL 化で用語 drift を parse error 化する対象を「設計書」から「配布物側の動作語彙」へ改める。DSL 化の実装は本 Issue のスコープ外であり、本項は方向の整合確認と方向記述の書き換えに留める。
- 理由: drift が実際に起きたのは動作語彙である（強キャリアの値域が3通りに分裂した実測。問い2 を参照）。設計語彙（原理1〜7）では起きていない。加えて分割後は原理が配布物から名指されなくなるため、原理が変動しても配布物の動作へ波及しない。DSL 化の効き所が設計書から spec 側へ移る。
- 問い2 で決めた「1概念1ファイル」（キャリア軸 spec の新設）は parse 単位として素直であり、この方向と整合する。
