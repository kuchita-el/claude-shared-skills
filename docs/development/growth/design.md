...
9. **進捗の単一出典**:
   ...
   - **ADR 化は不要**。進捗の単一出典は運用ルールであり後戻りコストは限定的。本決定事項＋§5 の記述で足り、ADR 化候補として追跡しない（ADR 運用ルールの粒度判定 3 点未満）。
