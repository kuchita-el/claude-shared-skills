...
| 配布元 `docs/development/growth/` に設計文書 | `plugins/growth/` 同梱（配布物に設計文書を含める） | 配布物のみを導入した利用者から解決できない参照を配布物へ残さない。設計文書は配布物に含めず、配布物が持つのは学習ループを回すスキルとその動作契約、および配布される学びの実体に限る（#657）。同梱は当初「複数プラグイン文書の混在を避け、設計とコードをコロケーションできる」「設計文書は自動ロードされないため配布の害がない」を理由に採用していたが、#657 で覆した——配布の害は自動ロードの有無ではなく参照の解決性で測る。同じ判断軸により、配布元固有の管理表〔旧 §7 の候補表〕は先行して #652 で配布元へ移している |
...
- 理由: 判定根拠は**配布物内の被参照の濃度**である。`career-promotion-spec.md` と `learning-promotion-spec.md` は #384 / #383 の対をなす移送規約で、実行主体もともに既存ワークフローである。それでも扱いを分けたのは濃度が実測で非対称だったためである。前者は配布物内の被参照ゼロで、スキルからの参照も fallback 挙動を行内で述べきった出典 citation にすぎない。後者は残留する spec すべてと `learnings.md` から参照され、配布元へ移すと配布物→配布元の逆向き参照を複数作って参照方向の一方向性に抵触する。
...
- 理由: 強キャリアの値域は実測で3通りに分裂していた（本書 §3 と `career-promotion-spec.md`・`learning-store-spec.md`・`personal-store-spec.md` で列挙が食い違う）。強キャリアは複数の spec と distill / intake の2スキルが使うため、#409 が採った「定義は用語集ではなく使う場所へ置く」では「使う場所」が一意に定まらない。単一出典を立てなければ名指しを除去しても分裂は続く。
