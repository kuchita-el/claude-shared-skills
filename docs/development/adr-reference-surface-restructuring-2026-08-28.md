...
行の欠落・追加が無いことを先に示す。再構成前の 208 行は、`1-136`（判定コア）／`137-158`（判定結果の行き先）／`160-167`（束ねの制約）／`169-208`（命名規約）と 2 本の区切りの空行に過不足なく分解でき、それぞれ再構成後の `adr-scoping.md`（判定コア）／`adr-destination.md`（行き先＋命名規約）／`adr-splitting.md`（束ね）へ移した。`diff --ignore-blank-lines` で突き合わせた結果、**変更された行は次の5行だけ**であり、いずれも移動に伴う参照表現の書き換えである。削除された行は0件、条文の追加も0件である。
