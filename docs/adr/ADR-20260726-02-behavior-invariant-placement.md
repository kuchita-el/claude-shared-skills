...
- 既存の `context-budget.md` も配布物からの参照経路を持たないが、同ファイルの読者はスキル作者であり実行主体ではないため、ポインタは張らない。ただし作者向け規約が配布プラグイン配下に置かれている点は配置として不自然であり、`plugins/` 配下から出す是非は follow-up で扱う（#580。ADR-20260627 の上書き手続きを要しうる）。
