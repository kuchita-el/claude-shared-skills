...
- 既存の `context-budget.md` も配布物からの参照経路を持たない。同ファイルの読者が実行主体かスキル作者かの判定自体が未決であるため、本決定ではポインタを張らない。読者が作者であれば参照経路の欠落ではなく文書の性質ということになるが、その場合は作者向け規約が配布プラグイン配下に置かれている点が配置として不自然になる。判定と配置は follow-up で扱う（#580）。ADR-20260627 は同ファイルを `dor-default.md` / `completion-judgment.md` と同型の分割と位置づけており、判定次第では同 ADR の上書き手続きを要する。
