...
# ADR-202606040737-01: 複数スキルが共有する参照資源はプラグインルート references/ に集約する（DoR定義の単一ソース化）
...
ADR-202605250838-01 は、共有資源（サブエージェント定義）を各スキル配下から**プラグインルートへ巻き上げる**原則を既に確立している。本判断はその同型適用である。なお ADR-202605250838-01 が扱った agents/ は自動検出機構によるもので、パス変数参照を伴わない。本ADRは `${CLAUDE_PLUGIN_ROOT}` によるパス参照を本リポで初めて導入する点が異なる（Claude Code 公式仕様で skill content 内のインライン置換が保証されていることを確認済み）。
