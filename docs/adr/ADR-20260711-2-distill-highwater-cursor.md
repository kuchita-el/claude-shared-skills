...
3. **provenance 導出は維持する**。カーソルより新しいスライス内でも、既に `promoted` または `pending` 候補を持つ観測は provenance で処理源から除外する（重複候補生成の防止）。有界化（カーソル）と重複排除（provenance）は役割が異なり、両者は合成される。
...
- Amends: ADR-20260711-store-state-model-captures-stateless（決定3 の有界化機構 facet ＝ recency 窓 → 処理済みカーソル）
