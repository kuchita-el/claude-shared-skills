...
- Related: ADR-20260711-3-adr-two-axis-status-validity-model（ADR分割の手続き＝決定3 の出典。あわせて採番方式 `ADR-YYYYMMDD[-N]` は ADR-20260402 の旧記述 `docs/adr/{番号}-{タイトル}.md` を上書きする。同方式は上書き済みの ADR-20260511 が定めたもので、決定8「採番方式・配置（ADR-20260511 から不変で引き継ぐ）」として現行 ADR へ不変のまま引き継がれた）
