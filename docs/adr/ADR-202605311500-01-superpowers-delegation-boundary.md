...
# ADR-202605311500-01: Delivery実装メカニクスを superpowers に委譲し、dev-workflow は Discovery＋接続契約に縮退する
...
Related: ADR-202606062346-01-superpowers-soft-delegation（本ADRがパークした委譲の依存方式・非導入時フォールバックを 606-3 が充足。上書きでない）
