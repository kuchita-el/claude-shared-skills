...
- 2026-07-31: 決定2 の射程の注記のうち、既存の `lint-adr.sh` / `next-adr-id.sh` のテストを「順次そちらへ寄せる方針を採っている」と述べていた箇所を、移行が完了した後の配置の事実へ改めた。あわせて配布物内／外を分ける理由の括弧書きを、#623 で根拠を `docs/distribution-boundary.md` へ集約したという時点固定の記述へ改めた。移行の完了により前者は偽になる。後者は、理由の本文を注記側へも残すと根拠が2箇所に存在して片方が黙って陳腐化する一方、「根拠は同文書が定める」のように可変文書を現在の参照先として指す形は記録の参照原則（`manage-adr` の編集判定リファレンス）に反するため、双方を避ける時点固定の過去形を採った。注記の前段（本項の列挙が抽出時点の対象を定めたものであり、配布物内／外の別は本 ADR の射程外であるという射程の言明）と決定2 の本文は不変であり、変えたのは注記の時制と理由付けの置き場のみ。front-matter（`status`/`validity`）不変の非core 編集（#623）。
- 2026-07-30: `## 関連ADR` の ADR-202607261459-01 への `Related:` 行を、退役の事実と現行の規定の在り処を述べる記録の行へ改めた。同 ADR が現行の ADR 化要否基準で非該当となり `廃止済み` へ退役したため、有効 ADR が退役 ADR を `Related:` で指す状態が lint-adr レイヤ4 の違反になる。行ごと削除すると「本機構が何を配送しているか」の出所と、決定3 の drift-lint 配置が本抽出で移設された経緯が辿れなくなるため、削除せず記録として残した。決定の骨子（ADR 運用機構の独立プラグインへの抽出と配送範囲）は不変で、変えたのは参照の書式と退役後の指し先の明示のみ。front-matter（`status`/`validity`）不変の非core 編集（#626）。
