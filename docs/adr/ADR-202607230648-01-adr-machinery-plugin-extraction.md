...
- 2026-07-31: 決定2 の射程の注記のうち、既存の `lint-adr.sh` / `next-adr-id.sh` のテストを「順次そちらへ寄せる方針を採っている」と述べていた箇所を、移行が完了した後の配置の事実へ改めた。あわせて配布物内／外を分ける理由の括弧書きを `docs/distribution-boundary.md` へのポインタへ置き換えた。移行の完了により前者は偽になり、後者は同文書へ根拠を集約する編集（#623）と重複して片方が黙って陳腐化するためである。注記の前段（本項の列挙が抽出時点の対象を定めたものであり、配布物内／外の別は本 ADR の射程外であるという射程の言明）と決定2 の本文は不変であり、変えたのは注記の時制と理由付けの置き場のみ。front-matter（`status`/`validity`）不変の非core 編集（#623）。
