...
status: 承認済み
validity: 上書き済み
superseded-by: ADR-20260713-captures-stateless-candidate-side-state, ADR-20260720-distill-ledger-as-explicit-input, ADR-20260720-2-distill-pending-reevaluation-policy, ADR-20260720-3-distill-ledger-matching-responsibility-boundary
---
...
- Superseded by: ADR-20260713-captures-stateless-candidate-side-state（決定1 の処理源 facet ＝ work-queue 選択〔status 軸の再走査抑止〕を provenance ＋ recency 導出へ上書き。原 ADR-20260711-store の Amend を ADR分割〔ADR-20260711-3 決定3〕で吸収した後継。`superseded-by` に列挙）
- Superseded by: ADR-20260720-distill-ledger-as-explicit-input（決定2〔入力定義の拡張と時間不変性の意図的放棄〕と決定1 の参照源 facet〔決定1b〕を逐語 restate で re-home。`superseded-by` に列挙）
- Superseded by: ADR-20260720-2-distill-pending-reevaluation-policy（決定3〔pending 候補の再評価ポリシー〕を逐語 restate で re-home。`superseded-by` に列挙）
- Superseded by: ADR-20260720-3-distill-ledger-matching-responsibility-boundary（決定4〔責務境界の確認〕を逐語 restate で re-home。`superseded-by` に列挙）
...
- 2026-07-19: `## Context` の ADR 化自己評価のうち、ADR-20260719 決定1 の粒度判定4項目の列挙を実際の項目名（後戻りコスト・複数モジュール／複数開発者への波及・採用理由の揮発・ツールで自動強制できない）へ訂正した。従来の列挙（影響範囲・不可逆性・トレードオフ・横断性）は決定1 の項目と一致しておらず、「トレードオフ」は決定2 の実務シグナル、「横断性」は決定1・決定2 のいずれにも存在しない項目だった。訂正後の4項目に照らしても、入力契約と再現性設計をまたぐ後戻りコスト、store 仕様3文書と distill／promote 境界への波及、時間不変性を手放した判断理由の揮発性、散文で置いた責務境界の機械強制不能性という形で全項目に該当するため、「すべてに該当する」という評価自体は維持した。決定内容は不変で、変えたのは ADR 化判断の根拠記述のみの非core 編集（#515）。
