...
status: 承認済み
validity: 上書き済み
superseded-by: ADR-202607130831-01-captures-stateless-candidate-side-state, ADR-202607200855-01-distill-ledger-as-explicit-input, ADR-202607200856-01-distill-pending-reevaluation-policy, ADR-202607200858-01-distill-ledger-matching-responsibility-boundary
---

# ADR-202606291631-01: distill の入力契約拡張と既存ルール台帳突合
...
しかし当時の distill の入力契約は「入力源は正準パス `captures.md` のみ。in-repo の `plugins/growth/.local/` は走査しない」と定めていた（`personal-store-spec.md`「構成上の保証」、`distill-procedure.md` §2、`distill/SKILL.md` Phase 1 スコープ）。台帳突合はこの入力契約の拡大であり、ADR-202607191825-01 決定1 の粒度判定4項目（後戻りコスト・複数モジュール／複数開発者への波及・採用理由の揮発・ツールで自動強制できない）すべてに該当する構造的判断のため、本 ADR で束ねる。#417 本文自身も「責務線引き＋distill 入力範囲の拡大が ADR 級か判定する。該当すれば本線引きを束ねる ADR を起票する」と求めている。

### 参照の時点固定

`## Decision` は、本 ADR を分割した後継が逐語 restate の対象としているため改変しない。そこでいう distill の「既存責務」は起票時点（2026-06-29）の責務を指し、以後の仕様変更を追わない。
...
- Superseded by: ADR-202607130831-01-captures-stateless-candidate-side-state（決定1 の処理源 facet ＝ work-queue 選択〔status 軸の再走査抑止〕を provenance ＋ recency 導出へ上書き。原 ADR-202607111014-01 の Amend を ADR分割〔ADR-202607112231-01 決定3〕で吸収した後継。`superseded-by` に列挙）
- Superseded by: ADR-202607200855-01-distill-ledger-as-explicit-input（決定2〔入力定義の拡張と時間不変性の意図的放棄〕と決定1 の参照源 facet〔決定1b〕を逐語 restate で re-home。`superseded-by` に列挙）
- Superseded by: ADR-202607200856-01-distill-pending-reevaluation-policy（決定3〔pending 候補の再評価ポリシー〕を逐語 restate で re-home。`superseded-by` に列挙）
- Superseded by: ADR-202607200858-01-distill-ledger-matching-responsibility-boundary（決定4〔責務境界の確認〕を逐語 restate で re-home。`superseded-by` に列挙）
- Related: ADR-202606261847-01-growth-plugin-separation（growth の疎結合分離。distill の台帳読み取りは distill 内部の入力であり、promote → Issue 起票の疎結合とは別レイヤ）
- Related: ADR-202606282107-01-career-decision-model（distill が仮説を生成し、確定・裁定は下流が担うという責務線引きの先例）
- 関連Issue: #417, #416

## 変更履歴

- 2026-07-19: `## Context` の ADR 化自己評価のうち、ADR-202607191825-01 決定1 の粒度判定4項目の列挙を実際の項目名（後戻りコスト・複数モジュール／複数開発者への波及・採用理由の揮発・ツールで自動強制できない）へ訂正した。従来の列挙（影響範囲・不可逆性・トレードオフ・横断性）は決定1 の項目と一致しておらず、「トレードオフ」は決定2 の実務シグナル、「横断性」は決定1・決定2 のいずれにも存在しない項目だった。訂正後の4項目に照らしても、入力契約と再現性設計をまたぐ後戻りコスト、store 仕様3文書と distill／promote 境界への波及、時間不変性を手放した判断理由の揮発性、散文で置いた責務境界の機械強制不能性という形で全項目に該当するため、「すべてに該当する」という評価自体は維持した。決定内容は不変で、変えたのは ADR 化判断の根拠記述のみの非core 編集（#515）。
- 2026-08-02: 記録の参照原則に沿い、`## Context` が可変文書を現在の参照先として指していた記述の時点を固定した。`## Decision` は分割後継が逐語 restate の対象としているため改変せず、そこでいう「既存責務」の時点を `## Context` の「参照の時点固定」節で明示する形を採った（後継 ADR-202607200858-01 と同じ流儀）。決定の骨子・却下理由は不変。退役 ADR を射程へ含める本Issue限りの特例に基づく非core 編集（#658）。
