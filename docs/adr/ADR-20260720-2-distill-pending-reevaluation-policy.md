...
2. **`## Consequences` の調整**: 原 ADR の Consequences 5項を後継3本へ配分した。本 ADR は「得られる利益」（決定2 と決定3 の共同帰結。既知ルールの再発見を「N 回再発」知見へ変換する部分が決定3 に対応する）と、「本 ADR に含めない範囲」のうち出力スキーマの具体（再発回数を専用フィールドにするか本文記述にするか）を保持する。「受容したコスト」「自己参照の留保」「将来の留保事項」は決定2 に対応するため ADR-20260720 が保持する。原 ADR に存在しない記述の新規追加は行っていない。
