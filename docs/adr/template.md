---
status: 提案中             # 承認軸・不変。提案中 / 承認済み / 却下 のいずれか（起票時は「提案中」で開始し、以後は上書きしない）
validity:                   # 有効性軸・可変。有効 / 上書き済み / 廃止済み のいずれか（提案中・却下のときは空のまま）
superseded-by:               # validity=上書き済み のとき必須（例: ADR-YYYYMMDD[-N]-<slug>）。それ以外は空のまま
---
...
- **承認軸**（`status`・不変の履歴事実）: `提案中` → `承認済み` または `却下`
- **有効性軸**（`validity`・可変の現在の効力）: `有効` → `上書き済み` または `廃止済み`
