...
- Amends: ADR-20260629-distill-input-contract-and-ledger-matching（決定1 の work-queue 選択 facet ＝ status 軸の再走査抑止を provenance ＋ recency 導出へ改訂。原 ADR-20260711 から引き継ぐ凍結された歴史的相互参照。ADR-20260711-3 決定4 により新規起票では使わないが、既存関係は遡及編集せず保持する）
- Amended by: ADR-20260711-2-distill-highwater-cursor（決定3 の有界化機構 facet ＝ recency 窓 → 処理済みカーソルへ改訂。原 ADR-20260711 から引き継ぐ凍結された歴史的相互参照。同 facet はさらに ADR-20260712 で retention 射程が改訂されている）
