...
選定理由: ADR は規範文書として値域定義を提供する方が後続スキル（plan-issue / dev-loop）の実装判断を支援できる。1人運用の本リポジトリでも「どの観点で判断すべきか」のヒントとして機能する。値域の追加はパラメータ追加（非core改訂）にあたり、ADR-20260711-3 決定3 の3段構え編集機構の非core段（本 ADR を直接編集し `## 変更履歴` に1行追記）で行う。
...
- 判断者ロールの値域は1人運用前提の例示列挙のため、複数人運用への移行時に値域拡張または役割定義の精緻化を行う（ADR-20260711-3 決定3 の3段構え編集機構に従い、値域拡張は非core段＝直接編集＋`## 変更履歴`、決定の骨子を変える場合は core＝新規 ADR）
