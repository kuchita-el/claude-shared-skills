...
| フィールド | 型 | 値域 | 意味 |
|---|---|---|---|
| 観点ID | 文字列 | Issue 内で一意 | 同一観点への複数回参照を可能にする識別子 |
| 説明 | 自然文 | 自由記述 | 観点の内容。レビュアーが判断対象として認識できる粒度で記述する |
| 検出フェーズ | 列挙 | `refine` \| `plan` \| `実装後` | 観点が最初に検出されたワークフローフェーズ |
...
`refine` / `plan` / `実装後` の3値とする。

- `refine`: refine-issue スキルが DoR チェック中に検出した観点
- `plan`: plan-issue スキルが plan 生成中に検出した観点
- `実装後`: dev-loop スキルがセルフレビュー・コードレビュー結果から検出した観点（実装中の Phase 1 自己検知も含む。PR 本文出力時に集約）
