...
status: 承認済み
validity: 上書き済み
superseded-by: ADR-202606020010-01-principles-rationale-hub, ADR-202607182303-01-workflow-design-v2-terminology-flow-stock-axes
---

# ADR-202605131437-01: workflow-design.md v2 構造選定（Discovery追加・用語フレームワーク非依存化・フロー/ストック軸分離）
...
Superseded by: ADR-202606020010-01-principles-rationale-hub（責務定義 facet を継承）、ADR-202607182303-01-workflow-design-v2-terminology-flow-stock-axes（生存5決定を継承。多決定 ADR の部分 core 反転による分割後継）

Related: ADR-202607112231-01-adr-two-axis-status-validity-model（ADR採番方式 `ADR-YYYYMMDD[-N]` は本ADRの旧記述を上書き。同方式は上書き済みの ADR-202605110956-01 が定めたもので、決定8「採番方式・配置（ADR-202605110956-01 から不変で引き継ぐ）」として現行 ADR へ不変のまま引き継がれた）
