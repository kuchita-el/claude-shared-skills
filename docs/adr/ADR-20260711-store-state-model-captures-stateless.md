...
status: 承認済み
validity: 上書き済み
superseded-by: ADR-20260713-captures-stateless-candidate-side-state, ADR-20260712-captures-bounded-retention-aging
---
...
- Superseded by: ADR-20260713-captures-stateless-candidate-side-state（ADR分割により生存決定1〜3 を逐語 restate で re-home。`superseded-by` に列挙）
- Superseded by: ADR-20260712-captures-bounded-retention-aging（決定4 の retention 姿勢〔観測は削除しない〕を有界保持＋経年削除へ改訂して引き継いだ後継。`superseded-by` に列挙）
- Amends: ADR-20260629-distill-input-contract-and-ledger-matching（決定1 の work-queue 選択 facet ＝ status 軸の再走査抑止を provenance ＋ recency 導出へ改訂。凍結された歴史的相互参照、遡及編集しない）
- Amended by: ADR-20260711-2-distill-highwater-cursor（決定3 の有界化機構 facet を recency 窓 → 処理済みカーソルへ改訂。凍結された歴史的相互参照、遡及編集しない）
