...
3. **ドメインモデルに委譲境界を書かない**: `docs/{domain}/event-storming.md` ・ `domain-model.md` はツール非依存を保つ。superpowers への委譲対応は `docs/workflow-design.md` のスキル対応表に記載し、本 ADR を根拠として参照する。委譲は実装・技術の決定であり、ドメインの事実ではないため、モデル層に漏らさない。
...
- 接続契約の精密な per-aggregate 境界（どこまでを自作アダプタで繋ぐか）は未確定。`docs/workflow-design.md` のスキル対応表で漸進的に確定する
...
Related: ADR-20260402-workflow-design-v2-structure（workflow-design.md v2 構造。本ADRは同構造の Delivery 層に superpowers 委譲境界を導入する）
