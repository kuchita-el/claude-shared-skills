...
- **プロセスとメカニズムの分離**: ドメインモデルはプロセス（誰が・何を判断し・何を指示するか）を扱い、メカニズム（技術的にどう実行するか）はCI/CD等の外部システムの責務。デプロイのメカニズムは環境によらず同一だが、プロセスは環境ごとに異なる（QA環境: 自動指示またはQA担当者判断、本番: リリース基準判定と承認が必要）
