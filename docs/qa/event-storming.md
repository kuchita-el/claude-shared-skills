...
| 11 | テスト結果が報告された | 開発者にテスト完了と結果を報告 | 探索的テスト実行 |
...
**責務**: 手動テストの実施・不具合ハンドリング・結果報告

**含むコマンド**: 探索的テストを実行する、不具合を報告する、バグチケットを作成する、バグの再テストを実施する、テスト結果を報告する

**含むイベント**: 探索的テストが実行された、不具合が発見された、バグチケットが作成された、バグが未再現と確認された、テスト結果が報告された
...
| 報告待ち | 全テスト活動が完了し、結果報告を待っている |
| 完了 | テスト結果が報告され、このテストサイクルが終了 |
...
|---|---|---|---|
| リグレッション自動実行ポリシー | QA環境にデプロイされた | リグレッションテストを実行する | リグレッションテスト実装済み（テスト計画が準備完了）の場合のみ |
...
- **リリース基準判定の除外**: リリース基準判定はトリアージ機構の所在が不明確なため、検証コンテキスト内のイベントとしてモデリングせず、外部連携としてホットスポット化した。検証コンテキストの責務は「テストして結果を報告する」までであり、リリース可否の判断は別の責務
...
| 探索的テスト実行 | QA担当者による手動テストの実施・不具合ハンドリング・結果報告のサイクル | リグレッションテスト結果の確認も含む。再テスト時は新しいサイクル |
