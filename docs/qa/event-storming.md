...
- 対象外: リリース（成果物の公開）、本番デプロイ（本番実行環境への投入）、稼働確認、ロールバックは別途整理
...
| 4 | QA環境にデプロイされた | リリース済み成果物がQA実行環境に投入された | リグレッションテスト実行 |
...
| イベント名 | 理由 |
|---|---|
| 本番デプロイ基準が判定された | トリアージ機構の所在が不明確。検証コンテキスト内の集約ではなく、外部のトリアージ機構との連携として扱う（ホットスポット #2（BP #16）, #3（BP #17）） |
...
| 3 | PdM | 人間（外部・企画） | （本番デプロイ基準判定 — 除外済み、ホットスポット #3（BP #17）参照） |
| 4 | CI/CD | 外部システム（メカニズム） | QA環境にデプロイする（実行。指示は検証コンテキスト側のポリシーまたはQA担当者から受ける） |
| 5 | リグレッション自動実行ポリシー | ポリシー | リグレッションテストを実行する（QA環境にデプロイされた時に自動実行） |
| 6 | QAデプロイ自動指示ポリシー | ポリシー | QA環境にデプロイする（PRがマージされた時に自動指示） |
...
|---|---|---|---|
| 1 | 検証とリリースのコンテキスト分割（BP #15） | 全体 | **検証側は分離済み**（本ドキュメント）。リリース側は未決定: 本番デプロイのプロセス所有コンテキストがBP #16・#17の解決に依存。Big Pictureのコンテキスト一覧・フロー図・依存図の更新を含む |
| 2 | トリアージ機構の所在が不明（BP #16） | バグチケットが作成された | バグの「本番デプロイブロッカー判定」を誰が・どこで・誰に回答するかを定義する。企画側・対応コンテキスト・独立機構のいずれか |
| 3 | PdMの責務の帰属（BP #17） | 本番デプロイ基準判定（除外済み） | PdMが本番デプロイ可否を判断する行為が企画コンテキストの責務か、別の責務かを明確にする |
...
- **本番デプロイ基準判定の除外**: 本番デプロイ基準判定はトリアージ機構の所在が不明確なため、検証コンテキスト内のイベントとしてモデリングせず、外部連携としてホットスポット化した。検証コンテキストの責務は「テストしてサイクルを完了する」までであり、本番デプロイ可否の判断は別の責務
- **リリースとデプロイの概念分離**: リリース（成果物の公開。バージョンを持つ）とデプロイ（リリース済み成果物を実行環境に投入する行為）は別概念。QA環境デプロイも本番デプロイも「リリース済み成果物のデプロイ」であり、デプロイ先の実行環境が異なるだけ。ただしプロセス（判断・指示・確認）は環境ごとに異なるため、ドメイン上は別の行為として扱う
- **プロセスとメカニズムの分離**: ドメインモデルはプロセス（誰が・何を判断し・何を指示するか）を扱い、メカニズム（技術的にどう実行するか）はCI/CD等の外部システムの責務。デプロイのメカニズムは環境によらず同一だが、プロセスは環境ごとに異なる（QA環境: 自動指示またはQA担当者判断、本番: 本番デプロイ基準判定と承認が必要）
...
| バグチケット | テスト実行中に発見された不具合を記録するチケット。開発チケットに関連付けられる | QA側で起票し、対応コンテキスト（Big Picture #21）のトリアージ対象となる。本番デプロイブロッカー判定は外部のトリアージ機構で実施（ホットスポット #2） |
| リリース | ビルド成果物を公開する行為、およびその成果物。バージョンを持つ（GitHub Release、Dockerイメージタグ等）。運用に載せる意味は含まない | デプロイとは別概念 |
| デプロイ | リリース済み成果物を実行環境に投入する行為。デプロイ先の実行環境は目的に応じて複数存在しうる（QA環境、本番環境等） | メカニズム（技術的実行）はCI/CD、プロセス（指示・判断）はドメインコンテキストが所有 |
| 実行環境 | リリース済み成果物がデプロイされ稼働する環境。検証目的のQA環境と商用の本番環境が現時点で想定されている | |
| 本番デプロイ基準 | 特定バージョンを本番環境にデプロイ可能かどうかの判定基準 | 判定者・判定プロセスの所在は未確定（ホットスポット #3） |
