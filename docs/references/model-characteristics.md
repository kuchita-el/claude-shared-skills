...
この実測結果は §4 の推論的指針（レビュー・検証系→Opus、機械的変換→Sonnet）と整合している。model 設定を変更する際は `docs/dogfood/317-evaluation-methodology.md` の手法で再評価すること。配布は main へのコミットで届くため、版の繰り上げは要らない（ADR-202609061416-01）。
