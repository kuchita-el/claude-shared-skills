...
// ルール: 入力が1以上なら成功、それ未満なら失敗理由 "必要Approve数は1以上" を返す
```
...
**制約付き値の扱い:**
- 型そのものに制約を埋め込むのではなく、「型 + 生成関数のシグネチャ + ルールのコメント」の3点セットで表現する
...
ユースケース全体を関数合成として表現する。

```fsharp
// 入力 → ... → 出力 の流れを一つの関数型で表現
type チケットから実装に入るまで =
  チケット                                    // 入力
    -> Result<確定プラン, 作成失敗理由>      // 途中結果
    -> 実装プランが確定した                   // 最終イベント
```
...
// 例: レビュー依頼は検証ゲート通過後のみ
type レビュー依頼をする =
  検証ゲート通過済みPR -> レビュアーリスト -> Result<レビュー依頼がされた, 依頼失敗>
// 型名「検証ゲート通過済みPR」により、検証ゲート未通過のPRを渡せない
```
