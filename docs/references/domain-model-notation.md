...
| 記号 | 読み方 | 例 |
|---|---|---|
| `A =` | 「A は次のように定める」 | `実装プラン = 作成中 OR 確定` |
| `A AND B` | 「A と B の両方を持つ」 | `プランID AND チケットID` |
| `A OR B` | 「A か B のどちらか」 | `作成中 OR エスカレーション中 OR 確定` |
| `A (B)` | 「A は B の形で表す」または「A は B という値を内側に持つ」 | `プランID (文字列)`, `仕様不明点あり (質問内容を持つ)` |
...
**コマンド・ワークフローの失敗をどう表現するかは、プロダクトチーム全体で合意を取る必要がある**。失敗の扱い方は実装方針だけでなく、ビジネス側の判断（「失敗ケースをドメインの一部として扱うか」）を含むため、記法の選択前にチームで議論すること。

本ガイドでは選択肢を提示する。採用した方式を全コマンド・全集約で統一すること。

### 選択肢A: 失敗理由を型で列挙する（既定）

出力を「成功時」と「失敗時」に分け、失敗理由を判別共用体（OR）で列挙する。
...
**メリット**:
- 失敗ケースがドメイン語彙として明示され、非エンジニアが失敗シナリオを一覧できる
- プロダクトチームで失敗時の業務フロー（エスカレーション、リトライ、代替案）を合意できる
- 実装時に失敗の網羅性を検査しやすい（Wlaschin 本の Railway Oriented Programming と整合）

**デメリット**:
- 全コマンドに失敗理由の定義が必要でドキュメント量が増える
- 「内部エラー（DB接続失敗等）までドメイン語彙に含めるか」の線引きが必要

### 選択肢B: 失敗を明示しない

成功時のみを記述し、失敗は実装側の関心とする。
...
**デメリット**:
- 失敗時の振る舞いがドキュメントに残らない
- プロダクトチームで失敗時の業務フローを合意する場が失われる
- 実装時にエラーハンドリングの抜けが起きやすい

### 選択肢C: 複数イベントを返す

1コマンドで複数のドメイン変化が起きる場合、成功時に複数のイベントを返す。

```
注文を確定する:
    入力: 注文
    出力: 注文が確定した AND 在庫が引当てられた
```

**メリット**:
- 1コマンドで複数のドメイン変化が起きる場合の自然な記述
- Event Sourcing との親和性

**デメリット**:
- イベントの順序・因果関係の表現が別途必要
...
**既定は選択肢A**（または選択肢A + C）。失敗をドメイン語彙として明示することで、プロダクトチーム全体で合意形成の対象に含められる。

プロダクトチームの判断で他の選択肢を採用する場合、**ドメインモデル文書の冒頭に採用方針を明記**し、全コマンド・全集約で統一すること。途中で方針が混在すると読解が破綻する。

## 型テンプレート

各ドメインモデル文書で定義する型カテゴリと雛形を示す。以降の例は **選択肢A** を前提に記述する。他の選択肢を採用する場合は失敗時の記述を置き換えること。
...
- 失敗理由が値を持つ場合は `(質問内容を持つ)` のように括弧内に記述
...
| **失敗の扱い** | 本文書で採用する選択肢（A/B/C）を明記。全コマンド・集約で統一する |
