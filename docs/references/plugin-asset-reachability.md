...
配布プラグインの本文から同梱資産（`references/` `scripts/` `agents/`）へ到達する手段の単一の出典。どの走査面に書かれたどのパス変数が解決されるかを定める。配布プラグインを新設するとき、既存プラグインの本文へ同梱資産の参照を足すとき、サブエージェントの側から同梱資産へ届かせたいときに参照する。

本文書が扱うのは到達可能性（どの形なら届くか）だけである。到達の要否や、届く複数の形のうちどれを正規形とするかは扱わない。
...
観測条件: Claude Code 2.1.263 / 2026-09-06 に観測。全セルを観測済みで、未観測のセルは無い。
