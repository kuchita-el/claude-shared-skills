...
主要なワークフローフェーズは共有スキルとして実装されている。汎用実装メカニクスは superpowers に委譲し、dev-workflow は Discovery（堀）と接続契約を担う。委譲境界の根拠は [ADR-202605311500-01](../adr/ADR-202605311500-01-superpowers-delegation-boundary.md) を参照。

| 設計書のフェーズ | 対応スキル | 担い手（ADR-202605311500-01） | 状態 | 入力 | 出力 |
|---|---|---|---|---|---|
