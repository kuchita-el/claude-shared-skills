...
- プラグインルートパス（Claudeでは`${CLAUDE_PLUGIN_ROOT}`、Codexではhostが渡す解決済み`pluginRoot`の絶対パス。共有DoR・種別プロファイル・振る舞いの不変条件の参照に使用）
