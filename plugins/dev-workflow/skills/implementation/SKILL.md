...
3. **ブランチの準備**（新規作業時のみ。新ブランチは `origin/<base>` の最新を起点に切る。委譲経路のネイティブ worktree ツールでは基点を harness の `worktree.baseRef`＝既定 `fresh` が保証し、git fallback / 非導入経路では implementation が `git fetch`→`origin/<base>` 起点を接続契約として担う）
