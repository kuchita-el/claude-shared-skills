...
ブランチの完了（PR 作成／マージ／クリーンアップの選択提示）は `superpowers:finishing-a-development-branch` に委譲する（S6・参照機構②）。ただし PR は **既定で Draft 作成**し、その後 dev-workflow 固有の上乗せ層として **CI ゲート**を実行する: まず非 watch で required check の有無を同期判定し（**不在（CI 未設定）→ 即 Ready 化**）、存在すれば `gh pr checks <pr> --watch --required` を `run_in_background` で回して結審を待ち、**緑 → Ready 化＋レビュー依頼 ／ 赤 → Phase 1〜3 修正ループへ差し戻し ／ 収束不能 → エスカレーション** へ分岐する。Draft ＝ CI 未検証、Ready ＝ CI 緑という**コンテンツ状態に基づく判断**であり、feedback「Draft/Ready はコンテンツ状態で判断（一律 Draft 禁止）」と矛盾しない。責務分界（draft 作成不可時の `gh pr create --draft` フォールバック含む）・分岐表・採用方式の根拠・レビュー依頼ルールは `${CLAUDE_SKILL_DIR}/references/phase4-ci-gate.md` を参照。
