...
深刻度の値域（`${CLAUDE_SKILL_DIR}/references/review-findings-handling.md` §1.1 が正本として指す「4. 重大度の分類」節）の解決元は `${CLAUDE_PLUGIN_ROOT}/agents/code-reviewer.md` とする。references 本文はパス変数を展開できず到達手段を持てないため、Phase 2・Phase 5・フォールバック分岐のいずれで参照する場合も、本スキル本文が供給するこの解決元を用いる。
...
- **深刻度調整**: レビュー結果を `${CLAUDE_SKILL_DIR}/references/review-findings-handling.md` §1 のルールに従い周辺コード文脈で再評価し、ブロッカー/改善提案の2段で確定する。元の指摘内容は除外・改変しない（詳細運用は参照先）。
...
- `改善提案（未対応）` セクション（純粋な改善提案を記載）

詳細は `${CLAUDE_SKILL_DIR}/references/pr-body-template.md` を参照（PR 本文テンプレート全文、および「深刻度調整セクション」と `review-findings-handling.md` §1.5 のブロック形式との役割分担を含む）。
...
**制御フロー（再入構造）**: Phase 5 は Phase 4 の線形な後続ではなく、前半と後半の間に Phase 1〜3 を挟む。入口・前半の出口・後半への復帰・後半の CI ゲートという4遷移は `${CLAUDE_SKILL_DIR}/references/review-findings-handling.md` の §2.1（再入構造）と §2.2 の手順2・手順4 が正本。
...
- **セルフレビュー（S7）**: メインループ自身がレビュー契約と差分を突き合わせる（深刻度2段は冒頭で定めた値域の解決元に準拠、深刻度調整も同様に適用）。
