...
- **前半の出口**: 深刻度調整でブロッカーと確定した指摘のみを作業リストへ載せ Phase 1 へ渡す。ブロッカーが0件の場合（全指摘が改善提案へ格下げ／全件に反論）は Phase 1 を経由せず直接後半へ進み、修正コミットが無いため後半の CI ゲート再走行も行わない。確認が要る指摘が残る場合はエスカレーションが優先し、後半へ進まない。
