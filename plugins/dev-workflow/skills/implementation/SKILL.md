...
3. **ブランチの準備**（新規作業時のみ。`origin/<base>` の最新へ同期し**現 HEAD を最新ベースに据えてから**、superpowers 導入時は `superpowers:using-git-worktrees` に委譲、非導入時は最新ベース起点で通常のブランチ作成へフォールバック。最新化は接続契約として本スキルが担う）
