...
**原則（全操作共通）**: **変更/生成した ADR と、それが相互参照で指す相手をすべて含める** — forward（front-matter `superseded-by` が指す後継）と reverse（本文 `## 関連ADR` の `Supersedes:` が指す先行）の両方向。相手を漏らすと lint レイヤ3 が参照先不在で発火し、正しい操作でも exit 1（false negative）になる。

操作別の典型セット（いずれも上記原則に従い、対象 ADR 本文に `Supersedes:` があればその先行 ADR も追加する）:

| 操作 | 隔離コピーに含める ADR |
|---|---|
| 起票 | 生成した ADR（通常は相互参照なし） |
| 承認 | 対象 ADR ＋ 本文に `Supersedes:` があれば先行 ADR |
| 上書き | **旧 ADR ＋後継 ADR**（旧 `superseded-by`→後継の双方向）＋ 旧本文に別の `Supersedes:` があれば先行 ADR |
| 廃止 | 対象 ADR ＋ 本文に `Supersedes:` があれば先行 ADR |
| 却下 | 対象 ADR ＋ 本文に `Supersedes:` があれば先行 ADR |

- 上書きで旧 ADR 単体をコピーすると、旧の `superseded-by` が指す後継が隔離セットに不在となり forward が発火する（実測 exit 1）。旧＋後継の2ファイルで双方向が揃い exit 0 になる。
- 廃止・承認・却下の対象が過去に別 ADR を上書きしていて本文に `Supersedes: <先行>` を持つ場合、対象単体だと reverse（本文起点）が先行不在で発火する（実測 exit 1）。先行 ADR も含めれば exit 0 になる（廃止が現実的なケース）。
