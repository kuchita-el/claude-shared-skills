...
<採用結果としての影響・トレードオフを記述。受容したコスト、得られた利益、将来再検討する条件など。本ADRが意図的に決めなかった facet（パーク）は「## 保留した決定」へ置く>

## 保留した決定

<!--
本ADRが意図的に決めなかった facet（パーク）を、起票時点のスナップショットとして記録する。
- 意味論: 何を決めなかったかの不変記録。open/resolved 等の状態は持たせず、後から本ADRをさかのぼって更新しない（更新義務が drift を生み、退役後は凍結原則で編集もできないため）。
...
- パークする facet が無ければ本節ごと削除する（「該当なし」は書かない）。
- 意味論・充足規約・lint 要件の詳細と根拠は ADR-20260720-4-adr-parked-decision-section を参照。
-->

<パークした facet を列挙。無ければ本節ごと削除>
