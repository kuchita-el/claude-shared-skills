...
以下の順序でReadツールで読み込み、5点の内容をマージして規範とレビュー基準とする:
...
規範・検査観点・深刻度の値域・出力テンプレートは常に読み込む。レビュアーへ渡すのはこの5点をマージした1つの規範であり（ステップ7a）、深刻度の値域は当該節のみの抽出、出力テンプレートは本ステップを唯一の読み取り点とする取り決めを伴うため、マージ結果を単一のパスでは指せない。生成側とレビュー側が同一の規範で判定するために、深刻度の値域と出力テンプレートは本文としてマージして渡す（パスを渡して読ませない）。プロジェクト固有ファイルが存在する場合（Readが成功した場合）、その内容を追加のレビュー観点として加える。
...
- **規範とレビュー基準**: ステップ2で読み込んだ5点（規範・検査観点・深刻度の値域・出力テンプレート・プロジェクト固有基準）の全文
...
- レビュー結果が `FAIL`（ブロッカーが1件以上） → メインが Edit するのではなく、**新しい plan エージェントインスタンスを編集モードで起動**し（Agent tool の `subagent_type: dev-workflow:plan`。起動できない場合のフォールバックは生成モードと同様）、プランファイルのパス＋出力形式が定める経緯付録のパス＋レビュアーの指摘テーブルを渡して修正させる（エージェントが自前で Read → 該当箇所を Edit → 当該周回の指摘と対応の要旨を経緯付録へ追記 → 変更サマリを返却。plan 本文はメインに載らない）。経緯付録への追記は修正周回ごとに行わせる（名前の作り方は出力形式が正本であり、ここへ再掲しない）。修正後 7b に戻る（最大2周）。ここで渡すプランファイルのパスは、ステップ5で `${CLAUDE_PLUGIN_ROOT}/references/plan-location-resolution.md` の解決順序に従って解決した解決ディレクトリ内のパスを用いる（編集は in-place で行い、現作業ディレクトリへコピーしない）

**ループ終了条件:**

- ブロッカーが無くなった（`PASS`） → ステップ8へ
- 2周してもブロッカーが残る → 残ったブロッカーを経緯付録へ追記させたうえで、根本的な問題としてステップ8で質問する（未解消の指摘は経緯付録が保持する記録種別であり、質問だけを残して消さない）
...
レビュアーの改善提案が残っている場合は、質問ではなく報告として利用者へ伝える（`PASS` の判定は覆さない）。あわせて、当該の改善提案を経緯付録へ追記させる（未解消の指摘は経緯付録が保持する記録種別であり、チャット報告だけを残して消さない）。質問がない場合は、プランファイルのパスを報告して完了する。

**裁定結果の反映と論拠の移送:** 本ステップで判断依頼・確認事項が裁定されたら、plan エージェントを編集モードで起動し、確定した内容をプラン本文の当該エントリへ反映させ、裁定に至った論拠は経緯付録へ移させる（プラン本体には残さない）。渡す資産は 7c と同じ3点——プランファイルのパス・経緯付録のパス・裁定内容——とする。反映と移送をメインが直接 Edit で行わないのは、7c と同じくプラン本文をメイン context へ載せないためである。この結線を欠くと、経緯付録が保持すると宣言した記録種別のうち「裁定に至った論拠」だけが運用上一度も生成されず、裁定の論拠がプラン本体へ残るか失われるかのどちらかになる。
