---
description: Issueから plan サブエージェントで実装プランを作成し、独立レビュアーエージェントによるレビューと要確認事項の質問までを自動化する。Issue実装プランの作成、技術設計とタスク分解の計画立案に使用。「実装計画を立てて」「プランニングして」「タスク分解して」「設計を整理して」「Issueの計画を作って」といった依頼時にも使用
allowed-tools:
  - Read
  - Grep
  - Glob
