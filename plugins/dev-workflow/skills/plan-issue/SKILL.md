...
計画出力とサブエージェント用promptの必須資産は `plugins/dev-workflow/skills/plan-issue/references/plan-output-format.md`、`plugins/dev-workflow/skills/plan-issue/references/plan-prompt.md` を直接参照する。
...
分岐は**再生成（既定）**と**手動修正（軽微指摘のみ）**の2経路。却下理由が方針・設計判断・スコープ・タスク分解の粒度・検証方針の選択に関わるなら再生成、文言・体裁・参照リンクのみなら手動修正とし、**境界が曖昧な場合は再生成側に倒す**。
