...
取り込み Issue で career＝`ADR 差分` と裁定された仮説を、ADR として `docs/adr/` へ昇格する。
...
- **ADR 差分**: career 列＝`ADR 差分` かつ ADR 化推奨と判定した仮説のみを `docs/adr/` の diff に含める。career 列が他の値の仮説・ADR 化しないと判定した仮説を ADR ファイルに含めない。確認は PR の diff（反映前後の `docs/adr/` の比較）で目視する。
