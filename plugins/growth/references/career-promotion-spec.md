...
1. **粒度判定**: 当該 repo の ADR 運用ルールの粒度判定基準を文脈から探索して照合し、ADR 化要否を判定する。運用ルールが見つからない場合は、**growth 自身の判定基準**をベストエフォートで適用する（配布元プラグインの粒度判定基準は参照しない＝「ADR 運用ルールが見つからない場合」節）。判定基準は career 決定表 行3 の入力条件——**後戻りコストが高く横断再発する構造的な設計判断**（複数モジュール波及・採用理由揮発・却下選択肢ありの設計決定）——であり、[`distill-procedure.md`](../skills/distill/references/distill-procedure.md)「career-hypothesis の判定（決定表）」を単一出典とする。**判断に迷ったら ADR 化しない**（境界では「書かない」を優先）。
...
- **粒度判定（ADR 化要否）**: 上記ステップ1の growth 自身の判定基準（career 決定表 行3 の入力条件）をベストエフォートで適用する。**配布元プラグイン（ADR 専用プラグイン）の粒度判定基準は参照せず、その項目名・スコア境界を複製もしない。** growth の ADR 化提案は ADR 専用プラグインの有無と独立に成立させる方針であり、配布元の基準へ追随する義務を負わない。
- **既存 ADR との整合**: スキップする。既存 ADR 0件の状態とみなし、常に新規 ADR として扱う（上書き・廃止・in-place 改訂の分岐は適用しない）。

ADR 運用ルールを持たない repo に対しても ADR 起草を提案してよい。提案は既存の implementation → PR ワークフローで PR として提出され、受け入れ可否は当該 repo の PR マージゲート（人間）が締めるため、ベストエフォートの判定で提案する側に倒してよい。

このフォールバックは粒度判定と既存 ADR 整合のスキップのみを定め、上書き手順・編集機構は内包しない（それらは当該 repo の運用ルールが担い、無ければスキップする）。
...
粒度判定（当該 repo に ADR 運用ルールが無く、growth 自身の判定基準＝career 決定表 行3 の入力条件を適用した場合）:

| 観点 | 該当 | 理由 |
|---|---|---|
| 後戻りコストが高い | ○ | 配置を後から変えると全参照元の書き換えが要る |
| 横断再発する（複数モジュール波及） | ○ | 複数スキルが同一ファイルを参照する横断事項 |
| 採用理由が揮発しやすい | ○ | 「なぜ単一出典か」は記録しないと忘れられる |
| 却下選択肢がある | ○ | 各スキル配下へ重複コピーする案を却下している |

→ 全観点に該当＝ **ADR 化推奨**。既存 ADR と矛盾しなければ新規 ADR を採番方式で起票し、矛盾するなら Superseded 手続きを踏む。implementation → PR ワークフローで当該 repo の ADR 配置への追加を提出する。
...
| 観点 | 該当 | 理由 |
|---|---|---|
| 後戻りコストが高い | × | 局所的・機械置換で戻せる |
| 横断再発する（複数モジュール波及） | × | 書式の局所事項 |
| 採用理由が揮発しやすい | × | 自明な書式統一 |
| 却下選択肢がある | × | 対抗案を伴わない書式統一 |

→ いずれの観点にも該当せず＝ **ADR 化しない**（書式的な命名規約に該当。当該 repo の ADR 運用ルールに命名規約の ADR 化基準があれば同基準にも合致し、無くても growth 自身の判定基準で ADR 化しない）。ADR ファイルに反映しない。learnings.md 行き等として再裁定するか、Linter / フォーマッタ規約として扱う。
