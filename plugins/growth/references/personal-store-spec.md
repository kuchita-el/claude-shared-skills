...
| `actual` | 該当時必須 | 実際。実際に起きた結果。transcript からの**逐語引用**に限る。フィールドは常設し、値は transcript から抽出できる場合に記す（抽出不能なら空可）。「生記録性」節を参照 |
...
新フィールドの `origin`（出所）・`expected`（予測）・`actual`（実際）も同じ生記録性の制約下にあるが、transcript からの**引用可能性は一様でない**。`origin`（出所の判別）と `actual`（実際に起きた結果）は transcript に実在する痕跡（tool_result のエラー文字列・ユーザーの訂正発話等）から**逐語で引用**できる。一方 `expected`（予測した結果）は、当方が予測を明示的に言語化していない限り transcript に逐語では存在しないことが多く、痕跡（`type=thinking` / `tool_use.input` 等）に基づく**再構成**を要する。したがって `expected` は逐語引用に縛らず、痕跡に基づく予測の再構成を許す（`origin` / `actual` は逐語引用に限る）。
