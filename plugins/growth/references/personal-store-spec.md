...
| `actual` | 該当時必須 | 実際。実際に起きた結果。transcript からの**逐語引用**に限る（複数行は代表行の引用または改行を空白へ正規化して単一行に収める。「パース規約」節「引用の単一行畳み込み」を参照）。フィールドは常設し、値は transcript から抽出できる場合に記す（抽出不能なら空可）。「生記録性」節を参照 |
