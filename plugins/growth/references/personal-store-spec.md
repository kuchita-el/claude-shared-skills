...
- **メタフィールド**: `signal` / `session` / `origin` / `expected` / `actual` は `- key: value` 形式の単一行。エントリ内の `- key: value` 行をメタとして抽出する。`origin`（痕跡種別）・`expected`（予測）・`actual`（実際）も同一の単一行メタ規約に従い、`signal` と直交する独立軸として追加する（値域・必須性は「スキーマ」「痕跡種別」節を参照）。
...
AC の必須欄「生観察」「シグナル種別」「由来セッション参照」はそれぞれ `observation` / `signal` / `session` に対応する。新フィールドのうち `origin`（痕跡種別軸）は常に必須、`expected` / `actual` はフィールド常設・値は該当時必須（transcript から抽出できる場合に記す）。これらは `signal` と直交する独立軸であり、既存の必須欄（`observation` / `signal` / `session`）を置換・改名しない。
...
`origin` の値域は、観察の学習シグナルが**どんな痕跡として実在するか**（痕跡種別）を表す。Phase 1（痕跡ソースが現セッションの会話履歴に限られる）では transcript 由来の以下2値とする。

| 値 | 意味 |
|---|---|
| `tool-result` | ツール結果（tool-result）の痕跡。環境（権限・hook・コマンド失敗等）との摩擦の候補 |
...
- カーソル値は `captures.md` の `## <timestamp>` 見出し（ISO 8601）と**同一キー空間**で辞書順比較する。観測は capture 実行時刻の単調増加 timestamp を持つため「カーソルより新しい」は一意に定まる（ADR-202607111225-01〔ADR-202607121331-01 が restate〕末尾）。1 run 複数観察に付す run 内序数サフィックス（`-NN`・固定幅）は辞書順で記録順に整列するため、同一キー空間・単調増加の前提を保つ（ADR-202607112340-01）。
...
- **巻き戻し（distiller 改善時）**: distiller を改善したときは、カーソルを意図的に**保持集合の先頭**（retention horizon 先頭。または再走査したい範囲の起点）へ巻き戻して1回だけ再導出する。horizon を超えたバケットは経年削除済みで物理的に存在しないため、再走査の到達射程は自然に horizon 先頭で有界化される（ADR-202607121331-01 決定2）。巻き戻し範囲の観測を再走査し、provenance が live 候補（`promoted` / `pending`）の重複を止め、`rejected` 不可侵（ADR-202607200856-01 決定1）が棄却済み同一仮説の復活を止める。再導出後カーソルを最新へ戻す。改善判定は自動化せず、改善を入れた開発者が明示操作として巻き戻す。
- **巻き戻し（candidates.md 消失時）**: `candidates.md` が消失した場合はカーソルを**保持集合の先頭**（retention horizon 先頭）へ巻き戻し、保持されているバケット群（`captures-*.md`）から再導出する（ADR-202607111014-01 Consequence (b) の「再生成可能」をカーソル巻き戻しとして具体化。再導出の到達射程は ADR-202607121331-01 決定2 により horizon 先頭までへ有界化される）。
...
**必須欄 `origin` を欠くエントリ**（`origin` 導入以前の capture が書いたもの）も、distill は**読み飛ばさず処理源に含める**。欠損は「痕跡種別が不明」として扱い、特定の値を補って読まない。帰結は2つある。

- ハーネス強制摩擦の既定除外（`origin=tool-result` かつハーネス発のガード／deny／エラーを機械的に除く。ADR-202607010734-01 決定3）は、条件に一致しないため当該エントリへ効かない。除外されるべき観測が処理源に混じりうる
- 混入したノイズは distill の判定と人の確認で落とせるが、エントリごと処理源から除けば当該の観測は回収できない。**取りこぼしを避ける側へ倒す**（カーソル欠損時に全走査を既定とする上記「欠損時既定」と同じ向き）

欠損を `tool-result` とみなす扱いは採らない。既定除外が過剰に効き、`user-utterance` 由来の観測を落とすためである。旧エントリへ `origin` を後から補う一括変換も行わない（生記録性を侵すため。旧 `status` と同じく読み取り時に吸収する）。
...
- **guarantee-once 非退行**: 未 distill エントリを含むバケットは horizon 超でも削除しない（drop 条件の「全エントリ通過済み」を満たさないため）。未検証観測が齢で無音脱落しない（ADR-202607121331-01 制約〔ADR-202607111225-01 決定2 を restate〕維持）。
- **再導出可能性の核は horizon 内で維持する**。「観測を安易に捨てない」核（ADR-202607111014-01 決定4 の趣旨）は horizon 内の巻き戻し再導出可能性として restate され、horizon を超えた観測のバケット削除と両立する（ADR-202607121331-01 決定1・決定3）。
...
- **一意化規則**: 見出しキーは capture 実行時刻に、1 run 複数観察時の run 内序数サフィックス（`-NN`）を加えて一意化する（capture SKILL.md Step 1・ADR-202607112340-01）。秒精度の実行時刻のみでは 1 run 複数観察で衝突するため序数で割る。これにより同一 store 内で見出しキーが衝突せず、provenance 参照・candidate-status 前進の同定対象が一意に定まる。
- **将来の留保（Phase 3 並行 capture）**: hook 自発化（Phase 3）で同一 store への並行 capture が生じると、run 内序数は run をまたいだ大域一意性を保証しない。その段階では時刻順序を内包する一意 ID（UUIDv7 / ULID 等）への移行を再訪する。本 Phase では生成手段の統合コスト・可読性・ISO 8601 モデルとの整合から実行時刻＋序数を採る（案の比較・却下理由は ADR-202607112340-01）。
...
> Distill は処理源（セグメント `captures-*.md` 群）とは別に、既存ルール台帳（`CLAUDE.md` 2層・`learnings.md`・`candidates.md` 自身）を**読み取り専用の参照源**として突合に用いる（仮説は生成しない・台帳は書き換えない）。処理源とは別レイヤであり、本「構成上の保証」の対象（生観測が配布物に混入しないこと）には影響しない。詳細は distill-procedure.md §2 および ADR-202607130831-01（処理源）・ADR-202607200855-01（参照源）。
