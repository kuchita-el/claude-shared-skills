...
置き場こそ既存 memory 機構と同じ user-local 領域に同居するが、store は memory とは設計意図が異なる。memory は「個人で終わる」（共有のデッドエンド）。本 store は **昇格経路を必ず持つ**点で memory と differ する。store は学びの出発点であって終点ではない——貯めた生観測は検証され、支持されたものが committed の配布物へ昇格していく。`status` フィールド（後述）がこの昇格状態を追跡する。
...
| フィールド | 必須 | 内容 |
|---|---|---|
| 見出し（`## <timestamp>`） | 必須 | ISO 8601 形式のタイムスタンプ。観測時刻 |
...
各エントリは `status` フィールドで処理状態を持つ。Distill が未処理の観測を識別できるようにするための状態軸である。
