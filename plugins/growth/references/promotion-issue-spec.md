...
- **昇格先キャリアはラベルで区別しない**。career は distill が `career-hypothesis` の仮説として生成し、本文の `## キャリア` 注記として運ばれる。最終裁定は集約点（取り込み Issue）が人間トリアージで行うため、起票時にキャリアをラベルで固定しない（ADR-202606282107-01）。
