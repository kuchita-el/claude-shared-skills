...
| **裁定結果** | 候補ごとに人間が確定した career と空間（後述「裁定結果の記録形式」） | テーブル1行＝1候補 |
...
2. **public への昇格を拒否できる**: 公開ゲートは裁定で「閉じた」と締めれば public（パブリック/グローバル空間 = `learnings.md` 相当）への昇格を拒否できる。すべての候補が自動で public へ向かうことはない。
...
| 裁定結果の記録形式（career・空間のテーブル。宛先 repo を持たない） | 候補 Issue（`growth:promote`）のテンプレート自体（`promotion-issue-spec.md` #382 が担う） |
