---
description: capture は現セッション会話履歴（session jsonl）から摩擦シグナル（訂正・ツール拒否・反復試行・期待違反の補助検出器）と判断知（選好・却下理由・目標表明・設計判断）を検知し、解釈を加えない生観察として個人ローカル store に記録する。ハーネス強制済みの摩擦は既定除外する。判断は後回しにし「何が起きたか」のみを記す。セッション中の学習シグナルを貯めたいとき・growth の学習ループを手動起動するときに明示起動する（Phase 1）。
allowed-tools:
  - Read
  - Write
  - Bash(mkdir *)
...
現セッション会話履歴から摩擦シグナルと判断知を検知し、生観察を個人ローカル store へ記録する。

## 目的・原則

- **目的**: 学習シグナルの痕跡を「判断前」の状態で保存する。蒸留・候補化は Distill が担う。
- **2軸の検知**: 摩擦知（予測誤差＝訂正・ツール拒否・反復試行・期待違反の補助検出器）と、予測誤差の形を持たない判断知（選好・却下理由・目標表明・設計判断）の両方を拾う。復元不能性・価値の判定は Distill / promote に委ね、capture では行わない（ADR-20260701 D2）。
...
**A. 摩擦知（摩擦サブセットの補助検出器）**: 予測誤差の形を持つ摩擦を拾う。user 訂正系は復元不能性とも重なるため引き続き有効（退行させない）。
...
**B. 判断知検出子（予測誤差の形を持たない会話知）**: user 発話が決定内容（選好・理由付き却下・目標/意図/継続方針・設計境界の確定）を帯びていれば **recall 優先で軽く**拾う。復元不能性・配布価値の判定は distill/promote に委ね、capture では行わない（判断は後回し）。
...
これらはハーネス発のガード/deny/エラーとして機械判別できる。ユーザーが許可プロンプトを対話的に拒否した拒否（常設 deny ルールでない）は判断であり `ツール拒否` として捕捉対象に残す（除外しない）。**ただしハーネス非強制のルール再発（ガードを持たない CLAUDE.md ルール等の反復違反＝#417 の的）は除外しない**（保持する）。除外は機械判別可能な telemetry に限定し、判別が曖昧な摩擦は落とさず残す（精査は Distill）。
...
rm コマンドのツール呼び出しをユーザーが許可プロンプトで対話的に拒否した（常設 deny ルールによる自動拒否は D3 で既定除外。本例は対話的拒否のため捕捉対象）。
```
