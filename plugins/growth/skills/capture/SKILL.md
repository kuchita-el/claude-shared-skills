...
これらはハーネス発のガード/deny/エラーとして機械判別できる。**ただしハーネス非強制のルール再発（ガードを持たない CLAUDE.md ルール等の反復違反＝#417 の的）は除外しない**（保持する）。除外は機械判別可能な telemetry に限定し、判別が曖昧な摩擦は落とさず残す（精査は Distill）。
