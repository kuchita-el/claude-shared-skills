...
起票に失敗した仮説は `candidate-status: pending` のまま残り、再実行可能な状態を保つ。検証で棄却した仮説は `candidate-status: rejected` として確定し（ADR-202607200856-01 決定1、不可侵）、再提示されない。
