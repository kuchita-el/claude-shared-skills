...
1. **仮説読取**: `candidate-status: pending` の1件が対象。
...
例A と同じ仮説（検証は合格する）。ただし `gh issue create` が認証エラー／ネットワーク障害で失敗する状況。
...
1. **仮説読取**: `candidate-status: pending` の1件が対象。`tags: [decision-record]`。
...
1. **仮説読取**: 1件が対象。`tags: [decision-record]`。
2. **検証（型適応＝復元不能性）**:
   - 復元不能か: この決定は既に本リポジトリの ADR 記録（`CLAUDE.md`「ADR」節等）に記録済み＝リポから決定的に復元可能 → 反証条件**(a)に該当**。
