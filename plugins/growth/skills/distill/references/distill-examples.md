...
完了報告: 入力 unprocessed 4件 / 棄却0件 / 分類 判断誤り3件・環境摩擦1件 / 採用仮説2件（2 < 4。再発知見変換0件）。
...
完了報告: 入力 unprocessed 3件 / 棄却3件 / 採用仮説0件（「仮説化できる規範はありませんでした（棄却3件）」と報告。例: §9 エラー・境界処理）。
...
完了報告: 入力 unprocessed 4件 / 棄却1件 / 分類 判断誤り2件・環境摩擦1件 / 採用仮説2件（2 < 有効3 < 入力4。再発知見変換0件）。純記述が仮説に漏れず、有効分のみがクラスタ化され、各仮説に provenance・scope-hypothesis・career-hypothesis・candidate-status が付いて `candidates.md` へ upsert されることを確認する。
...
完了報告: 入力 unprocessed 3件 / 棄却0件 / 分類 判断誤り1件・環境摩擦2件 / 型内訳 behavior-diff 2件・decision-record 0件 / 採用仮説2件（うち再発知見へ変換1件）。
...
完了報告: 入力 unprocessed 1件 / 棄却0件 / 分類 判断誤り1件・環境摩擦0件 / 型内訳 behavior-diff 0件・decision-record 1件 / 採用仮説1件（再発知見変換0件）。
