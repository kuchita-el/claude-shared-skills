...
`candidates.md` が消失した場合（重複排除の参照源が失われた）、§9 に従いカーソルを**先頭**へ巻き戻し `captures.md` 全体から再導出する。**H-1 と同一の観測**（`07-08T10:00` の一括フォーマット摩擦）を入力に、candidates.md の有無だけが挙動を分けることを示す。
...
入力（captures.md、巻き戻し後カーソルより新しい1件。H-1 と同一 timestamp の観測）:
...
1. **消失検知＋巻き戻し（§9）**: `candidates.md` 不存在 → 参照源喪失。カーソルを先頭（`2026-01-01T00:00:00Z`）へ巻き戻し `captures.md` 全体から再導出する。
