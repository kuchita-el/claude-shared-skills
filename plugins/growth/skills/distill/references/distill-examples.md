...
| 判断群 signal を `decision-record` 型へ整形し、behavior-diff 要求・N 再発を免除して4欄で保存する（ADR-202607010734-01 D4） | 例E |
...
| distiller 改善時・candidates.md 消失時にカーソルを保持集合の先頭（retention horizon 先頭）へ巻き戻して再導出し、`rejected` 不可侵で棄却済み同一仮説を pending 復活させない（B3・B5） | 例H |
| カーソル欠損時に保持集合の先頭全走査へ劣化し、処理後に distill-state.md を新規作成する（欠損フォールバック。B4） | 例I |
...
## 例E: 判断知の decision-record 化（ADR-202607010734-01 D4）
...
3. **優先度付け（§4）**: 知識型＝`decision-record`（判断知）→ 高優先（復元不能性・ADR-202607010734-01 決定1）。痕跡種別（`origin`）は優先度に用いない。
...
6. **台帳突合（§7）**: `decision-record` は **N 再発カウント免除**。§7 を通さず §6 整形のまま §8 へ送る（既にリポに記録済みか＝復元可能性の検証は promote の型適応検証の責務。ADR-202607010734-01 D5）。
...
distiller 改善時（candidates.md は健在）・candidates.md 消失時の2つの巻き戻しトリガー（§2.1 巻き戻し・§9）を検証する。巻き戻し再導出でも provenance 導出が live 候補（`promoted`/`pending`）の重複を止め、`candidate-status: rejected` 不可侵（§7.4・ADR-202607200856-01 決定1）が棄却済み**同一**仮説の pending 復活を止める。
...
1. **巻き戻し（§2.1）**: 開発者が改善後カーソルを保持集合の先頭相当（retention horizon 先頭）へ巻き戻す。
...
distill-state.md（消失検知で保持集合の先頭へ巻き戻し後）:
...
2. **候補スライス（有界化）**: 保持集合の先頭へ巻き戻し → 保持バケット群の全観測（1件）がスライス。
...
カーソル（`distill-state.md` またはその `- distill-cursor:` 行）が欠損している場合、「保持集合の先頭（retention horizon 先頭）」を既定として一度だけ保持バケット群を全走査へ劣化し（データ損失なく安全に劣化）、処理後に `distill-state.md` を新規作成することを検証する（§2.1 手順4・§9 欠損時既定）。
...
1. **カーソル読取（§2.1 手順4・§9）**: `distill-state.md` 不存在 → カーソル欠損。「**保持集合の先頭**（retention horizon 先頭）」を既定とし、一度だけ保持バケット群を全走査する（エラーにしない）。
...
完了報告: 処理源2件（カーソル欠損のため保持集合の先頭全走査）/ 棄却0件 / 型内訳 behavior-diff 2件・decision-record 0件 / 優先度 2件とも N=1（単発・同列） / 採用仮説2件（再発知見変換0件）/ 前進後カーソル 2026-07-11T10:00:00Z（distill-state.md 新規作成）。
...
### ケースK-2: guarantee-once（未 distill の間は削除しない・処理後は horizon 超なら削除）

distiller が長期間未実行（またはカーソルが過去へ巻き戻された）で、horizon を超えた古いバケットに未 distill エントリが残る場合、そのバケットが**未処理の間は `rm` されない**こと、および**同一 run で処理された後は horizon 超なら削除される**ことを検証する（AC3・guarantee-once 非退行）。guarantee-once の要点は「未処理観測を齢だけで無音脱落させない（最低1回は distill を通す）」であり、処理後の猶予期間を設けるものではない。
...
3. **経年削除の評価（カーソル前進後・同一 run）**:
   - `captures-2026-03-01.md`: (a) `03-01 ≤ 04-01`〔前進後カーソル〕✓ ∧ (b) `03-01 < 05-12` ✓ → **rm**。
   - `captures-2026-04-01.md`: **処理前**（カーソル 03-15 時点）は最大見出しキー `04-01 > 03-15` で drop 条件(a) 不成立 → **もし本 run で処理していなければ保持**（guarantee-once 保護）。しかし手順2 で処理源として仮説化しカーソルを `04-01T09:00` へ前進させた**後**は、(a) `04-01T09:00 ≤ 04-01T09:00` ✓ ∧ (b) `04-01 < 05-12` ✓ → **同一 run の経年削除で rm される**（処理済み・horizon 超は保持集合〔直近 M 日 ∪ 未 distill 含む全バケット〕のいずれにも該当しないため）。

> **guarantee-once の核心**: バケットが rm されうるのは、その最大見出しキーがカーソル ≤ になった後＝**distill が少なくとも1回そのバケットのエントリを処理源として通した後**に限る。未 distill エントリ（見出しキー > カーソル）を含む限り最大見出しキー > カーソルとなり drop 条件(a) を満たさず、horizon 超でも保持される。**もし本 run で distill が起動しなければ**（例: distill 未実行）、04-01 は未 distill のまま次に distill が走るまで削除されない。distill が走れば必ず「処理源として仮説化 → （horizon 超なら）削除」の順で進み、未処理観測が齢だけで無音脱落することはない（ADR-202607121331-01 制約〔ADR-202607111225-01 決定2 を restate〕維持）。

**期待結果**: `captures-2026-04-01.md` は本 run で処理源として仮説化された後、同一 run の経年削除で `captures-2026-03-01.md` とともに rm される（**計2バケット削除**）。処理前は guarantee-once で保護され、horizon 超という理由だけで未処理のまま削除されることはない。処理後は horizon 超のため保持集合外となり猶予なく削除される（retention の再導出射程は horizon 先頭までゆえ、horizon 超の処理済み観測を残す必要がない。ADR-202607121331-01 決定2）。

完了報告: 処理源1件（04-01・未 distill）/ 採用仮説1件 / 前進後カーソル 2026-04-01T09:00:00Z / **経年削除 2バケット（captures-2026-03-01.md・captures-2026-04-01.md／ともに horizon 60日超・処理後に全通過済み）**。

---

## 例L: セグメント跨ぎのカーソル・見出しキー一意性（AC6）

複数の日付バケットに跨る観測列で、単一 `distill-state.md` カーソルと各バケット見出しキーが**同一 ISO 8601 キー空間**で辞書順比較され、provenance 参照が一意に定まること、同一 UTC 日の複数観察が同一バケットに `-NN` サフィックスで同居することを検証する（AC6・ADR-202607112340-01）。
...
2. **per-entry カーソル比較（§2.1 手順5・辞書順・同一キー空間）**: カーソル `2026-07-10T14:30:00Z-01` と各見出しキーを**文字列辞書順**で比較する（見出しキーは `Z` 直後に `-NN` が付き厳密 ISO 8601 日時ではないため日時 parse せず辞書順。personal-store-spec.md「カーソルの格納場所」・ADR-202607112340-01）。
