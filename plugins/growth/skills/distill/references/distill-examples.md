...
> **guarantee-once の核心**: バケットが rm されうるのは、その最大見出しキーがカーソル ≤ になった後＝**distill が少なくとも1回そのバケットのエントリを処理源として通した後**に限る。未 distill エントリ（見出しキー > カーソル）を含む限り最大見出しキー > カーソルとなり drop 条件(a) を満たさず、horizon 超でも保持される。未検証観測が齢で無音脱落しない（ADR-20260712 制約 ＝ ADR-20260711-2 決定2 維持）。
