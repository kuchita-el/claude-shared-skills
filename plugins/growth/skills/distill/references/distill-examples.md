...
3. **分類と重み付け**: エントリ1〜3 は `user-utterance` → 判断誤り（高優先）。エントリ4 は `tool-result` → 環境摩擦（低優先）。エントリ3・4 は後段で1候補へ畳まれ、混在クラスタは `user-utterance` を含むため**判断誤り（高優先）**に倒す（procedure §4.1 混在クラスタ）。
4. **トリガー×振る舞い差分の推論**:
...
完了報告: 入力 unprocessed 3件 / 棄却0件 / 分類 判断誤り1件・環境摩擦2件 / 採用候補2件（うち再発知見へ変換1件）。
