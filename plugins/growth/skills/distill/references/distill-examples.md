...
> **guarantee-once の核心**: バケットが rm されうるのは、その最大見出しキーがカーソル ≤ になった後＝**distill が少なくとも1回そのバケットのエントリを処理源として通した後**に限る。未 distill エントリ（見出しキー > カーソル）を含む限り最大見出しキー > カーソルとなり drop 条件(a) を満たさず、horizon 超でも保持される。**もし本 run で distill が起動しなければ**（例: distill 未実行）、04-01 は未 distill のまま次に distill が走るまで削除されない。distill が走れば必ず「処理源として仮説化 → （horizon 超なら）削除」の順で進み、未処理観測が齢だけで無音脱落することはない（ADR-20260712 制約〔ADR-20260711-2 決定2 を restate〕維持）。

**期待結果**: `captures-2026-04-01.md` は本 run で処理源として仮説化された後、同一 run の経年削除で `captures-2026-03-01.md` とともに rm される（**計2バケット削除**）。処理前は guarantee-once で保護され、horizon 超という理由だけで未処理のまま削除されることはない。処理後は horizon 超のため保持集合外となり猶予なく削除される（retention の再導出射程は horizon 先頭までゆえ、horizon 超の処理済み観測を残す必要がない。ADR-20260712 決定2）。
