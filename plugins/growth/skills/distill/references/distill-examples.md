...
各例が検証するケース（TC 番号は実装プラン `docs/plans/issue-347-2.md` のテストケース対応表に対応）:

| TC | 検証するケース | 例 |
|---|---|---|
| TC2 | 表層の語彙差を無視して同一トリガー×振る舞い差分を畳む | 例A |
| TC3 | 同一 `signal` でも振る舞い差分が異なれば畳まない | 例A |
| TC5 | 純記述的な観察を棄却する | 例B |
| TC6 | 実行不能・空の観察を棄却する | 例B |
| TC11 | 重複＋純記述＋有効の混在から有効分のみクラスタ化する | 例C |
