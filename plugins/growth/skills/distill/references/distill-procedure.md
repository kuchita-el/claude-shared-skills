...
> **Phase 1 の既知の制約（冪等性）**: Phase 1 では `status` を `promoted` へ反転する主体（後続の昇格処理）がまだ存在しない（personal-store-spec.md「状態管理」）。このため distill を再実行すると同じ `unprocessed` 群を再処理し、同一候補を再提示する（`status` 軸の冪等性が効かない）。これは候補化止まりという責務境界の帰結として許容する。`status` 軸の冪等性は、後続 Phase で昇格処理（`status` 反転）が実装された時点で効くようになる。
