...
- **目的**: 個人ローカル store に溜まった未処理の生観察を、クラスタ化・重複排除し、実行可能な振る舞い差分（規範）の**仮説**へ変換する（DESIGN.md §3 Distill・原理1）。あわせて観察を出所から分類・重み付けし、既存ルール台帳と突合して `candidates.md` の信号対雑音比を高める（#417）。
...
| 出所（`origin`） | 分類 | 含意 |
|---|---|---|
| `user-utterance` | **判断誤り（学び）** | ユーザー発話由来。当方の判断・提案の誤り。文脈依存の振る舞い改善であり、学びとして価値が高い仮説 |
| `tool-result` | **環境摩擦** | ツール結果由来。権限・hook・コマンド形式等、環境との摩擦。決定的・静的ルールで解消できる仮説 |
...
- `origin` 欄が無い旧スキーマのエントリ（#416 以前）に遭遇した場合は、分類を `判断誤り`（高優先側）に倒して取りこぼしを防ぐ（環境摩擦と誤って低優先化・畳み込むより、仮説化して下流に委ねる方が安全側）。
...
- **混在クラスタの分類**: §5 のクラスタ化で出所の異なる観察が1仮説へ畳まれた場合（例: `user-utterance` の訂正と `tool-result` の失敗が同じトリガー×差分）、構成観察に **`user-utterance` が1件でも含まれれば判断誤り（高優先）** とする。ユーザーが一度でも明示訂正した規範は学びの重みを持つため、安全側（高優先）に倒す。

### 4.2 重み付け（ランキング）

- **判断誤り（`user-utterance` 由来の訂正）を高優先、環境摩擦（`tool-result` 由来のリトライ）を低優先**にランキングする。
- ランキングは仮説リストの**出力順位**として実現する（§8。高優先＝先頭）。分類結果は別メタ欄として candidates.md に永続化しない（`origin` から provenance 経由で常に再導出できるため。永続化は drift を生む）。代わりに出力順位と完了報告の分類内訳で観測可能にする。
- **分類の記録形式（A′設計）**: 本スキルは分類結果を candidate のメタ欄に永続化せず、**出力順位（§8）＋完了報告の分類内訳（§8）＋worked example（distill-examples.md 例D）**で分類を観測可能・追跡可能にする形で記録する。仮説ごとの分類は provenance→`origin` で常に再導出できるため監査可能であり、永続タグを置かないことで drift を防ぐ。この方式は ADR-20260629 の責務境界（distill の判断を軽く保つ）とも整合する。摩擦は「仮説化しない」のではなく低優先化・再発知見化（§7）で扱うため、分類の確認は永続フィールドでなくこの観測経路で行う。
- **環境摩擦の畳み込み・低優先化**: 環境摩擦は (a) §7 の台帳突合で既存ルールに一致すれば**再発知見へ畳み込む**、(b) 一致しなければ低優先仮説として末尾に置く、(c) §3 の合否境界に触れるノイズは棄却済み、で扱う。落とすのは (c) の場合に限る（§3）。
