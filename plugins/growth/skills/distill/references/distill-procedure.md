...
|---|---|---|---|
| 1 | 候補が **強キャリア**（hook / lint / test / 型 / CI・skill / CLAUDE.md）へ構造変換可能である（テキスト規範より強い構造的キャリアへ畳める。機構強制／モデル媒介の区別と定義は DESIGN §3「強キャリア」） | `強キャリア` | 当該キャリアを実装する repo（多くは配布元プラグイン repo）。テキスト規範ではなく構造へ変換する |
