...
- **分類は軽い判定にとどめる（#417・責務境界）**: 観察の分類（環境摩擦 / 判断誤り）は**知識型（`behavior-diff` / `decision-record`）で決める軽い判定**で行い、痕跡種別（`origin`）は優先度に用いない（ADR-20260701 決定1）。内容で重い価値判断（本物か・配布価値があるか）はしない（§4）。既存ルールとの突合も「既知 / novel」までしか判定せず、予測力・配布価値の検証には踏み込まない。後者は promote の責務である（§7・ADR-20260629）。
...
2. **`behavior-diff`（摩擦知）→ 再発 N で決まる**。§7 の台帳突合・N 再発カウントで扱い、単発は低優先、既知ルールの反復違反が積もれば「配布ルールが効いていない」知見へ昇格する（#417・ADR-20260629）。
...
- ランキングは仮説リストの**出力順位**として実現する（§8。高優先＝先頭）。分類結果は別メタ欄として candidates.md に永続化しない（知識型（`tags`）＋再発 N（§7）から常に再導出できるため。永続化は drift を生む）。代わりに出力順位と完了報告の分類内訳で観測可能にする。
- **分類の記録形式（A′設計）**: 本スキルは分類結果を candidate のメタ欄に永続化せず、**出力順位（§8）＋完了報告の分類内訳（§8）＋worked example（distill-examples.md 例D）**で分類を観測可能・追跡可能にする形で記録する。仮説ごとの分類は知識型（`tags`）＋再発 N（§7）で常に再導出できるため監査可能であり、永続タグを置かないことで drift を防ぐ。この方式は ADR-20260629 の責務境界（distill の判断を軽く保つ）とも整合する。摩擦は「仮説化しない」のではなく低優先化・再発知見化（§7）で扱うため、分類の確認は永続フィールドでなくこの観測経路で行う。
