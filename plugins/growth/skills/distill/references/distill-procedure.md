...
|---|---|---|---|
| 1 | 候補が **hook / lint / test / 型 / CI（機構強制）または skill / CLAUDE.md（モデル媒介）へ構造変換可能**な強キャリアである（テキスト規範より強い構造的キャリアへ畳める。機構強制は「二度と起こせない構造」、モデル媒介は構造化された指示だが遵守は確率的＝DESIGN §3 強キャリア） | `強キャリア` | 当該キャリアを実装する repo（多くは配布元プラグイン repo）。テキスト規範ではなく構造へ変換する |
