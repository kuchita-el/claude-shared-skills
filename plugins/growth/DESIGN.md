...
flowchart LR
    PE(["予測誤差<br/>（シグナル）"])
    OBS["観察"]
    CAND["候補（振る舞い差分）<br/>スコープ仮説 ・ キャリア仮説"]
    ADJ{{"取り込み（最終裁定）"}}
    NORM["規範（検証済みの学び）"]
    ...
    CAND -->|"昇格〔検証〕"| ADJ
    ADJ -->|"配布〔二段ゲート〕"| NORM
    NORM -.->|"測定 → 撤回（忘却 / 畳み込み）"| NORM
```

凡例: 角丸＝起点（予測誤差）／四角＝知識の状態（観察・候補・規範）／六角＝裁定点／実線＝ドメイン動作・〔 〕＝ゲート／破線＝撤回ループ。
...
| ドメイン概念 | 実装の担い手 |
|---|---|
| 捕捉 → 観察 | `capture` → `captures.md`（`status`） |
| 蒸留 → 候補 | `distill` → `candidates.md`（`candidate-status`） |
| 昇格〔検証〕 | `promote` → `gh` で Issue 起票（成功時に `status` 反転） |
| 取り込み（最終裁定） | GitHub Issue ＋ refine / DoR / PR（＝二段ゲート L2） |
