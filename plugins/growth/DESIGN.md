...
| ドメイン概念 | 実装の担い手 |
|---|---|
| 観測 → 捕捉記録 | `capture`（2検出器: 予測誤差検出器・教示信号検出器）→ `captures.md`（`status`） |
...
原理3（客観痕跡 > 主観内省）の摩擦知側の実装。痕跡はソースごとに取得時期が異なる。取得は hook によるリアルタイム検知ではなく、セッションログの事後解析を主軸とする（hook は発火タイミングと状態管理が複雑なため）。なお本表の痕跡取得は、痕跡を伴わない proactive な判断知（自発的な選好・目標表明）を構造的に取りこぼす——痕跡ベース検出のカバレッジ限界（偽陰性）である。これを capture の教示信号検出器が復元不能性を基準に別途捕捉して補う（精緻化された原理3）。
