...
1. **エンジン**: 学習ループの各段を担うスキル群。1段＝1スキルで関心を分離し、段ごとに別タイミングで起動する。
   ...
   - `distill` スキル（Distill）: store に溜まった未処理の生観察を Capture と非同期にバッチで蒸留し、クラスタ化・重複排除して実行可能な振る舞い差分の候補へ変換する（候補化止まり）。
   - 以降の段（Route / Promote / Distribute）は後続 Phase で追加する。検証を経た候補は Issue 起票（小さければ PR）として既存ワークフローに渡す。
...
4-補. **ライブ相乗り解析 UX の適否**: ユーザーのセッション実行中に割り込んで解析・提示する mid-session 型の相乗りは**不採用**とする。「ライブ相乗り」を「ユーザーのライブ作業に割り込まない、セッション境界・別時間での自発解析」と再定義し、決定事項4 の併存設計（単一＝SessionEnd hook、横断＝ローカルスケジューラ）をそのまま採用形とする。
   ...
   - **不採用の論拠**: (1) 即時性の便益は弱い。本設計の抽出精度の中核は横断解析＝原理2・6（再現したら本物 / フリート学習）であり、精度は単一セッションの鮮度ではなく複数セッション横断の再現から得る。即時性が寄与するのは候補シグナルの recall であって、検証済みナレッジの precision ではない。(2) 妨害コストは実在し、かつ横断照合前の未検証・低精度シグナルで割り込む二重ペナルティを伴う。(3) 横断一括確認は構造的にバッチ（複数セッションのログを要し週次・ローカル必須＝#379）であり、単一ライブセッションへの相乗りとは非両立。「ライブ」が意味を持つのは単一セッション内確認のみで、そこでも SessionEnd 境界での捕捉で足りる。(4) mid-session 割り込みを担う発火機構は #379 で確立していない。
   ...
   - **将来の見直し条件（Phase 3 実測）**: 本判断は実プロダクト・実ユーザー不在の現時点での演繹（原理・制約・#379 の機構非対称性からの論証）であり、Phase 3 で次を実測して再評価する。(a) SessionEnd 境界での提示で文脈鮮度が実用上十分か。(b) mid-session 提示の便益が妨害コストを上回る特定ユースケースが実在するか（例: 危険操作の即時検知。ただしこれは real-time guard 機能であり capture/reflect パイプラインとは別物＝現スコープ外）。(b) が実証された場合に限り、限定的な live 提示を再導入しうる。
