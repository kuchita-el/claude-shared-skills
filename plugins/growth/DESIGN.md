...
```
  ...
  - **`status`**: `unprocessed`（未処理）→ `promoted`（昇格済み）の2値。promote が起票成功後に反転する。この `status` は distill の**再走査抑止フラグ**を兼ね、`promoted` の観測記録は distill が再走査しない（[`references/personal-store-spec.md`](references/personal-store-spec.md)）。
