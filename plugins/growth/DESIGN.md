...
この「種別」軸（dev-workflow 改善 / 汎用ルール）は、学び置き場内の**共有境界軸**（パブリック / 閉じた ＝ universal / project-local。後述「母集団 × 適用範囲マトリクス」）とは直交する。本表下段「汎用ルール → growth 学び置き場」を、[`references/learning-store-spec.md`](references/learning-store-spec.md) が 2空間（共有境界）へ細分して具体化する（#344）。
