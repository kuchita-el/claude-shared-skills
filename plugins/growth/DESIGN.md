...
1. **振る舞い差分を保存する（記述ではなく規範）**: 保存対象は「気付き」ではなく「次回どう違う行動を取るか」という実行可能な差分。実行不能な内省は捨てる。これにより肥大化と腐敗を構造的に防ぐ。ただし例外として、選好・却下理由・目標表明・設計判断（判断知）は、振る舞い差分へ畳めない一回性の決定であっても文脈付き決定知（decision-record）として保存する（カウントでなく決定の記録として残す。詳細は §3 distill 出力契約）。
2. **気付きは仮説であり、検証されるまで配布しない**: 内省は幻覚しうる。未検証の気付きを全利用者へ配布することは、毒を配ることに等しい。仮説は予測を伴い、次の機会で検証され、支持されて初めて昇格する。
3. **客観痕跡 > 主観内省（痕跡ベース検出の偽陰性は復元不能性で補う）**: 自己報告は作話（confabulation）でありうる。摩擦知（環境摩擦・手続違反）については、より信頼できる教師信号は外部の客観痕跡——git revert、CI 失敗、同一指示の反復、ツール拒否、再試行の連続——であり、痕跡計測で必要十分である。判断知（選好・却下理由・目標表明・設計判断）のうち reactive なもの（当方の出力への応答として現れる訂正・理由付き却下など）は会話に痕跡を残し痕跡ベースでも捉えうるが、proactive な表明（自発的な選好・目標の宣言）は会話に痕跡を伴わないことがあり、痕跡ベースの検出器はこれを構造的に取りこぼす。これは判断知そのものが痕跡を持たないという型の性質ではなく、痕跡ベース検出の**カバレッジの限界（偽陰性）**である。この偽陰性を埋めるため、痕跡の有無と独立な**復元不能性**（リポジトリ・ハーネスから後で復元できないか）を直交基準として併用する。内省システムは「痕跡を計測する」を主軸に置きつつ、痕跡ベース検出が取りこぼす判断知を復元不能性で補足する。
...
凡例: 角丸＝起点（学習の契機）／四角＝知識の状態（観測記録・候補・規範）／六角＝裁定点（制度化＝人による二段ゲート L2）／〔 〕＝動作に伴うゲート／破線＝撤回ループ。

上図がドメインモデルの正典（ユビキタス言語）。知識の状態・ドメイン動作・ゲートを domain 語で一箇所に固定し、実装の担い手は下表へ射影する。学習シグナルの価値は単一でなく2軸——**摩擦知**（環境摩擦・手続違反）と**判断知**（選好・却下理由・設計判断）——で、型は観測記録・候補を貫く **`type` 属性**として運ばれる（fork でなく同一流路・共有ライフサイクル。ADR-20260701）。1観測が両型にまたがる混在ゾーンの partition 硬度は非排他 tag（多値 set）で確定し、`candidates.md` の `type`（単値）→ `tags`（多値 set）改称を設計決定として記録した（決定事項10・#440）。

| ドメイン概念 | 実装の担い手 |
|---|---|
| 観測 → 観測記録 | `capture`（2検出器: 予測誤差検出器・教示信号検出器）→ `captures.md`（`status`） |
| 仮説形成 → 候補 | `distill` → `candidates.md`（`type: behavior-diff｜decision-record`・`candidate-status`） |
...
- **キャリア仮説（career-hypothesis）**: 仮説形成（distill）が候補へ付与する「配布先キャリア ＋ 宛先 repo」の仮説タグ（`<career> / repo: <宛先 repo 仮説>` の1行）。スコープ仮説と対称・直交。distill 時点では確証せず、キャリアと宛先 repo の最終裁定は集約点（取り込み Issue）が担い、promote はルーティング不可知に運ぶ（ADR-20260628-2）。4分類の判定表は [`skills/distill/references/distill-procedure.md`](skills/distill/references/distill-procedure.md)（「career-hypothesis の判定（決定表）」）を SoT とする。

### 保存設計

学習ループを流れるデータは 生記録 → 実行可能な差分 → 配布規範 と姿を変える。中核語彙は **観測記録**（旧 観察 / `observation`。観測が記録した生記録＝解釈・原因分析・対策を含まない事実のみ）・**振る舞い差分**（「次回どう違う行動を取るか」を一文で表す実行可能な差分。原理1。観測記録 → 候補 → learnings.md へ受け継がれる学びの核）・**候補**（仮説検証待ちの振る舞い差分。`candidates.md` のエントリ）・**規範**（それを命じる learnings.md の見出し一文＝テキストとして配布される最も弱いキャリア）。これらが載る成果物は個人ローカル〜配布物にまたがる3つ:

| 成果物 | 役割 | パス（SoT） |
|---|---|---|
| `captures.md`（観測置き場） | 観測の書き込み先・仮説形成の入力源 | `~/.claude/projects/<project-id>/growth/captures.md`（personal-store-spec.md） |
| `candidates.md`（候補ファイル） | 仮説形成の出力先・仮説検証の入力源。観測置き場でも learnings.md でもない第3の成果物 | 個人ローカル（personal-store-spec.md） |
| `learnings.md`（学び置き場） | 検証済みの汎用振る舞い規範を集約した配布物（1欄スキーマ） | `plugins/growth/learnings.md`（learning-store-spec.md） |

各成果物はカタカナ「ストア」や「配布物 / 最終置き場」等の言い換えを避け、上記の名称・ファイル名で統一する。
...
- **個人 store のエントリ状態（実装層の正典モデル）**: 個人 store の各エントリは実装層の状態フィールドを持つ。`captures.md` は `status`、`candidates.md` は `candidate-status` を持ち、両者を `provenance`（由来）が結ぶ。promote（仮説検証）が Issue 起票に成功すると、同一 `provenance` で両ファイルの状態を同時に前進させる。値のスキーマ詳細は [`references/personal-store-spec.md`](references/personal-store-spec.md) が SoT（#345）で、下図は状態遷移の正典。

```mermaid
stateDiagram-v2
    direction LR

    state "captures.md（status）" as CAP {
        [*] --> unprocessed
        unprocessed --> promoted: promote 起票成功で反転
    }

    state "candidates.md（candidate-status）" as CAND {
        state "promoted" as cpromoted
        [*] --> pending: distill が生成
        pending --> rejected: promote が棄却
        pending --> cpromoted: promote 起票成功
    }
```

  - **`status`**: `unprocessed`（未処理）→ `promoted`（昇格済み）の2値。promote が起票成功後に反転する。
  - **`candidate-status`**: `pending`（検証待ち・既定）/ `rejected`（promote が棄却）/ `promoted`（promote が起票成功後に付与する任意・推奨値）の3値。
  - **`provenance`（由来）**: 候補の出自を追跡するメタ。値は `captures.md` の `## <timestamp>` 見出し。distill の upsert・promote の状態反転で同一性判定に使う（避ける語: 出自キー・由来参照）。

### 客観痕跡の取得

原理3（客観痕跡 > 主観内省）の摩擦知側の実装。痕跡はソースごとに取得時期が異なる。取得は hook によるリアルタイム検知ではなく、セッションログの事後解析を主軸とする（hook は発火タイミングと状態管理が複雑なため）。なお本表の痕跡取得は、痕跡を伴わない proactive な判断知（自発的な選好・目標表明）を構造的に取りこぼす——痕跡ベース検出のカバレッジ限界（偽陰性）である。これを capture の教示信号検出器が復元不能性を基準に別途観測して補う（精緻化された原理3）。
...
1. **エンジン**: 学習ループの各段を担うスキル群。1段＝1スキルで関心を分離し、段ごとに別タイミングで起動する。
   ...
   - `distill` スキル（Distill＋Route 統合）: store に溜まった未処理の生観察を Capture と非同期にバッチで仮説形成し、クラスタ化・重複排除して候補へ変換する。出力は知識型に応じた2系統——摩擦知は実行可能な振る舞い差分（`type: behavior-diff`）、判断知は文脈付き決定知（`type: decision-record`＝decision / rejected-alternatives / rationale / context。behavior-diff 要求と N 再発カウントを免除）。各候補に**スコープ仮説タグ**（`project-local` / `universal`）・**career 仮説タグ**（昇格先キャリア＋宛先 repo の仮説）・provenance を付与し、候補ファイル（`candidates.md`）へ永続化する（Route をタグ付与として統合。scope・career とも仮説どまりで、確定（裁定）は集約点に置く。候補永続化までで責務を終える）。
   - `promote` スキル（Promote・新規）: `candidates.md` の候補を仮説として検証し（原理2＝未検証を配布経路に乗せないフィルタ。検証は `type` で適応し、behavior-diff は予測・反証、decision-record は「復元不能で・まだ有効で・配布価値があるか」を検査する）、検証通過候補を `gh` で**自動起票**（起票前ゲートなし）して既存ワークフローへ疎結合に渡す。起票成功後に store（`captures.md`）の `status` を `unprocessed → promoted` へ反転する。promote は**ルーティング不可知**で、distill の scope・career 仮説を昇格 Issue 本文へ注記として運ぶのみ（career の裁定は行わない）。L2 承認は起票後の既存ワークフロー（refine-issue / DoR / PR レビュー）が担う。
...
3. **個人 store の置き場**: 生の観測は個人ローカル（共有されない）に置き、検証を経たものだけ committed へ昇格させる。昇格経路を必ず持つことで memory の「個人で終わる」欠点を克服する。具体形式・置き場（`~/.claude/projects/<project-id>/growth/captures.md`、per-project）・状態管理は #345 で確定（[`references/personal-store-spec.md`](references/personal-store-spec.md)）。
...
8. **Promote 段の段マッピングと検証→起票モデル**（#348）:
   ...
   - **Route を `distill` へ統合する**。仮説形成は「どの観点（project-local / universal）で仮説形成するか」の視点を本質的に要するため、スコープ仮説タグの付与は Distill と密結合（[`references/learning-store-spec.md`](references/learning-store-spec.md)「仮説形成は project-local と universal の2観点に分かれ…」が裏付け）。distill の責務境界を1段拡張し、候補を `candidates.md` へ provenance・scope 仮説タグ付きで永続化する。**career（昇格先キャリア）仮説タグの付与も同様に distill の責務とする**（scope と対称・直交。Route→distill 統合を career についても完成させる）。career の確定（裁定）は promote ではなく集約点（取り込み Issue）で行い、promote はルーティング不可知に運ぶのみとする（ADR-20260628-2）。
...
10. **混在ゾーンの partition 硬度＝非排他 tag**（#440。ADR-20260705-2 decision 2 が委譲した未決事項を確定）:
   ...
   本決定事項は自セクションを ADR-20260705-2 の現行正典語彙（観測＝capture / 仮説形成＝distill / 仮説検証＝promote / 仮説＝候補 / スコープ・キャリア）で記述する。他セクション（§3 図表・他決定事項）の語彙追随は #438 が担い、本決定では行わない。英語実装識別子（`candidates.md`・`behavior-diff`・`decision-record`・`type`/`tags`）は据え置く（ADR-20260705-2 原則0）。
   ...
   - **partition 硬度＝非排他 tag（J1 確定）**: partition 硬度を**非排他 tag（多値 set）**で確定する。選択肢 A（単一 type へ倒す＝排他）/ B（非排他 tag）/ C（provenance 共有の2候補へ分割）のうち **B** を採る。
   ...
   - **仮説形成（distill）の evidence-gated 分岐（確定）**: 観測段の型仮説（#437「観測段の型は仮説」）を仮説形成（distill）が **evidence-gated** で確定する。既定は単一タグ（最尤の1知識型）、第2タグは**陽性証拠がある時のみ**付与し、**既定 both（無条件の両タグ付与）を禁ずる**。
     ...
     - ADR-20260701 §4「distill 出力2系統分離」は**不変**: 系統数は2のまま、両タグ候補が両系統に入る＝系統メンバーシップが関数（1候補→1系統）から集合関係（1候補→1〜2系統）になるのみ。
   ...
   - **整合確認（AC4）**: 本決定（partition 硬度＝非排他 tag）が上流前提・既存 ADR と整合することを各該当節参照つきで確認した。
     ...
     - **(3) ADR-20260701 §1（価値2軸）/§4（出力2系統分離）**: tag 採用が二重損失（平板化 or N 再発喪失）を回避し、§4 の系統数2が不変（両タグ候補が両系統に入り、系統メンバーシップが関数から集合関係になるのみ）。
     ...
     - **軸区別（J3・前提確認クリア）**: #440 の混在ゾーン（**知識型軸**: behavior-diff / decision-record）は、distill-procedure.md §4.1 の既存「**混在クラスタ**」（**出所軸**: user-utterance / tool-result）と直交する別概念。前者は「1観測が両知識型にまたがる」、後者は「出所の異なる観察が1候補へ畳まれる」。両者は独立軸であり互いを置換しない。
