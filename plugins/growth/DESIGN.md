...
flowchart LR
    SL[("session jsonl")]
    CAPS[("captures.md<br/>観察 ・ status")]
    CAND[("candidates.md<br/>候補 ・ candidate-status<br/>scope仮説 ・ career仮説")]
    ISS[/"GitHub Issue<br/>取り込み＝最終裁定"/]
    LRN[("learnings.md<br/>規範 ・ 1欄")]

    SL -->|capture / P1| CAPS
    CAPS -->|distill / P1| CAND
    CAND -->|"promote / P1 〔検証ゲート〕"| ISS
    ISS -->|"distribute / P2 〔L2: refine / DoR / PR〕"| LRN
    ISS -.->|promote: status反転| CAPS
```

凡例: 箱＝成果物（状態・タグを内包）／実線＝スキル（P＝Phase, 〔 〕＝ゲート）／破線＝副作用。

上図が正典。成果物の流れ・各成果物の状態欄・distill が付与する2仮説タグ・2ゲートの位置（検証＝起票前／L2＝起票後）を一箇所に固定する。配布後のフィードバック（measure / retire・fan-out / fan-in・忘却 / 畳み込み）は前方フローと性質が異なるため本図に含めず、トポロジ図（第2の図、未作成）と決定事項7／[`references/learning-store-spec.md`](references/learning-store-spec.md) が扱う。以下は図が表せない理由・制約の補足のみ:

- **capture のシグナル**: 予測誤差は 訂正 / ツール拒否 / 反復試行 / 期待違反 / 客観痕跡（revert・CI fail 等）の5種別。判断は後回しにし観察のみ記録する。
- **2仮説タグを確証しない理由**: scope（`project-local` / `universal`）・career とも Phase 1 では経験的に確証できず仮説のまま終端する。最終裁定は集約点「取り込み Issue」（ADR-20260628-2）。
- **起票前ゲートを置かない理由**: 検証段（原理2）が未検証候補のフィルタとして残るため。L2 の仕組み化ゲートは起票後の既存ワークフロー（refine-issue / DoR / PR）が担い、下流ゲートとの二重化を避ける。
