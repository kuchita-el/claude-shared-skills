...
凡例: 角丸＝起点（予測誤差シグナル）／四角＝知識の状態（捕捉記録・仮説・規範）／六角＝裁定点（制度化＝人による二段ゲート L2）／実線＝ドメイン動作・〔 〕＝動作に伴うゲート／破線＝撤回ループ。

上図がドメインモデルの正典（ユビキタス言語）。知識の状態・ドメイン動作・ゲートを domain 語のみで一箇所に固定する。実装の担い手（スキル・ファイル・機構）はドメインから分離して下表に射影する。実装フェーズは「作るものの定義」でなく配信スケジュールのため本節に持たず、§5 ロードマップが持つ。

| ドメイン概念 | 実装の担い手 |
|---|---|
| 捕捉 → 捕捉記録 | `capture` → `captures.md`（`status`） |
| 蒸留 → 仮説 | `distill` → `candidates.md`（`candidate-status`） |
| 昇格〔検証〕 | `promote` → `gh` で Issue 起票（成功時に `status` 反転） |
| 制度化（二段ゲート L2） | `intake` → GitHub Issue ＋ refine / DoR / PR |
| 配布 → 規範 | `distribute` → `learnings.md`（1欄） |
| 測定 → 撤回 | 使用台帳 → 集約ダイジェスト（fan-in） |

以下は図・表が表せない理由・制約の補足のみ:

- **capture のシグナル**: 予測誤差シグナルは 訂正 / ツール拒否 / 反復試行 / 期待違反 / 客観痕跡（revert・CI fail 等）の5種別。判断は後回しにし、解釈を加えず捕捉記録として残す。
- **2仮説タグを確証しない理由**: scope（`project-local` / `universal`）・career とも Phase 1 では経験的に確証できず仮説のまま終端する。最終裁定は集約点＝制度化（`intake` が起こす Issue。ADR-20260628-2）。
- **起票前ゲートを置かない理由**: 検証段（原理2）が未検証仮説のフィルタとして残るため。L2 の仕組み化ゲートは起票後の既存ワークフロー（refine-issue / DoR / PR）が担い、下流ゲートとの二重化を避ける。
- **measure / retire**: 観測値は配布ファイルに持たせず、各コンシューマのローカル使用台帳→集約ダイジェスト（fan-in）に置く。撤回は origin 側 learnings.md からの物理除去（中間の降格状態を配布ファイルに持たない）。発火観測・使用台帳の事後解析再構成は決定事項7／[`references/learning-store-spec.md`](references/learning-store-spec.md)。
- **用語移行中（本節が現行正典）**: 上図・射影テーブルの domain 語（予測誤差シグナル・捕捉記録・仮説・制度化）が用語の現行正典。旧語（観察・候補・取り込み）を使う §6 決定ジャーナル・各スキル・spec・ADR-20260628-2、および `intake` スキルの改名・`キャリア`／`スコープ` の日本語化は、ユビキタス言語移行として follow-up（PR 本文参照）。実装識別子（`captures.md`・`candidates.md`・`candidate-status`・各スキル名）は据え置き、射影テーブルが domain 語↔識別子の対応を担う。
...
1. **エンジン**: 学習ループの各段を担うスキル群。段・担当スキル・入出力成果物・付与タグ・ゲート位置は **§3 学習ループ図が正典**（1段＝1スキルで関心分離、段ごとに別タイミング起動）。各スキルの責務境界・status反転・ルーティング不可知性は §3 補足と決定事項8 を参照。図に出ない spec ポインタのみ以下に残す:
   - 配布（Phase 2）の昇格 Issue テンプレート: [`references/promotion-issue-spec.md`](references/promotion-issue-spec.md)（#382。#383 の career 決定モデル ADR-20260628-2 に合わせ再定義予定）。
   - 出来事ベースの昇格 Issue を `learnings.md` の1欄エントリへ翻訳する変換規約: [`references/learning-promotion-spec.md`](references/learning-promotion-spec.md)（#383）。
