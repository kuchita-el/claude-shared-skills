...
3. **客観痕跡 > 主観内省（痕跡を残さない判断知は復元不能性で拾う）**: 自己報告は作話（confabulation）でありうる。摩擦知（環境摩擦・手続違反）については、より信頼できる教師信号は外部の客観痕跡——git revert、CI 失敗、同一指示の反復、ツール拒否、再試行の連続——であり、痕跡計測で必要十分である。しかし判断知（選好・却下理由・目標表明・設計判断）は痕跡を一切残さない proactive な表明でありうるため、痕跡ベースの検出器には構造的に不可視である。これに対しては痕跡の有無と独立な**復元不能性**（リポジトリ・ハーネスから後で復元できないか）を直交基準として用いる。内省システムは「痕跡を計測する」を主軸に置きつつ、痕跡を残さない判断知を復元不能性で補足する。
...
- **Capture**: 価値の異なる2種の知識を検知して生の観察を記録する。**摩擦知**（環境摩擦・手続違反）は予測誤差シグナル（訂正・ツール拒否・反復試行・期待違反・客観痕跡＝revert / CI fail 等）で拾う（これらは摩擦サブセットの補助検出器であり、ハーネスが既に強制済みの摩擦は既定除外する）。**判断知**（選好・却下理由・目標表明・設計判断）は予測誤差の形を持たないため、判断知検出子で別途拾う（価値軸は復元不能性）。判断は後回しにする。
- **Distill**: 蓄積をクラスタ化・重複排除し、知識型に応じて2系統で候補化する。摩擦知は実行可能な振る舞い差分（behavior-diff）、判断知は文脈付き決定知（decision-record＝decision / rejected-alternatives / rationale / context）。
...
原理3（客観痕跡 > 主観内省）の摩擦知側の実装。痕跡はソースごとに取得時期が異なる。取得は hook によるリアルタイム検知ではなく、セッションログの事後解析を主軸とする（hook は発火タイミングと状態管理が複雑なため）。なお判断知は痕跡を残さないため本表の痕跡取得では拾えず、capture の判断知検出子が復元不能性を基準に別途捕捉する（精緻化された原理3）。
...
1. **エンジン**: 学習ループの各段を担うスキル群。1段＝1スキルで関心を分離し、段ごとに別タイミングで起動する。
   - `capture` スキル（Capture）: 現セッションの会話履歴から摩擦知（予測誤差シグナル）と判断知（判断知検出子）の2種を検知し、生観察を個人ローカル store に記録する。
   - `distill` スキル（Distill＋Route 統合）: store に溜まった未処理の生観察を Capture と非同期にバッチで蒸留し、クラスタ化・重複排除して候補へ変換する。出力は知識型に応じた2系統——摩擦知は実行可能な振る舞い差分（`type: behavior-diff`）、判断知は文脈付き決定知（`type: decision-record`＝decision / rejected-alternatives / rationale / context。behavior-diff 要求と N 再発カウントを免除）。各候補に**スコープ仮説タグ**（`project-local` / `universal`）・**career 仮説タグ**（昇格先キャリア＋宛先 repo の仮説）・provenance を付与し、候補ファイル（`candidates.md`）へ永続化する（Route をタグ付与として統合。scope・career とも仮説どまりで、確定（裁定）は集約点に置く。候補永続化までで責務を終える）。
