...
1. **振る舞い差分を保存する（記述ではなく規範）**: 保存対象は「気付き」ではなく「次回どう違う行動を取るか」という実行可能な差分。実行不能な内省は捨てる。これにより肥大化と腐敗を構造的に防ぐ。ただし例外として、選好・却下理由・設計判断（判断知）は、振る舞い差分へ畳めない一回性の決定であっても文脈付き決定知（decision-record）として保存する（カウントでなく決定の記録として残す。詳細は §3 distill 出力契約）。
2. **気付きは仮説であり、検証されるまで配布しない**: 内省は幻覚しうる。未検証の気付きを全利用者へ配布することは、毒を配ることに等しい。仮説は予測を伴い、次の機会で検証され、支持されて初めて昇格する。
3. **客観痕跡 > 主観内省（痕跡を残さない判断知は復元不能性で拾う）**: 自己報告は作話（confabulation）でありうる。摩擦知（環境摩擦・手続違反）については、より信頼できる教師信号は外部の客観痕跡——git revert、CI 失敗、同一指示の反復、ツール拒否、再試行の連続——であり、痕跡計測で必要十分である。しかし判断知（選好・却下理由・設計判断）は痕跡を一切残さない proactive な表明でありうるため、痕跡ベースの検出器には構造的に不可視である。これに対しては痕跡の有無と独立な**復元不能性**（リポジトリ・ハーネスから後で復元できないか）を直交基準として用いる。内省システムは「痕跡を計測する」を主軸に置きつつ、痕跡を残さない判断知を復元不能性で補足する。
