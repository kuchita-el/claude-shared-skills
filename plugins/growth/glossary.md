...
### 蒸留（distill）
- **定義**: 個人ローカルの捕捉置き場の未処理の観察をクラスタ化・重複排除し、実行可能な振る舞い差分の候補へ変換する第2段。候補には2観点（project-local / universal）でスコープ仮説を付与する（スコープ仮説の付与は独立段でなく蒸留に内在する責務）。
...
- **生き残る語**: タグそのものは [[スコープ仮説（scope-hypothesis）]]。キャリア選択の [[Route判定規則]] は別概念で存続する。
...
- **避ける語**: スコープ仮説の付与（蒸留の責務。退役した「経路判定」）と混同しない。こちらはキャリア選択（昇格先媒体の決定）
