...
### 蒸留（distill）
- **定義**: 個人ローカルストアの未処理の観察をクラスタ化・重複排除し、実行可能な振る舞い差分の候補へ変換する第2段。候補には2観点（project-local / universal）でスコープ仮説を付与する（スコープ仮説の付与は独立段でなく蒸留に内在する責務）。
...
| `status` | 処理状態 | 生捕捉ストアの処理状態 → 処理状態（status） |
...
| コード識別子 | 日本語 | 意味・対応エントリ |
|---|---|---|
| `captures.md` | 生捕捉ストア | → 生捕捉ストア（captures.md） |
