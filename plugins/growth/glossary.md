...
### 捕捉（capture）
- **定義**: 現セッションの会話履歴から予測誤差シグナルを検知し、生の観察を個人ローカルの捕捉置き場に記録する学習ループの第1段。解釈・原因分析・対策は含めない。
...
- **定義**: 摩擦知の探知対象。「驚き」の源泉。訂正・ツール拒否・反復試行・期待違反・客観痕跡の5シグナル種別で表現される（摩擦サブセットの補助検出器）。判断知は予測誤差の形を持たず、別途 [[判断知検出子（judgment-knowledge detector）]] が拾う。
...
### 判断知検出子（judgment-knowledge detector）
- **定義**: 予測誤差の形を持たない判断知（選好・理由付き却下・目標/意図・設計境界）を user 発話から recall 優先で拾う capture の検出子。復元不能性・価値判定は distill/promote に委ねる。
