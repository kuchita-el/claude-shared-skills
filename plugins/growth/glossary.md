...
### 捕捉（capture）
- **定義**: 現セッションの会話履歴から学習シグナル（予測誤差検出器・教示信号検出器の2系統）を検知し、生の観察を個人ローカルの捕捉置き場に記録する学習ループの第1段。解釈・原因分析・対策は含めない。
- **使用箇所**: DESIGN.md §3・§4 / skills/capture/SKILL.md
- **避ける語**: 「reflect」（reflect/distill 分離（#347）以前の旧称。現在は capture に統一）

### 蒸留（distill）
- **定義**: 個人ローカルの捕捉置き場の未処理の観察をクラスタ化・重複排除し、実行可能な振る舞い差分の候補へ変換する第2段。候補には [[スコープ仮説（scope-hypothesis）]]（2観点 project-local / universal）と [[キャリア仮説（career-hypothesis）]]（昇格先媒体）の2つの仮説を付与する（旧「経路判定」も career の判定も独立段でなく蒸留に内在する責務。#411）。
...
### 昇格（promote）
- **定義**: 蒸留が生成した候補を検証し、検証通過分を `gh` で Issue へ自動起票して既存ワークフローへ渡す第3段（Phase 1 スキル）。候補の scope/career 仮説は確定せず注記のまま運ぶ（ルーティング不可知。#411）。最終裁定は集約点「取り込み Issue」が担う。
...
- **定義**: 「次回どう違う行動を取るか」を一文で表す実行可能な規範。捕捉置き場 → 候補 → learnings.md へ受け継がれる学びの核。
...
### 捕捉置き場（captures.md）
- **定義**: 個人ローカルに置かれる、未検証の生の捕捉（観察）を蓄積するファイル。パス `~/.claude/projects/<project-id>/growth/captures.md`。捕捉の書き込み先・蒸留の入力源。
...
### 由来（provenance）
- **定義**: 候補の出自を追跡するメタフィールド。値は捕捉置き場の `## <timestamp>` 見出し。蒸留の upsert・昇格の状態反転で同一性判定に使う。
- **使用箇所**: references/personal-store-spec.md / skills/promote/references/promote-procedure.md
...
- **定義**: 蒸留が候補に付与する共有範囲の仮説タグ。値は `universal`（全プロジェクト対象）/ `project-local`（チーム・プロジェクト限定）。確証せず仮説のまま終端する。
...
- **注記**: career は蒸留が候補へ [[キャリア仮説（career-hypothesis）]] として付与する（#405/#411、ADR-20260628-2）。promote は確定せず注記で運び（ルーティング不可知）、career と宛先 repo の最終裁定は集約点「取り込み Issue」が担う
...
- **定義**: 蒸留が候補に付与する、昇格先キャリアと宛先 repo の仮説タグ。`<career> / repo: <宛先 repo 仮説>` の1行形式。4分類（`強キャリア` / `改善還元` / `ADR 差分` / `learnings.md`）の判定表は distill-procedure.md「career-hypothesis の判定（決定表）」が単一出典。[[スコープ仮説（scope-hypothesis）]]と対称・直交（キャリア軸 ⊥ 空間軸）。distill 時点では仮説で確証せず、最終裁定は集約点「取り込み Issue」（ADR-20260628-2）。
...
### 昇格（広義）（promotion）
- **定義**: 学びが捕捉置き場 → 候補 → Issue → learnings.md へ段階的に移送される過程全体。段・スキルとしての [[昇格（promote）]] より広い。
- **使用箇所**: DESIGN.md §3
- **避ける語**: 文中で「昇格」が段（promote）と過程（promotion）のどちらを指すか曖昧なときは明示する

### 予測誤差（prediction error）
- **定義**: 摩擦知の探知対象。「驚き」の源泉。訂正・ツール拒否・反復試行・期待違反・客観痕跡の5シグナル種別で表現される（予測誤差検出器が拾う摩擦サブセット）。判断知は予測誤差の形を持たず、別途 [[教示信号検出器（judgment-knowledge detector）]] が拾う。
...
### 教示信号検出器（judgment-knowledge detector）
...
- **英語別名**: 括弧内の `judgment-knowledge detector` は暫定。日本語正典 `教示信号検出器` を先行させ（ADR-20260705-2 原則0）、英語 gloss（`teaching-signal detector` 等）の確定は完全移行 follow-up で行う。
