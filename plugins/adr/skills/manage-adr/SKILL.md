...
**commit 前ゲートとの二面構成**: 同梱の PreToolUse フックが `git commit` 時にも drift-lint を実行し、違反があれば exit 2 で commit をブロックする。本節の自己検証は、そのブロックを操作の直後に前倒しで潰す位置づけである。自己検証を飛ばしても違反は commit 時に必ず捕まるが、修正の手戻りが大きくなる。
