...
名詞末尾がう段9文字に該当するケース（例: `理由`（由）, `一覧`（覧）, `通知中`（中、ただし「中」はう段外）, `〜中`（同左））のうち、語尾文字がう段に該当するものはリント上は違反として検出されない可能性がある。具体的には:
...
ただし、う段9文字（う/く/ぐ/す/つ/ぬ/ぶ/む/る）に該当する漢字末尾の名詞（例: 漢字訓読みで「す」「く」等で終わる稀少なケース）は、機械検査では区別できない。本規約は「規約として人が守る」前提で運用し、リントはう段判定のみ機械化する。形態素解析による厳密な動詞句判定は将来課題として別途検討する。
