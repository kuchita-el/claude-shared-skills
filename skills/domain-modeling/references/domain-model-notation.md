...
コマンド・ワークフローの失敗は、出力を「成功時」と「失敗時」に分け、失敗理由を判別共用体（OR）で列挙する。失敗ケースをドメイン語彙として明示し、プロダクトチーム全体で業務フロー（判断依頼、リトライ、代替案）を合意する対象とする。
...
**ポイント**:
- 状態遷移は「ある状態 → 別の状態」の関数として表現し、**出力は状態型**
- コマンドは**外部に公開するユースケース**を表し、**出力はイベント**（状態変化の結果を過去の事実として通知）
- コマンドは内部で1つ以上の状態遷移を合成して実装される（例: `実装プランを作成する` コマンドは内部で `確定する` 状態遷移を呼び出し、その結果の確定プランを `実装プランが確定した` イベントに変換して返す）
- コマンドの失敗時出力は失敗理由、状態遷移の失敗時出力も失敗理由（両者とも失敗の扱い節の規約に従う）
...
**記述ルール**:
- **ワークフロー名は動詞句「〜する」を既定とする**。契約として何を達成するかを名前に反映する
...
- **状態遷移の制約**: OR で無効な状態組合せを排除（例: 判断依頼中プランには確定日時が存在しない）
- **必須項目**: AND で必須属性として定義
...
- `出力:` は同期的な返り値を表すラベルなので、非同期な外部コマンドでは使わない（内部コマンドと意味が混ざるのを避けるため）
...
- **ワークフローは動詞句「〜する」を既定**: 契約として達成する結果を名前に反映。例外は業務用語として定着した名詞句（`日次集計` 等）のみ
