...
上記命名規約3項目（イベント=過去形、コマンド=動詞句〔う段終止形〕、ポリシー=「〜ポリシー」接尾辞 等）はいずれも形態素解析を要する言語学的判定であり、grep ベースの近似（語尾文字一致）では原理的に誤検出・誤許容が避けられない。たとえば名詞末尾がう段9文字に該当するケース（カタカナ語・漢字訓読み等）は誤許容され、過去形を「〜した／〜された」末尾だけで判定すると受身・使役・否定形・複合動詞のたびにパッチが必要になる。
