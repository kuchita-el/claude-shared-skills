...
1. **英単語キーワードは最小限**: `AND` `OR` `入力` `イベント` `失敗時` `契機` のみ使用。`type` `of` `let` 等のプログラミングキーワードは使わない
...
| `契機: X` | 「コマンドの契機（外部指示・イベント受信・ポリシー・スケジュール）は X」（コマンド専用、値構築では使わない） | `契機: 外部指示` |
...
**用語衝突回避の補足**: `契機:` で書くのは「コマンドが受信／観測するイベント」（外部指示・イベント受信・ポリシー・スケジュールの起点）であり、`イベント:` で書くのは「コマンドが発火するドメインイベント」である。同じ「イベント」という語が双方に登場するが、観測（入力側）と発火（出力側）で役割が異なる点に注意する。
...
→ 「実装プランを作成する」は名前が動詞句（う段終止形）なので **コマンド**。外部指示を契機に「チケット」と「開発者」を受け取り、「実装プランが確定した」イベントを発火しつつ「確定プラン」状態に遷移する。Either.Left 系の同期失敗は箇条書きで「仕様不明点あり」「技術方針未決」を列挙する。
...
範囲・形式に制約がある値は、型と生成ルールをセットで書く。**値構築はドメインイベントを発火しない純粋関数**であり、コマンド（集約の状態変化を起こす契機）とは別ジャンルとして扱う。記法上もフィールド構成を分けて区別する。
...
**記述ルール**:
- フィールドは `入力:` `生成:` `失敗時:` の3種。`契機:` は使わない（値構築は集約の状態変化を起こさず外部からの起動契機を持たないため）
- `イベント:` も使わない（ドメインイベントを発火しないため）
- `生成:` で生成される値型を記述する
...
- **英単語は最小限**: `AND` `OR` `入力` `イベント` `失敗時` `契機` のみ使用
...
このため、命名規約は **本ガイドに「規約として人が守る」前提で残し、Linter（`scripts/lint-domain-doc.sh`）による機械検証からは一時撤退** する。Linter は構造検証（禁止記号・廃止記法・契機フィールド必須化等）に専念する。命名遵守は規約参照と人間レビューで補完する。
