...
| `イベント: X` | 「コマンドが発火するドメインイベント。`/` で遷移後状態を続けて並列列挙可。複数イベントを `OR` で並列列挙可。`OR` と `/` の混在は禁止」 | `イベント: 実装プランが確定した / 確定プラン`, `イベント: 検証ゲートを通過した OR 検証ゲートが失敗した` |
...
→ 「実装プランを作成する」は名前が動詞句（う段終止形）なので **コマンド**。「チケット」と「開発者」を受け取り、「実装プランが確定した」イベントを発火しつつ「確定プラン」状態に遷移する。Either.Left 系の同期失敗は箇条書きで「仕様不明点あり」「技術方針未決」を列挙する。
...
1コマンドが複数のドメインイベントを発火する、または複数のドメイン変化を引き起こす場合、`イベント:` フィールドに `OR` または `/` で列挙する。

- **`OR` で並列列挙**: コマンドの結果として発火しうるイベントを並べる（境界条件で分岐する終端事実）。例: `検証ゲートを通過した OR 検証ゲートが失敗した`
- **`/` で並列列挙**: 1つの結果に伴って同時に発生する「イベント / 遷移後状態」または「イベント / イベント」を並べる。例: `実装プランが確定した / 確定プラン`, `注文が確定した / 在庫が引当てられた`
- **`OR` と `/` の混在は禁止**: `A OR B / C` のような混在記述は許容しない。優先順位の解釈余地を排除して記法を単純に保つため。混在が必要な複雑なケースは、コマンドを分割するか、別ジャンル（ポリシー等）への切り出しを検討する

```
注文を確定する:
    入力: 注文
    イベント: 注文が確定した / 在庫が引当てられた
    失敗時:
        - 在庫不足
        - 与信不足
...
- `入力:` で受け取るデータを記述
- `イベント:` で発火イベント・遷移後状態を記述。複数イベントは `OR` で、イベント＋遷移後状態は `/` で並列列挙する
...
- `イベント:` で「発火するイベント / 遷移後の状態」を `/` 区切りで列挙する。複数イベントを並べる場合は `OR` を使う
