...
Issueの実装プランを作成し、`docs/plans/issue-{番号}.md` に保存する。プランには技術設計・タスク分解に加え、AC全項目と対応する**テストケース対応表**を含める。
...
- **プランファイルのパス**: `docs/plans/issue-{番号}.md`
- **レビュー基準**: ステップ2で読み込んだレビュー基準の全文（デフォルト＋プロジェクト固有）

**7b. レビュアーエージェントの起動:**

Agent tool（`subagent_type: dev-workflow:plan-reviewer`）で起動する。モデルは定義の `inherit` に従う。Agent toolが使えない場合や起動に失敗した場合は、`agents/plan-reviewer.md` の定義内容をプロンプト本文へ埋め込んでインラインで直接実行する（サブエージェント側からの定義ファイル再Readは行わない）。
