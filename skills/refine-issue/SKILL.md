...
2. **デフォルト**: 本スキルと同じリポジトリの `../../defaults/dor/definition.md`（本ファイルからの相対パス）
...
| スキル          | 目的                     | 出力                                           |
| --------------- | ------------------------ | ---------------------------------------------- |
| `/ready-check`  | Ready かどうかの簡易判定 | 合否判定 + 不足項目                            |
| `/refine-issue` | 詳細な精査と改善提案     | DoRチェック + 確認事項 + 改善提案 + コード調査 |

- **簡易判定**: まず `/ready-check` で Ready かどうかを確認
- **詳細精査**: Not Ready の場合や、作業開始前の詳細確認には `/refine-issue` を使用
...
2. 本スキルと同じリポジトリの `../../defaults/dor/definition.md`（本ファイルからの相対パス）
