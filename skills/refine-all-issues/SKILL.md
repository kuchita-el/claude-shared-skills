...
2. **デフォルト**: 本スキルのリポジトリの `defaults/dor/definition.md`
...
```bash
# ラベル作成（既に存在する場合はスキップ）
gh label create "needs-clarification" --description "精査の結果、確認事項あり" --color "d93f0b" 2>/dev/null || true
gh label create "ready-for-work" --description "精査完了、作業開始可能" --color "0e8a16" 2>/dev/null || true
```
...
Task toolを使用して、各Issueを並列で精査する。各エージェントに以下を指示:
...
2. 本スキルリポジトリの defaults/dor/definition.md
...
2. 本スキルリポジトリの `defaults/dor/definition.md`（デフォルト）
