---
description: 依存パッケージ更新の影響分析を6段階パイプラインで自動化する
allowed-tools:
  - Read
  - Grep
  - Glob
...
指定パッケージの更新について、バージョン解決からコード影響分析まで6段階で自動分析し、推奨アクションを提示する。
...
| レジストリ情報取得 | `npm info {pkg}` | `pnpm info {pkg}` | `yarn npm info {pkg}` | `bunx npm-info {pkg}` |
...
1. **現在バージョンの確認**: PM別コマンドで現在インストールされているバージョンを取得
2. **最新バージョンの確認**: PM別コマンドでレジストリから最新バージョン情報を取得
...
```
react@19.1.0 の peerDeps:
  react-dom: ^19.0.0 (現在: 18.2.0 → 非互換)
```

### Phase 4: 逆peerDeps特定

1. **対象パッケージに依存しているパッケージの特定**: PM別コマンド（explain/why）で逆依存を取得
2. **互換性の確認**: 逆依存パッケージが更新先バージョンをサポートしているか確認

出力例:

```
react@18.2.0 に依存しているパッケージ:
  react-router-dom@6.20.0 (peerDeps: react ^16.8 || ^17 || ^18 → react@19は範囲外)
  @testing-library/react@14.1.0 (peerDeps: react ^18.0.0 → react@19は範囲外)
```
...
- **PMコマンドの実行許可**: PM固有のコマンド（`npm ls`, `pnpm info` 等）は `allowed-tools` に含めていない。`settings.json` の `allowedTools` でプロジェクトごとに許可設定すること（例: `Bash(npm *)`, `Bash(pnpm *)`, `Bash(yarn *)`, `Bash(bun *)`）
...
- **将来の拡張**: 他の言語・エコシステム（Python/pip, Rust/cargo 等）への拡張は、Phase 0のPM検出とコマンド対応表を追加することで対応可能
