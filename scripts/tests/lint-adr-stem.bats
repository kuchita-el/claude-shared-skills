...
# 実測がある）。
#
# 【識別子部の抽出を形式検査より緩くする判断と、それを守る corpus】
# lint-adr.sh は識別子部の抽出を形式検査より緩くしている。抽出を形式検査と同じ厳しさへ
# 変えると、旧規約ファイル名どうしの重複が報告されなくなる。面⑤が使う
# invalid/28-legacy-duplicate-id corpus は、この判断を編集から守るために置かれている。
#
...
# 守る corpus であり、形式違反も同時に報告されるため識別子重複違反を明示的にアサートする。
@test "面⑤: 識別子重複の検出" {
    collect_init
...
# 既存の valid corpus にレイヤ5 が発火しないことも併せて固定する。
@test "面⑧: 誤検出の回避" {
    collect_init
    ...
    collect_not_contains "$output" "識別子重複違反" \
        '(AC5/レイヤ5-誤検出回避): 適合 corpus では発火しない: "識別子重複違反" を含まない'
    collect_not_contains "$output" "H1 整合違反" \
        '(AC5/レイヤ5-誤検出回避): 適合 corpus では発火しない: "H1 整合違反" を含まない'
