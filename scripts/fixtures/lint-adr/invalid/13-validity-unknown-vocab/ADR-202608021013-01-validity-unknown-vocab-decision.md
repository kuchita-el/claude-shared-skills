...
validity: 有郊
---
# ADR-202608021013-01-validity-unknown-vocab-decision: 語彙外 validity の決定

## Context

fixture 用。`validity` が `有効` の誤字（`有郊`）で、有効性語彙（`有効` / `上書き済み` / `廃止済み`）に属さない ADR。レイヤ1の語彙メンバシップ違反（validity）を検出させる。
