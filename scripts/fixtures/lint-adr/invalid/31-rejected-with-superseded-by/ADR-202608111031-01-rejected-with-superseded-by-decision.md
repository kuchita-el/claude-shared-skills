...
30-proposed-with-superseded-by が 提案中 側を担うのに対し、本 ADR 群は 却下 側を担う。種別7 の条件は `status=提案中` と `status=却下` の選言であり、既存の 14-proposed-with-validity / 15-rejected-with-validity（種別6）が両状態を別 fixture で押さえているのと対称に置く。
