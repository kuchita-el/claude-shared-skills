...
for id in "${retain_ids[@]}"; do
  if printf '%s\n' "${migration_ids[@]}" | grep -Fxq "$id"; then fail "retain候補をrelease/migrationへ追加しています: $id"; fi
done
check_row() {
...
if jq -e '.migrations[]? | select(.oldPluginId == "dev-workflow" and .newPluginId == "dev-workflow" and .oldEntryAction == "delete")' "$ledger" >/dev/null; then fail "protected old plugin ID: dev-workflow"; fi
...
actions_file="${4:-}"; [ -n "$actions_file" ] || usage; [ "$errors" -eq 0 ] || exit 1
mapfile -t add_ids < <(printf '%s\n' "${split_ids[@]}" | sed '/^$/d' | sort)
mapfile -t runner_ids < <(printf '%s\n' "${release_ids[@]}" | sed '/^$/d' | sort)
