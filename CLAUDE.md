...
配布プラグイン全件に共通して掛かる8つの設計原則（工場と配布物の資産の別、工場固有の工程を利用者へ課さないこと、独立起動できる能力、成果物の4分類、契約項目、承認の帰属、host 依存方向、正本の一意性）と、それぞれの適用対象・非適用条件・既知の例外は `docs/development/coding-agent-plugin-design-principles.md` に単一出典化している。新しいプラグインを起こすとき、既存プラグインへ能力を足すとき、配布物が何を宣言すべきか迷ったときに着手前に読む（本文を本節へ転記しない）。
...
上記は**作成時（静的）軸**のフットプリント規律である。スキル**実行中**にメイン context へ何を載せ／載せないか（参照ファイルのパス渡し・生ツール出力のファイル化・サブエージェント返却のサマリ化・滞留カテゴリ規則）は実行時フロー軸として `docs/references/context-budget.md`（ADR-202608281231-01）に単一出典化している。実行時の context 設計はそちらを参照する（本節へ転記しない）。
...
出力・成果物の分量と作業範囲の規律は `docs/behavior-invariants.md`（ADR-202607261002-01 / -02 / -03）に単一出典化している。モデル名・モデル版を直書きせず、挙動として観測される不変条件の形で記述する。新規スキル・サブエージェントを追加する際は、本規約を設計時の単一出典として確認し、実行時の具体的な責務・成果物・停止条件・変更境界は各定義へ記述する（本文を配布物へ転記しない）。
...
`docs/` 配下へ何を残し何を残さないかの規律と、恒久文書／過去記録／区分対象外の区分基準は `docs/references/document-permanence.md`（ADR-202608301012-01）に単一出典化している。`docs/principles.md`「ストック情報とフロー情報」の既存軸へ接続しており、新しい軸を立てない。`docs/` 配下の文書を追加・改廃する際は本規約を確認する（本文を本節へ転記しない）。
...
- 設計判断を行ったら、却下代替の必要条件と粒度判定基準に照らして ADR 化要否を判定する（判定基準は `manage-adr` スキルが規定する。由来の決定は ADR-202608011651-01 決定1）
