...
```bash
./setup-codex.sh --with-superpowers
```

Claude Code と Codex はプラグインのインストール状態を別々に管理するため、Claude Code のキャッシュは直接共有しません。`superpowers` は既存の設計判断どおり任意依存です。
...
Codex では `./setup-codex.sh --with-superpowers` を一度実行し、新しいセッションを開いてください。
